...
class WaymoHandler:
    def __init__(self, segment_path):
        ...
        self.dataset_scene_by_scene = self.extract_dataset_from_tfrecord(self.segment_path)
        self.frame_counter = 0
        device = cuda.get_current_device()
        device.reset()
        end2 = time.time()
        print("Dataset prepared, in " + str(end2 - start2) + " seconds.")
    ...
    def is_finished(self):
        if self.frame_counter < len(self.dataset_scene_by_scene) - 1:
            return False
        else:
            self.frame_counter = 0
            # return True

    def extract_dataset_from_tfrecord(self, path):
    ...
    def get_decoded_point_cloud(self, frame):
    ...
    def decode_camera_calibration(self, frame):
    ...
    def decode_vehicle_pose(self, frame, frame_id):
        ...
        return mat_trans_frame

    def decode_static_tf(self, frame):
