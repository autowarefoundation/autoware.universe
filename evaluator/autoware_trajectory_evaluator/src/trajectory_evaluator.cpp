...
void store_trajectory(const autoware_planning_msgs::msg::Trajectory::ConstSharedPtr traj_msg,
                      const nav_msgs::msg::Odometry::ConstSharedPtr odom_msg,
                      std::vector<TrajectoryWithTimestamp> &trajectory_history,
                      size_t traj_history_limit) 
{
    if (traj_msg != nullptr) {
        TrajectoryWithTimestamp trajectory_with_timestamp;
        const auto& stamp = odom_msg->header.stamp;
        trajectory_with_timestamp.time_stamp = static_cast<double>(stamp.sec) + static_cast<double>(stamp.nanosec) * 1e-9;
...
void on_kinematic_state(const nav_msgs::msg::Odometry::ConstSharedPtr odom_msg,
                        std::vector<TrajectoryWithTimestamp> &trajectory_history,
                        std::vector<TimeErrorData> &time_errors,
                        MetricArrayMsg &metrics_msg) 
{
    if (odom_msg != nullptr) {        
        ...
        segment_vector.x = next_closest_point.trajectory_point.pose.position.x - closest_point.trajectory_point.pose.position.x;
        segment_vector.y = next_closest_point.trajectory_point.pose.position.y - closest_point.trajectory_point.pose.position.y;
        segment_vector.z = next_closest_point.trajectory_point.pose.position.z - closest_point.trajectory_point.pose.position.z;
        ...
        double progress = vehicle_vector.x * segment_vector.x + 
                        vehicle_vector.y * segment_vector.y + 
        ...
        if (progress >= 0 && progress <= segment_length_squared) {
                double distance = autoware::universe_utils::calcDistance2d(pose, next_closest_point.trajectory_point.pose);
