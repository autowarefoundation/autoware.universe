...
: Node("trajectory_evaluator", node_options)
{
    ...
    // traj_history_limit = declare_parameter<double>("trajectory_history_limit");
...
TrajectoryEvaluatorNode::~TrajectoryEvaluatorNode()
{
    if (!output_file_str_.empty()) {
        ...
        f << "Stamp(ns),TrajectoryIndex,PositionX,PositionY,PositionZ,ExpectedTime,ActualTime,TimeError\n";

        for (const auto& entry : time_errors_) {
            f << entry.stamp.nanoseconds() << ","
...
void TrajectoryEvaluatorNode::store_trajectory(const autoware_planning_msgs::msg::Trajectory::ConstSharedPtr traj_msg, const nav_msgs::msg::Odometry::ConstSharedPtr odom_msg) {
    if (traj_msg != nullptr) {
        ...
        if (trajectory_history_.size() > traj_history_limit) {
    ...
    const float min_velocity)
{
    ...
    for (size_t idx = nearest_segment_idx + 1; idx < trajectory_points.size(); ++idx) {
        ...
        if (velocity != 0.0) {
            ...
            const double distance = autoware::universe_utils::calcDistance2d(from.trajectory_point.pose, to.trajectory_point.pose);
...
void TrajectoryEvaluatorNode::on_kinematic_state(const nav_msgs::msg::Odometry::ConstSharedPtr odom_msg) {
    if (odom_msg != nullptr) {        
        ...
        for (const auto& trajectory : trajectory_history_) {
            ...
            if (min_distance < 1e-6) {
                ...
                RCLCPP_INFO(get_logger(), "Expected Time at: %f seconds", closest_point.time_from_start.seconds());
