...
using EvalNode = segmentation_diagnostics::SegmentationEvaluatorNode;
...
protected:
  void SetUp() override
  {
    ...
    const auto share_dir = ament_index_cpp::get_package_share_directory("segmentation_evaluator");
    options.arguments(
      {"--ros-args", "--params-file", share_dir + "/param/segmentation_evaluator.defaults.yaml"});

    dummy_node = std::make_shared<rclcpp::Node>("segmentation_evaluator_test_node");
    ...
    pcl_pub_ =
      rclcpp::create_publisher<PointCloud2>(dummy_node, "/segmentation_evaluator/input/pcl", 1);
  ...
  {
    const auto metric_str = segmentation_diagnostics::metric_to_str.at(metric);
    ...
    metric_sub_ = rclcpp::create_subscription<DiagnosticArray>(
      dummy_node, "/segmentation_evaluator/metrics", 1,
  ...
  PointCloud2 makePointcloud()
  {
    PointCloud2 pcl;
    return pcl;
  }

  void publishPcl(const PointCloud2 & pcl)
  {
  ...
  double publishPclAndGetMetric(const PointCloud2 & pcl)
  {
  ...
  rclcpp::Publisher<PointCloud2>::SharedPtr pcl_pub_;
...
{
  setTargetMetric(segmentation_diagnostics::Metric::segment_stat);
}
