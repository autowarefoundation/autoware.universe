...
`ros2 run map_loader elevation_map_loader --ros-argrid_map_msgs::msg::GridMapgs -p param_file_path:=path/to/elevation_map_parameters.yaml -p elevation_map_file_path:=path/to/elevation_map/`
...
| elevation_map_file_path           | std::string | elvation_map file (bag2)                                                                                    | path_default  |
...
| use_lane_filter                   | bool        | Wheter to filter elevation_map with vecotor_map                                                             | false         |
| lane_margin                       | float       | Value of how much to expand the range of vector_map [m]                                                     | 0.5           |
| lane_height_diff_thresh           | float       | Only point clouds in the height range of this value from vecotor_map are used to generate elevation_map [m] | 1.0           |
| lane_filter_voxel_size_x          | float       | Voxel size x for calculating point clouds in vecotor_map [m]                                                | 0.04          |
| lane_filter_voxel_size_y          | float       | Voxel size y for calculating point clouds in vecotor_map [m]                                                | 0.04          |
| lane_filter_voxel_size_z          | float       | Voxel size z for calculating point clouds in vecotor_map [m]                                                | 0.04          |
...
| pcl_grid_map_extraction/grid_map/resolution | float | Resolution of the grid map. Width and lengts are computed automatically. | 0.3 |
...
| Name | Type | Description | Default value |
| :--------------------------------------------------------- | :---- | :----------------------------------------------------------------------------- | :------------ |
| pcl_grid_map_extraction/outlier_removal/is_remove_outliers | float | Whether to perform statisctical outlier removal. | false |
