...
TEST_F(TestPartialMapLoaderModule, LoadPartialPCDFiles)
{
  ...
  ASSERT_EQ(static_cast<int>(result->new_pointcloud_cells_with_metadata.size()), 1);
  EXPECT_EQ(result->new_pointcloud_cells_with_metadata[0].metadata.cell_id, "/tmp/dummy.pcd");
}
