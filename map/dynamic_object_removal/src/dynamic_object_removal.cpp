...
DynamicObjectRemoval::DynamicObjectRemoval() : Node("dynamic_object_removal_node")
{
  ...
  sync_ = std::make_shared<message_filters::Synchronizer<MySyncPolicy>>(
    MySyncPolicy(10), pcl_sub_, obj_sub_);
  ...
  const std::shared_ptr<const autoware_auto_perception_msgs::msg::DetectedObjects> & obj_msg)
{
  ...
  for (const auto & detected_obj : obj_msg->objects) {
    ...
    Eigen::Affine3d detected_obj_pose =
      Eigen::Translation3d(object_pose.position.x, object_pose.position.y, object_pose.position.z) *
      quat;
    ...
    min_point << -abs(object_dimension.x) / 2 - 0.25, -abs(object_dimension.y) / 2 - 0.25,
      -abs(object_dimension.z) / 2 - 0.1, 1.0;
    max_point << abs(object_dimension.x) / 2 + 0.25, abs(object_dimension.y) / 2 + 0.25,
      abs(object_dimension.z) / 2 + 0.25, 1.0;

    // Remove object using objectRemoveCropBox function
    objectRemoveCropBox(
      cloud, min_point, max_point, detected_obj_pose.translation().cast<float>(), yaw);
  }
...
void DynamicObjectRemoval::objectRemoveCropBox(
  PointCloudXYZI::Ptr crop_cloud, const Eigen::Vector4f min_point, const Eigen::Vector4f max_point,
  const Eigen::Vector3f translation, double orientation_yaw)
{
