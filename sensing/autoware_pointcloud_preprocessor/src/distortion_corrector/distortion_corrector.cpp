...
>>>>>>> original/main
{
  ...
  template <class T>
  std::deque<geometry_msgs::msg::Vector3Stamped>
  DistortionCorrector<T>::get_angular_velocity_queue()
  {
    ...
    std::deque<geometry_msgs::msg::Vector3Stamped>::iterator & it_imu)
  {
    ...
    if (use_imu && !angular_velocity_queue_.empty()) {
      ...
      it_imu = it_imu == std::end(angular_velocity_queue_) ? std::end(angular_velocity_queue_) - 1
                                                           : it_imu;
    }
  }
    ...
    const std::string & base_frame, const std::string & lidar_frame)
  {
    ...
    if (base_frame == lidar_frame) {
      ...
      try {
        ...
        RCLCPP_WARN(node_->get_logger(), "%s", ex.what());
        RCLCPP_ERROR(
          node_->get_logger(), "Please publish TF %s to %s", base_frame.c_str(),
          lidar_frame.c_str());
    ...
    const bool & is_twist_valid, const bool & is_imu_valid)
  {
    ...
    baselink_quat_.setValue(
      ...
      autoware::universe_utils::cos(theta_ * 0.5f));  // baselink_quat.setRPY(0.0, 0.0, theta);
                                                      // (Note that the value is slightly different)
