...
class TestDistortionCorrector(unittest.TestCase):
    ...
    def generate_pointcloud(num_points, header, timestamp_offset_per_point=0.01):
        ...
        timestamps = np.array(
            [header.stamp.sec + header.stamp.nanosec * 1e-9 + timestamp_offset_per_point * i for i in range(num_points)]
        ).astype(np.float64)
        xyz_data = points_xyz.tobytes()
        timestamp_data = timestamps.tobytes()
        pointcloud_data = b''.join(xyz_data[i*12:i*12+12] + timestamp_data[i*8:i*8+8] for i in range(len(xyz_data)))
        fields = [
    ...
    def test_node_link(self):
        ...
        velocity_pub = self.test_node.create_publisher(TwistWithCovarianceStamped, '/test/sensing/vehicle_velocity_converter/twist_with_covariance', 10)
        pointcloud_pub = self.test_node.create_publisher(PointCloud2, '/test/sensing/lidar/top/mirror_cropped/pointcloud_ex', qos_profile=sensor_qos)
        ...
        self.test_node.create_subscription(PointCloud2, '/test/sensing/lidar/top/rectified/pointcloud_ex', pointcloud_callback, qos_profile=sensor_qos)
        ...
        while self.test_node.get_clock().now() - start_time < rclpy.duration.Duration(seconds=self.evaluation_time):
        ...
        self.assertEqual(received_pointcloud.width, num_points, 'The received pointcloud data has a different length than expected')

        # Check that the received pointcloud data is different from the original one
        # For this, let's convert the pointcloud data into numpy arrays and compare them
        ...
        original_pointcloud_arr = pointcloud2_to_xyz_array(pointcloud_msg)
        received_pointcloud_arr = pointcloud2_to_xyz_array(received_pointcloud)

        # You might want to allow some tolerance because of potential numerical errors
        # Therefore, np.allclose() is used instead of np.array_equal()
        # Adjust the tolerance according to your needs
        self.assertFalse(np.allclose(original_pointcloud_arr, received_pointcloud_arr, atol=1e-6), 'The received pointcloud data is not different from the original one')
