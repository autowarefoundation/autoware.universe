...
namespace imu_corrector
{
  ...
  protected:
    ...
    GyroBiasEstimationModule module = GyroBiasEstimationModule(velocity_threshold, timestamp_threshold, data_num_threshold);
  };
  ...
  TEST_F(GyroBiasEstimationModuleTest, GetBiasEstimationWhenVehicleStopped)
  {
    ...
    for (size_t i = 0; i < data_num_threshold + 1; ++i) {
      module.update_velocity(i * 0.1 * timestamp_threshold, 0.0); // velocity = 0.0 < 1.0 = velocity_threshold
  ...
  TEST_F(GyroBiasEstimationModuleTest, GetInsufficientDataExceptionWhenVehicleMoving)
  {
    ...
    gyro.z = 0.3;
    for (size_t i = 0; i < data_num_threshold + 1; ++i) {
      module.update_velocity(i * 0.1 * timestamp_threshold, 5.0); // velocity = 5.0 > 1.0 = velocity_threshold
      module.update_gyro(i * 0.1 * timestamp_threshold, gyro);
    }
    ASSERT_THROW(module.get_bias(), std::runtime_error);
  }
