...
class pure_pursuit_gain_updater:
    def __init__(self, control_dt = 0.033, max_time=10.0, max_input_delay=1.5):
        ...
        self.max_queue_size = round(max_time / control_dt)
        self.max_input_delay_size = round(max_input_delay / control_dt)
    ...
    def get_acc_gain_scaling(self):
        ...
        acc_dot = (acc_array[1:] - acc_array[:-1]) / self.control_dt
        acc_estimation_error = np.inf
        best_acc_coef = np.array(
            [
                -self.control_dt / parameters.acc_time_constant,
                self.control_dt / parameters.acc_time_constant,
            ]
        )
    ...
    def get_steer_gain_scaling(self):
        ...
        estimated_wheel_base_scaling = 1.0 / np.clip(
            1 + np.linalg.solve(self.A_wheel_base, self.B_wheel_base)[1] * parameters.wheel_base,
            0.1,
            1.0,
        )

        steer_dot = (steer_array[1:] - steer_array[:-1]) / self.control_dt

        steer_estimation_error = np.inf
        best_steer_coef = np.array(
            [
                -self.control_dt / parameters.steer_time_constant,
                self.control_dt / parameters.steer_time_constant,
            ]
        )
        for i in range(self.max_input_delay_size):
