...
TEST_F(LgsvlInterfaceTest, GearMappingStateCommand)
{
  ...
  // Setup subscribtion
  auto handle_state_cmd = [&expected_result,
                           &test_completed](const lgsvl_interface::VSD::SharedPtr msg) -> void {
    EXPECT_EQ(msg->current_gear, expected_result.gear);
    test_completed = true;
  };
  const auto sub_node =
    std::make_shared<rclcpp::Node>("test_lgsvl_interface_sub_state_command", "/gtest");
  ...
  auto wait_for_subscription_callback = [&test_completed, &expected_result, &executor, this](
                                          lgsvl_interface::GEAR_TYPE gear,
  ...
  wait_for_subscription_callback(
    static_cast<lgsvl_interface::GEAR_TYPE>(99u), lgsvl_interface::VSD::GEAR_DRIVE);
}
...
TEST_F(LgsvlInterfaceTest, GearMappingStateReport)
{
  ...
  // Setup Node execution
  const auto pub_node =
    std::make_shared<rclcpp::Node>("test_lgsvl_interface_pub_state_report", "/gtest");
  rclcpp::executors::SingleThreadedExecutor executor;
  executor.add_node(node_);

  // Setup Publisher
  const auto pub_ptr =
    pub_node->create_publisher<lgsvl_msgs::msg::CanBusData>(sim_state_rpt_topic, rclcpp::QoS{10});
  wait_for_subscriber(pub_ptr);

  auto publish_gear_and_wait = [&cbd_msg, &pub_ptr,
                                &executor](lgsvl_interface::GEAR_TYPE gear) -> void {
