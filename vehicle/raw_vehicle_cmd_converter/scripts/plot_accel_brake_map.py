    ...
    try:
        ...
        if plot_dimension == 2:
            ...
            for vel_idx, vel in enumerate(vel_list):
                plt.plot(stroke_list, np.array(
                    acc_list).T[vel_idx], label='vel={}[km/h]'.format(vel))
            plt.legend()
        else:
            fig = plt.figure()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--plot-dimension', default=3,
                        type=int, help='dimension of plotting map')
