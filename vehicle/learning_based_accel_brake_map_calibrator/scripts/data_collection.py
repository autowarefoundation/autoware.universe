...
class primotest(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.SPEED_THRESHOLD = self.get_parameter(
            'speed_threshold').get_parameter_value().double_value
        self.STEERING_THRESHOLD = self.get_parameter(
            'steering_threshold').get_parameter_value().double_value
        self.THROTTLE_DEADZONE = self.get_parameter(
            'throttle_deadzone').get_parameter_value().integer_value
        self.BRAKE_DEADZONE = self.get_parameter(
            'brake_deadzone').get_parameter_value().integer_value
        self.MAX_VELOCITY = self.get_parameter('max_velocity').get_parameter_value().double_value
        self.THROTTLE_THRESHOLD1 = self.get_parameter(
            'throttle_threshold1').get_parameter_value().integer_value
        self.THROTTLE_THRESHOLD2 = self.get_parameter(
            'throttle_threshold2').get_parameter_value().integer_value
        self.BRAKE_THRESHOLD1 = self.get_parameter(
            'brake_threshold1').get_parameter_value().integer_value
        self.BRAKE_THRESHOLD2 = self.get_parameter(
            'brake_threshold2').get_parameter_value().integer_value
        self.CONSISTENCY_THRESHOLD = self.get_parameter(
            'consistency_threshold').get_parameter_value().integer_value
        # Get topic names from parameters
        self.pitch_topic = self.get_parameter('pitch_topic').get_parameter_value().string_value
        self.actuation_status_topic = self.get_parameter(
            'actuation_status_topic').get_parameter_value().string_value
        self.steering_status_topic = self.get_parameter(
            'steering_status_topic').get_parameter_value().string_value
        self.velocity_status_topic = self.get_parameter(
            'velocity_status_topic').get_parameter_value().string_value
        ...
        self.progress_bar0 = tqdm(initial=self.k, total=self.MAX_DATA,
                                  desc="                                        Low speed: 0 - Throttle deadzone  ", dynamic_ncols=True)
        self.progress_bar1 = tqdm(initial=self.i, total=self.MAX_DATA, desc="                                        Low speed: Throttle deadzone - " +
                                  str(self.THROTTLE_THRESHOLD1) + " ", dynamic_ncols=True)
        self.progress_bar2 = tqdm(initial=self.j, total=self.MAX_DATA, desc="                                        Low speed: Throttle " +
                                  str(self.THROTTLE_THRESHOLD1) + " - " + str(self.THROTTLE_THRESHOLD2) + "       ", dynamic_ncols=True)
        self.progress_bar3 = tqdm(initial=self.h, total=self.MAX_DATA, desc="                                        Low speed: Throttle > " +
                                  str(self.THROTTLE_THRESHOLD2) + "          ", dynamic_ncols=True)
        self.progress_bar4 = tqdm(initial=self.d, total=self.MAX_DATA,
                                  desc="                                        High speed: 0 - Throttle deadzone ", dynamic_ncols=True)
        self.progress_bar5 = tqdm(initial=self.a, total=self.MAX_DATA,
                                  desc="                                        High speed: Throttle deadzone - " + str(self.THROTTLE_THRESHOLD2), dynamic_ncols=True)
        self.progress_bar6 = tqdm(initial=self.b, total=self.MAX_DATA, desc="                                        High speed: Throttle " +
                                  str(self.THROTTLE_THRESHOLD1) + " - " + str(self.THROTTLE_THRESHOLD2) + "      ", dynamic_ncols=True)
        self.progress_bar7 = tqdm(initial=self.c, total=self.MAX_DATA, desc="                                        High speed: Throttle > " +
                                  str(self.THROTTLE_THRESHOLD2) + "         ", dynamic_ncols=True)
        self.progress_bar8 = tqdm(initial=self.kk, total=self.MAX_DATA,
                                  desc="                                        Low speed: 0 - Brake deadzone     ", dynamic_ncols=True)
        self.progress_bar9 = tqdm(initial=self.ii, total=self.MAX_DATA,
                                  desc="                                        Low speed: Brake deadzone - " + str(self.BRAKE_THRESHOLD1) + "    ", dynamic_ncols=True)
        self.progress_bar10 = tqdm(initial=self.jj, total=self.MAX_DATA, desc="                                        Low speed: Brake " +
                                   str(self.BRAKE_THRESHOLD1) + " - " + str(self.BRAKE_THRESHOLD2) + "          ", dynamic_ncols=True)
        self.progress_bar11 = tqdm(initial=self.hh, total=self.MAX_DATA, desc="                                        Low speed: Brake > " +
                                   str(self.BRAKE_THRESHOLD2) + "             ", dynamic_ncols=True)
        self.progress_bar12 = tqdm(initial=self.dd, total=self.MAX_DATA,
                                   desc="                                        High speed: 0 - Brake deadzone    ", dynamic_ncols=True)
        self.progress_bar13 = tqdm(initial=self.aa, total=self.MAX_DATA,
                                   desc="                                        High speed: Brake deadzone - " + str(self.BRAKE_THRESHOLD1) + "   ", dynamic_ncols=True)
        self.progress_bar14 = tqdm(initial=self.bb, total=self.MAX_DATA, desc="                                        High speed: Brake " +
                                   str(self.BRAKE_THRESHOLD1) + " - " + str(self.BRAKE_THRESHOLD2) + "         ", dynamic_ncols=True)
        self.progress_bar15 = tqdm(initial=self.cc, total=self.MAX_DATA, desc="                                        High speed: Brake > " +
                                   str(self.BRAKE_THRESHOLD2) + "            ", dynamic_ncols=True)

        self.create_subscription(Float32, self.pitch_topic, self.pitch_topic_callback, 1)
        self.create_subscription(ActuationStatusStamped,
                                 self.actuation_status_topic, self.actuation_topic_callback, 1)
        self.create_subscription(SteeringReport, self.steering_status_topic,
                                 self.steer_topic_callback, 1)
        self.create_subscription(VelocityReport, self.velocity_status_topic,
                                 self.velocity_topic_callback, 1)
    ...
    def collection_throttling(self):
        ...
        if (mean(self.queue_velocity) < 0):
            self.acc.append(-1*mean(self.queue_acceleration)-self.g *
                            math.sin(math.radians(-1*mean(self.queue_pitch_angle))))
            self.acc2.append(-1*mean(self.queue_acceleration))
            self.pitch.append(-1*mean(self.queue_pitch_angle))
        else:
            self.acc.append(mean(self.queue_acceleration)-self.g *
                            math.sin(math.radians(mean(self.queue_pitch_angle))))
            self.acc2.append(mean(self.queue_acceleration))
            self.pitch.append(mean(self.queue_pitch_angle))

        # save data in csv file
        dict1 = {'Velocity': self.vel, 'Throttling': self.cmd, 'Acceleration_with_pitch_comp': self.acc, 'Acceleration_measured': self.acc2, 'Pitch_angle': self.pitch, 'Low_V_0_deadzone': self.k,
                 'Low_V_deadzone_thr1': self.i, 'Low_V_thr1_thr2': self.j, 'Low_V_thr2_max': self.h, 'High_V_0_deadzone': self.d, 'High_V_deadzone_thr1': self.a, 'High_V_thr1_thr2': self.b, 'High_V_thr2_max': self.c}
    ...
    def collection_braking(self):
        ...
        if (mean(self.queue_velocity) < 0):
            self.accb.append(-1*mean(self.queue_acceleration)-self.g *
                             math.sin(math.radians(mean(self.queue_pitch_angle))))
            self.accb2.append(-1*mean(self.queue_acceleration))
            self.pitch2.append(-1*mean(self.queue_pitch_angle))
        else:
            self.accb.append(mean(self.queue_acceleration)-self.g *
                             math.sin(math.radians(mean(self.queue_pitch_angle))))
            self.accb2.append(mean(self.queue_acceleration))
            self.pitch2.append(mean(self.queue_pitch_angle))

        dict2 = {'Velocity': self.velb, 'Braking': self.cmdb, 'Acceleration_with_pitch_comp': self.accb, 'Acceleration_measured': self.accb2, 'Pitch_angle': self.pitch2, 'Low_V_0_deadzone': self.kk,
                 'Low_V_deadzone_thr1': self.ii, 'Low_V_thr1_thr2': self.jj, 'Low_V_thr2_max': self.hh, 'High_V_0_deadzone': self.dd, 'High_V_deadzone_thr1': self.aa, 'High_V_thr1_thr2': self.bb, 'High_V_thr2_max': self.cc}
