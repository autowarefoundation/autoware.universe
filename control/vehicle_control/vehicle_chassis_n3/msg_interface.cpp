...
void steer_cmd_callback(char * steer_data)
{
  ...
  cout << "The steer_cmd_callback ms: " << tv.tv_usec / 1000 << 
  "   SteeringAngle" << steering_data->SteeringAngle << endl;
...
int run(void * dora_context)
{
  ...
  while (true) {
    ...
    if (ty == DoraEventType_Input) {
      ...
      cout << endl << endl << "veh_control/msg_interface: " << endl;
...
//-->>条件编译
  ...
  //start_send_timer(20);
  pthread_t client_thread_id2 = start_test_control_thread();
  ...
  pthread_cancel(client_thread_id2);
  close(can_fd);
