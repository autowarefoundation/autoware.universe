...
// Time methods.
	...
	void interp1d_map_linear(
			std::vector<double>
			const& sbase_coord_vect,
			std::vector<double> const& base_data_vect,
			std::vector<double>
			const& snew_coords,
	                        ...
	                        )
	{
				...
				: snew_coords)
		{
			...
			auto const&& xk = base_data_vect[left] + ratio *
			                                         (base_data_vect[right] - base_data_vect[left]);
	...
	void interp1d_linear(
			std::vector<double>
			const& sbase_coord_vector,
			std::vector<double> const& base_data_data_vector,
