...
ns_splines::BSplineSmoother::BSplineSmoother(
  size_t const & base_signal_length, double const & num_of_knots_ratio,
  double const & smoothing_factor)
: lambda_{smoothing_factor}, npoints_{base_signal_length}, knots_ratio_{num_of_knots_ratio}
{
   ...
   *
   *   y = Basis* projection_mat * (any data of original length) - (Basis* projection_mat can be
   * reused). Same applies for the derivatives.
   * */
...
std::vector<double> ns_splines::BSplineSmoother::fPlusCube(
  double const & ti, double const & ki) const
{
  ...
  Eigen::MatrixXd & basis_mat, Eigen::MatrixXd & basis_dmat, Eigen::MatrixXd & basis_ddmat)
{
  ...
  auto const & Dc = basis_ddmat.transpose() * basis_ddmat;
  auto const & Bc = basis_mat.transpose() * basis_mat;
   ...
   *                          -- > ynew = Anew(t) * x  = Anew(t) * coeffs  = Anew(t) * inv(At*A)*A *
   * ydata
...
void ns_splines::BSplineSmoother::InterpolateInCoordinates(
  const Eigen::MatrixXd & ybase, Eigen::MatrixXd & data_tobe_interpolated) const
{
...
{
  Eigen::MatrixXd const & lambdaD = lambda_ * basis_ddmat;
...
void ns_splines::BSplineSmoother::getFirstDerivative(
  const Eigen::MatrixXd & ybase, Eigen::MatrixXd & ybase_dot) const
{
  ybase_dot = projection_mat_dot_wb_ * ybase;
}

void ns_splines::BSplineSmoother::getSecondDerivative(
  const Eigen::MatrixXd & ybase, Eigen::MatrixXd & ybase_dot_dot) const
{
