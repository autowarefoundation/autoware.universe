...
bool InterpolatingSplinePCG::Interpolate(
...
bool InterpolatingSplinePCG::compute_coefficients(std::vector<double> const & ybase)
{
  ...
  if (interp_type_ == 1 || is_a_constant_vector) {
    ...
    std::transform(
      ybase.cbegin(), ybase.cend(), b_coeff.cbegin(), std::back_inserter(coefficients_),
      [](auto & ai, auto & bi) {
...
void InterpolatingSplinePCG::set_bd_coeffs_(
...
bool InterpolatingSplinePCG::prepareCoefficients(
  std::vector<double> const & tbase, std::vector<double> const & ybase)
{
...
/*
 *  When we call interpolate method, interpolator checks whether there are existing parameters to
 * use. If we are not using the existing parameters we need to initialize the interpolator setting
 * reusing_coefficients=false, so that the interpolator re-computes the interpolation coefficients.
 * */
