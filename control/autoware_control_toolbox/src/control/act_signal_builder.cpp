  ...
  const Eigen::VectorXd & time_vec, const double & frequency_hz)
{
  ...
  auto sin_vec = Eigen::VectorXd(time_vec.unaryExpr([&w_](auto const & t) { return sin(w_ * t); }));
...
{
  double const w_ = 2 * M_PI * frequency_hz;  // [rad/sec]
  auto sqr_vec = Eigen::VectorXd(time_vec.unaryExpr([&w_](auto const & t) {
  ...
  const Eigen::VectorXd & time_vec, const double & frequency_hz)
{
  ...
  auto trg_vec = Eigen::VectorXd(time_vec.unaryExpr([&period, &tmax](auto const & t) {
    auto tn = t / tmax;
