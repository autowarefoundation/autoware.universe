...
def curvature(rdot, rddot):
    ...
    for k in range(rddot.shape[1]):
        cr = np.cross(rdot[:, k].flatten(), rddot[:, k].flatten())
        cv = cr / np.linalg.norm(rdot[:, k]) ** 3
        curvature.append(cv)
        crossprd.append(cr)

    return np.array(curvature), np.array(crossprd)
...
if __name__ == "__main__":
    ...
    xnoisy = x + (np.random.randn(nx) * noise_std * 1 + noise_std * 1) * 1
    ynoisy = y + (np.random.randn(nx) * noise_std * 1 + noise_std * 3) * 1
    ...
    '''
        BOTH of the ls_qr and Demmler-Reisch  methods can be used. 
        We don't prefer normal matrix solution of Least Squares due to information lost. 
    '''
    ...
    ## Curvature
    curv0, crossproduct = curvature(r0dot, r0ddot)
    ...
    curvsp, crprdct = curvature(rsdot, rsddot)
    ...
    # To see whether until which point we can trust the interpolator.
    plt.plot(curv0, 'r-.', alpha=0.6, lw=2, label='original curvature')
    plt.plot(curvsp, label='spline curvature')
    plt.grid()
    plt.legend()
    plt.show()
