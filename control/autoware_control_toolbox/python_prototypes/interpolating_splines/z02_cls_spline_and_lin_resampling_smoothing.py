...
def fT(nx, interpolation_type='spline'):
    ...
    if interpolation_type == 'spline':
        t = 0
        tprimitive = [1, t, t ** 2, t ** 3]
        tprimitive_dd = [0, 0, 2, 6 * t]

        T0 = block_diag(*[tprimitive] * mx_1)
        T0dd = block_diag(*[tprimitive_dd] * mx_1)

        t = 1
        tprimitive = [1, t, t ** 2, t ** 3]
        T1 = block_diag(*[tprimitive] * mx_1)
        T1dd = block_diag(*[tprimitive_dd] * mx_1)

        T = np.vstack((T0, T1))
        Tdd = np.vstack((T0dd, T1dd))
...
def my_cls_solve(A, b, C, d, Add, lmbda=10):
    """
    ...
    ## Define a weight matrix from Ad
    # Weight = lmbda * Add.T @ Add
    Weight = np.eye(m)

    Q, R = np.linalg.qr(np.vstack([Weight @ A, C]))
    ...
    invRtil = np.linalg.inv(Rtil.T)
    b2 = 2 * Qtil.T @ (Q1.T @ b) - 2 * invRtil @ d  # - lmbda * Qtil @ invRtil @ Add
    w = np.linalg.pinv(Rtil) @ b2

    # Substitute we and solve another equation
    Qd, Rd = np.linalg.qr(Add.T @ Add)

    invR = np.linalg.pinv(R + lmbda * Rd)
    xhat = invR @ (Q1.T @ b - Q2.T @ w / 2)  # - lmbda * Add
...
if __name__ == "__main__":
    ...
    interp_type = 'spline'
    nx = 100
    x = np.linspace(0, 10, nx) + np.random.randn(nx) * 0.2
    ...
    # due to the data matrix construction we need nx-1 size
    # of T and F
    A, C, Add = fT(nx=nx, interpolation_type=interp_type)
    ...
    eta_x = my_cls_solve(A, xdata, C, dc, Add)
    eta_y = my_cls_solve(A, ydata, C, dc, Add)
