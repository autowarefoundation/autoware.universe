...
TEST(ACTspline, splinePCapproximationVectorwise)
{
  ...
  std::transform(tbase.cbegin(), tbase.cend(), std::back_inserter(ybase), [&omega](auto const & t) {
    return sin(omega * t);
  });
  ...
  std::transform(
    tnew.cbegin(), tnew.cend(), std::back_inserter(yvalidation),
    [&omega](auto const & t) { return sin(omega * t); });
  ...
  /**
   * Interpolate and store the solutions in yinterp. The interpolator does not keep the computed
   * spline coefficients. This construction is memoryless.
   * */
  ...
  std::transform(
    yvalidation.cbegin(), yvalidation.cend(), yinterp.cbegin(),
    std::back_inserter(interpolation_error),
    [](auto const & x0, auto const & x1) { return (x1 - x0) * (x1 - x0); });

  auto const & sum_of_squares =
    std::accumulate(interpolation_error.cbegin(), interpolation_error.cend(), 0.);
...
TEST(ACTspline, splinePCapproximationPointwise)
{
  ...
  /**
   * Interpolate and store the solutions in yinterp. The interpolator does keep the computed spline
   * coefficients.
   * */
  std::vector<double> yinterp;
...
TEST(ACTspline, linePCGapproximation)
{
  ...
  std::vector<double> ybase;
  std::transform(
    tbase.cbegin(), tbase.cend(), std::back_inserter(ybase),
    [&ac, &bc](auto const & t) { return ac * t + bc; });
...
TEST(ACTspline, linePCGextrapolation)
{
  ...
  std::transform(
    yvalidation.cbegin(), yvalidation.cend(), yinterp.cbegin(),
    std::back_inserter(interpolation_error), [&ac, &bc, &tbase](auto const & y0, auto const & y1) {
