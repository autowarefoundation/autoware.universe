...
template <
  typename T,
  typename std::enable_if<std::is_floating_point<T>::value, std::vector<T>>::type * = nullptr>
...
template <
  class T, typename std::enable_if<std::is_floating_point<T>::value, bool>::type * = nullptr>
bool isEqual(T a, T b)
{
