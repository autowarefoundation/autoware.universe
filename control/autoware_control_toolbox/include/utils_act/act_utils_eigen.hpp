 ...
 * */
template <typename T>
eigen_dynamic_type<T> crossProduct(
  eigen_dynamic_type<T> const & r0, eigen_dynamic_type<T> const & r1)
{
  auto m = r0.rows();
...
std::vector<std::vector<double>> ToTriplets(
  const Eigen::MatrixBase<ArgType> & arg_mat, double const & eps_tol, size_t const & rowstart = 0,
 ...
 * */
template <typename T>
eigen_dynamic_type<T> difference_matrix_bspline(int const & n, int const & k, int const & d = 0)
{
