...
: SimModelInterface(6 /* dim x */, 2 /* dim u */),
  ...
  acc_scaling_factor_(std::max(acc_scaling_factor, 0.0)),
  steer_scaling_factor_(std::max(steer_scaling_factor, 0.0))
{
  ...
  const Eigen::VectorXd & state, const Eigen::VectorXd & input)
{
  ...
  const double acc_des =
    sat(input(IDX_U::ACCX_DES), vx_rate_lim_, -vx_rate_lim_) * acc_scaling_factor_;
  const double steer_des =
    sat(input(IDX_U::STEER_DES), steer_lim_, -steer_lim_) * steer_scaling_factor_;
