...
void SimplePlanningSimulator::initialize_vehicle_model()
{
  ...
  const double acc_scaling_factor = declare_parameter("acc_scaling_factor", 1.0);
  const double steer_scaling_factor = declare_parameter("steer_scaling_factor", 1.0);
  ...
  if (vehicle_model_type_str == "IDEAL_STEER_VEL") {
    ...
    vehicle_model_ptr_ = std::make_shared<SimModelDelaySteerAcc>(
      ...
      acc_scaling_factor, steer_scaling_factor);
