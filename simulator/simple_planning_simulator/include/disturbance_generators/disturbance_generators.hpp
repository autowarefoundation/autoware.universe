...
/**
 * @brief Identity class that maps the input as is when the time-delay value is zero in the
 * settings.
 * */
...
public:
  ...
  InputDisturbance_TimeDelayPade(
...
public:
   ...
   * @return pair_type pair ([ml, bl] , [mr, br])  where m is the slope of the deadzone, and b is
   * x-intercept of the deadzone boundary
   * */
  [[nodiscard]] virtual std::array<double, 4> getCurrentDeadZoneParameters() const
  {
    std::array<double, 4> deadzone_params{
      current_ml_slope_, current_bl_threshold_, current_mr_slope_, current_br_threshold_};
...
public:
  ...
  InputDisturbance_DeadZone(
    double const & m_lr_variance, double const & b_lr_mean, double const & b_lr_variance_in_percent,
    double const & sin_mag, bool const & use_time_varying_deadzone);

  double getDisturbedInput(double const & delta_u) override;

private:
  /**
   *  @brief [m, b]_{lr} are randomized. The equation of linear part (increasing) y = m*(x +
   * a*sin(2*pi*x+phi) - b).
   * */
  std::mt19937 generator_;
...
public:
  ...
  /**
   * @brief Single parameter output disturbance generator. For the acceleration, this parameter may
   * be {mu}-road surface friction, or time-varying slope variable.
   * */
...
public:
  ...
  OutputDisturbance_SlopeVariation(
    double const & rs_mean_slope_val, double const & rs_fluctuation_,
    double const & rs_angular_velocity, bool const & rs_use_time_varying_slope);
