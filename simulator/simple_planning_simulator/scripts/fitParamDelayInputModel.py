...
def getLinearInterpolate(_tm, _val, _index, ti):
    ...
    val_i = tmp_val + (tmp_next_val - tmp_val) / (tmp_next_t - tmp_t) * (ti - tmp_t)
    return val_i


def getFittingTimeConstantParam(cmd_data, act_data, delay, args, speed_type=False):
    ...
    if args.cutoff_freq > 0:
        ...
        diff_act_cmd_sample = lowpass_filter(diff_act_cmd_sample, cutoff_freq=args.cutoff_freq)
    ...
    error = np.linalg.norm(diff_act_cmd_sample + tau * d_act_sample) / d_act_sample.shape[1]
    return tau, error
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bag_file", "-b", required=True, type=str, help="rosbag file", metavar="file"
    )
    parser.add_argument(
        "--cutoff_time",
        default=1.0,
