...
private:
  ...
  std::vector<int> map_sig_vec_to_pyin;  // index in "map_sig_vec_to_pyin" is index in "py_inputs" and value in "map_sig_vec_to_pyin" is index in "signals_vec_names"
  std::vector<int> map_pyout_to_sig_vec;  // index in "map_pyout_to_sig_vec" is index in "pymodel_outputs" and value in "map_pyout_to_sig_vec" is index in "signals_vec_names"
...
public:
  ...
  {
    // index in "map_sig_vec_to_pyin" is index in "py_inputs" and value in "map_sig_vec_to_pyin" is index in "signals_vec_names"
  ...
  {
    // index in "map_pyout_to_sig_vec" is index in "pymodel_outputs" and value in "map_pyout_to_sig_vec" is index in "signals_vec_names"
...
private:
  std::vector<int> createConnectionsMap(std::vector<char*> connection_names_1, std::vector<char*> connection_names_2){
    ...
    // index in "connection_map" is index in "connection_names_2" and value in "connection_map" is index in "connection_names_1"
    for (auto name_2 : connection_names_2){
      ...
      for (size_t NAME_1_IDX = 0; NAME_1_IDX < connection_names_1.size(); NAME_1_IDX++){
        if (strcmp(name_2, connection_names_1[NAME_1_IDX]) == 0){  // 0 means strings are the same and we can create connection
  ...
  std::vector<double> fillVectorUsingMap(std::vector<double> vector1, std::vector<double> vector2, std::vector<int> map, bool inverse){
