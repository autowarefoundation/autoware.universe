...
void LidarMarkerDetector::points_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr & points_msg_ptr)
{
  ...
  const auto marker_pose_on_map_from_lanelet2_map = *std::min_element(std::begin(marker_pose_on_map_arrary_), std::end(marker_pose_on_map_arrary_), 
    [&self_pose_msg](const auto& lhs, const auto& rhs) 
    {
      return tier4_autoware_utils::calcDistance3d(lhs.position, self_pose_msg.pose.pose.position) < tier4_autoware_utils::calcDistance3d(rhs.position, self_pose_msg.pose.pose.position);
    }
  );

  const auto marker_pose_on_base_link_from_lanele2_map = tier4_autoware_utils::inverseTransformPoint(marker_pose_on_map_from_lanelet2_map.position, self_pose_msg.pose.pose);
  ...
  diff_position_from_self_position_to_lanelet2_map.x = marker_pose_on_map_from_lanelet2_map.position.x - marker_pose_on_map_from_self_pose.pose.position.x;
  diff_position_from_self_position_to_lanelet2_map.y = marker_pose_on_map_from_lanelet2_map.position.y - marker_pose_on_map_from_self_pose.pose.position.y;
