...
void PCDMetadata::import(const std::string& pcd_metadata_path, const std::string& pcd_dir)
{
    ...
    for (const auto& node : config)
    {
        ...
        if (
            ...
            node.first.as<std::string>() == "lower_x" ||
            node.first.as<std::string>() == "lower_y" ||
            node.first.as<std::string>() == "tag_name") 
        {
            continue;
        }
        ...
        if (std::filesystem::exists(abs_path_pcd.str()) && std::filesystem::is_regular_file(abs_path_pcd.str()))
        {        
...
void PCDMetadata::segmentIndexToBoundary(const SegmentIndex & sid, float & min_x, float & min_y, float & max_x, float & max_y)
{
