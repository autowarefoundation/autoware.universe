...
PoseArrayInterpolator::PoseArrayInterpolator(
  ...
  const std::deque<PoseWithCovarianceStamped::ConstSharedPtr> & pose_msg_ptr_array): logger_(node->get_logger())
{
  success_ = true;
  ...
  if (pose_msg_ptr_array.size() <= 1) {
    RCLCPP_WARN_STREAM_THROTTLE(logger_, *node->get_clock(), 1, "No Pose!");
    success_ = false;
    return;
  }
