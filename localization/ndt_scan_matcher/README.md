...
| `is_activated`                            | whether the node is in the "activate" state or not                                     | "activate" state                                                                                                                                                                                                    | not "activate" state                                                                                                                                                                                                                                                                                                                                                     | none                                     | no                                                                       |
...
| `local_optimal_solution_oscillation_num`  | the number of times the solution is judged to be oscillating                           | the number of times is smaller than 10                                                                                                                                                                              | the number of times is larger than 10                                                                                                                                                                                                                                                                                                                                    | none                                     | yes                                                                      |
...
| `distance_initial_to_result`              | the distance between the position before convergence processing and the position after | the distance is **shorter** than 3                                                                                                                                                                                  | the distance is **longer** than 3                                                                                                                                                                                                                                                                                                                                        | none                                     | no                                                                       |
...
| `is_activated`     | whether the node is in the "activate" state or not | "activate" state           | not "activate" state            | none                          |
...
| `is_need_rebuild`                                   | whether it need to rebuild the map. If the map has not been loaded yet or if `distance_last_update_position_to_current_position encounters` is an Error state, it is considered necessary to reconstruct the map, and `is_need_rebuild` becomes `True`. | none                                                                                                     | none                            | none                                                                                                    |
| `is_updated_map`                                    | whether map is updated. If the map update couldn't be performed or there was no need to update the map, it becomes `False`                                                                                                                              | `is_updated_map` is `True`. Or both `is_updated_map` and `is_need_rebuild` are `False`.                  | none                            | `is_updated_map` is `False` but `is_need_rebuild` is `True`                                             |
...
| `maps_to_remove_size`                               | the number of maps to be removed                                                                                                                                                                                                                        | none                                                                                                     | none                            | none                                                                                                    |
