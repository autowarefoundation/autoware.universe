...
void GyroOdometer::callbackImu(const sensor_msgs::msg::Imu::ConstSharedPtr imu_msg_ptr)
{
  ...
  twist.header = imu_msg_ptr_->header;
  twist.twist.linear = twist_with_cov_msg_ptr_->twist.twist.linear;
  twist.twist.angular.x = imu_msg_ptr_->angular_velocity.x;
  twist.twist.angular.y = imu_msg_ptr_->angular_velocity.y;
  twist.twist.angular.z = imu_msg_ptr_->angular_velocity.z;
  twist_raw_pub_->publish(twist);

  geometry_msgs::msg::TwistWithCovarianceStamped twist_with_covariance;
  twist_with_covariance.header = imu_msg_ptr_->header;
  twist_with_covariance.twist.twist.linear = twist_with_cov_msg_ptr_->twist.twist.linear;
  twist_with_covariance.twist.twist.angular.x = imu_msg_ptr_->angular_velocity.x;
  twist_with_covariance.twist.twist.angular.y = imu_msg_ptr_->angular_velocity.y;
  twist_with_covariance.twist.twist.angular.z = imu_msg_ptr_->angular_velocity.z;
  ...
  if (
    std::fabs(imu_msg_ptr_->angular_velocity.z) < 0.01 &&
