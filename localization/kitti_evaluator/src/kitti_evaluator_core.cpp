...
: rclcpp::Node("kitti_evaluator")
{
    ...
    std::string vehicle_pose_topic = declare_parameter("input_vehicle_pose_topic", "/localization/pose_twist_fusion_filter/pose");
    std::string ground_truth_pose_topic = declare_parameter("input_ground_truth_pose_topic", "/ground_truth");
    std::string error_pose_topic = declare_parameter("output_pose_error_topic", "/relative_pose_error");
...
void KittiEvaluator::callbackGroundTruthOdometry(geometry_msgs::msg::PoseStamped::ConstSharedPtr pose_ground_truth_msg_ptr)
{
    ...
    if(!vehicle_pose_queue.empty())
    {
        ...
        // RCLCPP_INFO(get_loggerrocker -e LIBGL_ALWAYS_SOFTWARE=1 --x11 --user --volume ~/adehome -- ghcr.io/autowarefoundation/autoware-universe:latest(), "%lf %lf %lf %lf", vehicle_trans(1,0), vehicle_trans(1,1), vehicle_trans(1,2), vehicle_trans(1,3));
...
void KittiEvaluator::calculateError(Eigen::Affine3d & vehicle_trans, Eigen::Affine3d & groud_truth_trans, Eigen::Affine3d & error_trans)
{
