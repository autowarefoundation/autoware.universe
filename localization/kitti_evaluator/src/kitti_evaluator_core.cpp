  ...
  geometry_msgs::msg::PoseStamped::ConstSharedPtr pose_ground_truth_msg_ptr)
{
  ...
  while (!vehicle_pose_queue.empty() &&
         rclcpp::Time(vehicle_pose_queue.front()->header.stamp) <
           rclcpp::Time(curr_time) - rclcpp::Duration::from_seconds(delta)) {
  ...
  geometry_msgs::msg::PoseStamped::ConstSharedPtr & pose_groud_truth)
{
  ...
  // RCLCPP_INFO(get_logger(), "%lf %lf %lf %lf", curr_transform_groud_truth(0,0),
  // curr_transform_groud_truth(0,1), curr_transform_groud_truth(0,2),
  // curr_transform_groud_truth(0,3));

  Eigen::Affine3d delta_ground_truth =
    last_transform_groud_truth.inverse() * curr_transform_groud_truth;
