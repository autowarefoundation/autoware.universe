...
void AutowarePoseCovarianceModifierNode::check_gnss_pose_timeout()
{
  ...
  if (time_diff.seconds() > gnss_pose_timeout_sec_) {
    ...
    pose_source_ = AutowarePoseCovarianceModifierNode::PoseSource::NDT;
    std_msgs::msg::String msg_str_pose_selected;
    msg_str_pose_selected.data = "NDT";
    pose_source_pub_->publish(msg_str_pose_selected);
  }
}
  ...
  std::array<double, 36> & ndt_covariance_in)
{
  ...
  auto interpolate = [](double x, double x_min, double x_max, double y_min, double y_max) {
  ...
  auto modify_covariance = [&](double ndt_covariance, double gnss_covariance) {
    ...
    const double ndt_std_dev = std::sqrt(ndt_covariance);
    const double gnss_std_dev = std::sqrt(gnss_covariance);

    const double ndt_std_dev_min = ndt_std_dev;
    const double ndt_std_dev_max = ndt_std_dev * 5;

    const double delta_ndt_std_dev = ndt_std_dev_max - ndt_std_dev_min;
    const double delta_gnss_std_dev = gnss_stddev_reliable_max_ - gnss_stddev_unreliable_min_;

    const double calculated_std_dev =
      ndt_std_dev_max -
      delta_ndt_std_dev * ((gnss_stddev_reliable_max_ - gnss_std_dev) / delta_gnss_std_dev);

    const double interpolated_std_dev = interpolate(
      gnss_std_dev, gnss_stddev_unreliable_min_, gnss_stddev_reliable_max_, ndt_std_dev_min,
      ndt_std_dev_max);

    const double calculated_covariance = std::pow(calculated_std_dev, 2);
