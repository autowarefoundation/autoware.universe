...
{
  if (particles_circular_buffer_.size() < 2 || pose_circular_buffer_.size() == 0) {
    return;
  }
  ...
  if (
    rclcpp::Time(particles_circular_buffer_.front().header.stamp) <
    rclcpp::Time(pose_circular_buffer_.back().header.stamp)) {
    return;
  }

  geometry_msgs::msg::PoseWithCovarianceStamped synced_pose{pose_circular_buffer_.back()};
  pose_circular_buffer_.pop_back();
...
modularized_particle_filter_msgs::msg::ParticleArray GNSSPoseCorrector::calculateWeightedParticles(
...
{
  modularized_particle_filter_msgs::msg::ParticleArray weighted_particles{predicted_particles};

  float sigma{static_cast<float>(std::sqrt(
