      ...
      resampling_interval_seconds_(declare_parameter("resampling_interval_seconds", 1.0f))
{
  ...
  tf2_broadcaster_ptr_ = std::make_shared<tf2_ros::TransformBroadcaster>(
      std::shared_ptr<rclcpp::Node>(this, [](auto) {}));

  predicted_particles_pub_ = this->create_publisher<modularized_particle_filter_msgs::msg::ParticleArray>(
      "predicted_particles", 10);
  resampled_particles_pub_ = this->create_publisher<modularized_particle_filter_msgs::msg::ParticleArray>(
      "resampled_particles", 10);
  mean_pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("predicted_mean_pose", 10);

  initialpose_sub_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
      "initialpose", 1,
      std::bind(&Predictor::initialposeCallback, this, std::placeholders::_1));
  twist_sub_ = this->create_subscription<geometry_msgs::msg::TwistWithCovarianceStamped>(
      "twist_with_covariance", 10,
      std::bind(&Predictor::twistCallback, this, std::placeholders::_1));
  weighted_particles_sub_ = this->create_subscription<modularized_particle_filter_msgs::msg::ParticleArray>(
    ...
    const geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr initialpose)
{
  ...
  const float yaw{
      static_cast<float>(tf2::getYaw(initialpose->pose.pose.orientation))};
...
void Predictor::timerCallback()
{
  ...
  const float dt = static_cast<float>((current_time - rclcpp::Time(particle_array.header.stamp)).seconds());
  ...
  for (int i{0}; i < particle_array.particles.size(); i++) {
    ...
    const float wz{
        twist.twist.twist.angular.z + prediction_util::nrand(4 * sqrt(twist.twist.covariance[5 * 6 + 5]))};
    tf2::Quaternion q;
  ...
  modularized_particle_filter_msgs::msg::Particle mean_state{
      calculateMeanState(particle_array)};
    ...
    const modularized_particle_filter_msgs::msg::ParticleArray::ConstSharedPtr weighted_particles_ptr)
{
  ...
  std::optional<modularized_particle_filter_msgs::msg::ParticleArray>
      resampled_particles{resampler_ptr_->resampling(particle_array)};
