...
public:
  ParticleFilter() : Node("particle_filter"), frame_id_("map"), child_frame_id_("base_link")
  {
    ...
    subscriber = std::make_shared<SyncroSubscriber>(
      rclcpp::Node::SharedPtr{this}, "/ll2_image", "/projected_image");
    subscriber->setCallback(std::bind(&ParticleFilter::syncrhoCallback, this, _1, _2));

    sub_pose_stamped_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      pose_topic, 10, std::bind(&ParticleFilter::poseCallback, this, _1));
