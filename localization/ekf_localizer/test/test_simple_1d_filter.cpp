...
TEST(Simple1DFilter, InitializationAndAccess)
{
  ...
  int64_t time_in_ns =
    std::chrono::duration_cast<std::chrono::nanoseconds>(initial_time.time_since_epoch()).count();
...
TEST(Simple1DFilter, Update)
{
  ...
  int64_t new_time_in_ns =
    std::chrono::duration_cast<std::chrono::nanoseconds>(new_time.time_since_epoch()).count();
...
TEST(Simple1DFilter, MahalanobisGate)
{
  ...
  EXPECT_EQ(
    filter.get_x(), 10.0);  // The update should have been ignored, so x_ should still be 10.0
}
