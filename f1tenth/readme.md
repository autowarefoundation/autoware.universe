...
## Flash JetPack 5.1.1 to Jetson Xavier NX (Approximate Time Investment: 1-1.5 hours)
...
## Install ROS2 galactic (Approximate Time Investment: 15 minutes)
...
## Set up Autoware development environment (Approximate Time Investment: 30 minutes)
...
## Set up Autoware workspace (Approximate Time Investment: 6-7 hours)
...
2. Launch the `teleop_twist_keyboard` node for keyboard tele-operation. Focus on (select) this terminal and use `U`, `I`, `O` keys to manually control the f1tenth car in the simulation.
...
The default path for the recording is at `"/tmp/path"`. The recording will be automatically erased after reboot. 
...
2. Replay a trajectory from your previously saved file.
