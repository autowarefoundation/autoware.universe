...
#ifndef F1TENTH__PARTICLE_FILTER__RANGE_LIBC__VENDOR__DISTANCE_TRANSFORM_H_
#define F1TENTH__PARTICLE_FILTER__RANGE_LIBC__VENDOR__DISTANCE_TRANSFORM_H_
...
/// The MArray class represents a D-dimensional dope vector
/// (https://en.wikipedia.org/wiki/Dope_vector) of scalar type T. Given an array stored sequentially
/// in memory, this class wraps it provinding a multi-dimensional matrix interface. It is possible
/// to take slices, windows or even permutations without actually modifying the underlying memory,
/// no element hence moved.
...
public:
  ...
  {
    for (std::size_t i = 0; i < D; ++i) _size[i] = _array ? size[i] : 0;
    _offset[D - 1] = 1;
    for (std::size_t j = D - 1; j > 0; --j) _offset[j - 1] = _size[j] * _offset[j];
  }
   ...
   */
  MArray(
    const T * array, const std::size_t accumulatedOffset, const std::size_t size[D],
  ...
  /**
   *    @brief Reorders the sub-matrixes s.t. the one at 0 <= i < D goes to 0 <= order[i] < D, i.e.
   * m[*]..[*]_i...[*] swaps with m[*]...[i]...[*]_order[i]...[*].
   *    @param order              A permutation of the matrix indices.
   *    @param p                  The output permuted matrix.
   *    @note Example: transpose a 2D matrix by swapping the access indices - MArray<T,2> m, mt;
   * std::size_t trans_ord[2] = {1, 0}; m.permute(trans_ord, mt);
   */
  inline void permute(const std::size_t order[D], MArray<T, D> & p)
  {
    bool included[D];
    for (std::size_t d = 0; d < D; ++d) included[d] = false;
   ...
   *    @note Example: transpose a 2D matrix by swapping the access indices - MArray<T,2> m, mt;
   * std::size_t trans_ord[2] = {1, 0}; mt = m.permute(trans_ord);
   */
  ...
  inline void window(const std::size_t start[D], const std::size_t size[D], MArray<T, D> & p)
  {
    ...
    p._accumulatedOffset = _accumulatedOffset;
    for (std::size_t d = 0; d < D; ++d) p._accumulatedOffset += _offset[d] * start[d];
  ...
  {
    for (std::size_t i = 0; i < D; ++i) s[i] = _size[i];
  }
  ...
  inline std::size_t totalSize() const
  {
    ...
    for (std::size_t i = 1; i < D; ++i) total *= _size[i];
    return total;
  }
   ...
   *    @return The total offset from the beggining of the stored array of the i-th element at
   * dimension d.
   */
...
private:
  ...
  std::size_t _accumulatedOffset;  ///< Offset of the first element of this matrix from the
                                   ///< beginning of the stored array.
  std::size_t _size[D];            ///< Sizes of this matrix, for each dimension.
  std::size_t _offset[D];  ///< Jumps' offsets from the beginning of a "row" to the beginning of the
                           ///< next one, for each dimension.
};

/// The MArray class represents a 1-dimensional dope vector
/// (https://en.wikipedia.org/wiki/Dope_vector) of scalar type T. Given an array stored sequentially
/// in memory, this class wraps it provinding a mono-dimensional matrix interface. It is possible to
/// take slices, windows or even permutations without actually modifying the underlying memory, no
...
public:
   ...
   */
  MArray(
    const T * array, const std::size_t accumulatedOffset, const std::size_t size,
   ...
   */
  MArray(
    const T * array, const std::size_t accumulatedOffset, const std::size_t size[1],
...
private:
  ...
  std::size_t _accumulatedOffset;  ///< Offset of the first element of this vector from the
                                   ///< beginning of the stored array.
...
public:
   ...
   *    @param size               Sizes of the D-dimensional matrix.
   */
  MMArray(const std::size_t size[D]) : MArray<T, D>(), _arrayPtr(nullptr) { resize(size); }

  /**
   *    @brief Copy constructor.
   */
  MMArray(const MMArray<T, D> & mma) : MArray<T, D>(), _arrayPtr(nullptr) { *this = mma; }

  /**
   *    @brief Move constructor.
   */
  MMArray(MMArray && mma)
  : MArray<T, D>(std::forward<MMArray<T, D>>(mma)), _arrayPtr(std::move(mma._arrayPtr))
  {
  }
  ...
  inline void resize(const std::size_t size[D])
  {
    ...
    for (std::size_t i = 1; i < D; ++i) total *= size[i];
    if (total > 0) {
...
private:
  std::unique_ptr<T[]>
    _arrayPtr;  ///< Sharp pointer in memory to this matrix, with automatic storage (de)allocation.
};
...
/// The DistanceTransform class provides static method to compute a distance field over any
/// multi-dimensional regularly sampled function. The dimension is fixed at compile time. It is also
/// possible to compute the index of the nearest minimum of each sample.
class DistanceTransform
{
public:
  /**
   *    @brief Compute the L2-norm distance field D of a DIM-dimensional sampled function f. D gets
   * the distance from the local minima of f.
   *    @param f              A DIM-dimensional, regularly sampled function.
   *    @param D              The resulting distance field of f.
   *    @param squared        Compute squared distances (L2)^2 - avoiding to compute square roots -
   * (true) or keep them normal (false - default).
  ...
  inline static void distanceTransformL2(
    const MMArray<Scalar, DIM> & f, MMArray<Scalar, DIM> & D, const bool squared = false)
  {
    if (&D != &f) {
      std::size_t size[DIM];
      f.size(size);
    ...
    if (DIM % 2 == 0) D = std::move(fCopy);
    if (!squared) element_wiseSquareRoot(D);
  }

  /**
   *    @brief Compute the L2-norm distance field D of a 1-dimensional sampled function f. D gets
   * the distance from the local minima of f.
  ...
  inline static void distanceTransformL2(
    const MMArray<Scalar, 1> & f, MMArray<Scalar, 1> & D, const bool squared = false)
  {
    std::size_t size[1];
    f.size(size);
    if (&D != &f) D.resize(size);
    distanceL2(f, D);
    if (!squared) {
  ...
  /**
   *    @brief Compute the L2-norm distance field D of a DIM-dimensional sampled function f. D gets
   * the distance from the local minima of f. Compute also the (index of the) nearest local minimum
   * of each sample.
   *    @param f              A DIM-dimensional, regularly sampled function.
   *    @param D              The resulting distance field of f.
   *    @param I              Resulting array containing the (index of the) local minimum for each
   * sample.
   *    @param squared        Compute squared distances (L2)^2 - avoiding to compute square roots -
   * (true) or keep them normal (false - default).
   *    @note Arrays f and D can also be the same.
  ...
  template <typename Scalar = float, std::size_t DIM = 2>
  inline static void distanceTransformL2(
    const MMArray<Scalar, DIM> & f, MMArray<Scalar, DIM> & D, MMArray<std::size_t, DIM> & I,
  ...
  template <typename Scalar = float>
  inline static void distanceTransformL2(
    const MMArray<Scalar, 1> & f, MMArray<Scalar, 1> & D, MMArray<std::size_t, 1> & I,
    const bool squared = false)
  {
    std::size_t size[1];
    f.size(size);
  ...
  {
    for (std::size_t q = 0; q < I.size(); ++q) I[q] = I.accumulatedOffset(q);
  }

private:
  /**
   *    @brief The actual distance field computation is done by recursive calls of this method, in
   * lower dimenional sub-functions.
   *    @param f              A DIM-dimensional, regularly sampled function.
   *    @param D              The resulting distance field of f.
   */
  ...
  /**
   *    @brief The actual distance field computation as in the "Distance Transforms of Sampled
   * Functions" paper, performed in a mono-dimensional function.
   *    @param f              A 1-dimensional, regularly sampled function.
   *    @param D              The resulting distance field of f.
   */
  template <typename Scalar = float>
  ...
  {
    if (f.size() == 0 || f.size() > D.size()) return;
    if (f.size() == 1) {
      D[0] = f[0];
      return;
    }
    ...
    for (std::size_t q = 1; q < f.size(); ++q) {
      ...
      do {
        --k;
        // compute horizontal position of intersection between the parabola from q and the current
        // lowest parabola
  ...
  inline static void distanceL2(
    const MArray<Scalar, DIM> & f, MArray<Scalar, DIM> & D, const MArray<std::size_t, DIM> & Ipre,
  ...
  template <typename Scalar = float>
  inline static void distanceL2(
    const MArray<Scalar, 1> & f, MArray<Scalar, 1> & D, const MArray<std::size_t, 1> & Ipre,
...
public:
  ...
  {
    for (std::size_t q = 0; q < m.size(); ++q) m[q] = static_cast<Scalar>(std::sqrt(m[q]));
  }
};

}  // namespace dt

#endif  // F1TENTH__PARTICLE_FILTER__RANGE_LIBC__VENDOR__DISTANCE_TRANSFORM_H_
