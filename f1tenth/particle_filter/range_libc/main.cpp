...
DEFINE_string(
  trace_path, "",
  "Path to output trace map of memory access pattern. Works for Bresenham's Line or Ray Marching.");
...
// mark.getMap()->saveTrace(FLAGS_trace_path);
  ...
  if (
    utils::has("CDDTCast", methods) || utils::has("cddt", methods) ||
    ...
    if (utils::has("PrunedCDDTCast", methods) || utils::has("pcddt", methods)) {
      ...
      if (DO_LOG) {
        tlog.str("");
        mark.set_log(&tlog);
        summary << "pcddt," << construction_dur.count() + prune_dur.count() << "," << rc.memory()
                << std::endl;
      }
      if (FLAGS_which_benchmark == "grid")
        mark.grid_sample(GRID_STEP, GRID_RAYS, GRID_SAMPLES);
  ...
  if (utils::has("RayMarchingGPU", methods) || utils::has("rmgpu", methods)) {
    ...
    if (FLAGS_which_benchmark == "grid") {
      ...
      Benchmark<RayMarching>::get_grid_samples(
        samples, GRID_STEP, GRID_RAYS, GRID_SAMPLES, map.width, map.height);
      // warmup
    ...
    if (FLAGS_which_benchmark == "random") {
      ...
      // samples[3*i+2] << std::endl; 		std::cout << "   expected: " << rm.calc_range(samples[3*i],
      // samples[3*i+1], samples[3*i+2]) << std::endl; 		std::cout << "   got: " << outs[i] <<
      // std::endl;
      // 	}
      // }
    }
