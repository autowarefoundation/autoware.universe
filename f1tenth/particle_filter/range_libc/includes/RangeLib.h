...
#ifndef F1TENTH__PARTICLE_FILTER__RANGE_LIBC__INCLUDES__RANGELIB_H_
#define F1TENTH__PARTICLE_FILTER__RANGE_LIBC__INCLUDES__RANGELIB_H_
...
// #define _DO_MOD 0 // this might not be necessary (aka 1 & 0 might be equivalent), will evaluate
// later #define _NO_INLINE 0
//
#if _USE_LRU_CACHE
...
// this stuff is for ROS integration, not necessary for raw usage
  ...
  bool saveTrace(std::string filename)
  {
    ...
    unsigned error =
      lodepng::encode(png, reinterpret_cast<const unsigned char *>(image), width, height, state);
  ...
  bool error() { return has_error; }

  // returns memory usage in bytes
  int memory() { return sizeof(bool) * width * height; }
};
...
struct DistanceTransform
{
  ...
  int memory() { return width * height * sizeof(float); }
};
...
public:
  ...
  void saveTrace(std::string fn)
  {
    ...
    map.saveTrace(fn);
#else
    std::cout << "WARNING: trace map not generated, must compile with trace support enabled."
              << std::endl;
#endif
  }
  ...
  void numpy_calc_range_angles(
    float * ins, float * angles, float * outs, int num_particles, int num_angles)
  {
#if ROS_WORLD_TO_GRID_CONVERSION == 1
    // cache these constants on the stack for efficiency
    float inv_world_scale = 1.0 / map.world_scale;
  ...
  void eval_sensor_model(
    float * obs, float * ranges, double * outs, int rays_per_particle, int particles)
  {
    float inv_world_scale = 1.0 / map.world_scale;
  ...
  void calc_range_repeat_angles_eval_sensor_model(
    float * ins, float * angles, float * obs, double * weights, int num_particles, int num_angles)
  {
#if ROS_WORLD_TO_GRID_CONVERSION == 1
    // cache these constants on the stack for efficiency
    float inv_world_scale = 1.0 / map.world_scale;
  ...
  void calc_range_many_radial_optimized(
    float * ins, float * outs, int num_particles, int num_rays, float min_angle, float max_angle)
  {
#if ROS_WORLD_TO_GRID_CONVERSION == 1
    // cache these constants on the stack for efficiency
    float inv_world_scale = 1.0 / map.world_scale;
...
public:
...
#if ROS_WORLD_TO_GRID_CONVERSION == 1
    rmc->set_conversion_params(
      m.world_scale, m.world_angle, m.world_origin_x, m.world_origin_y, m.world_sin_angle,
      m.world_cos_angle);
#endif
  ...
  void numpy_calc_range(float * ins, float * outs, int num_casts)
  {
    ...
    std::cout << "Cannot use GPU numpy_calc_range without ROS_WORLD_TO_GRID_CONVERSION == 1"
              << std::endl;
    return;
#endif
    maybe_warn(num_casts);
    ...
    float * ins, float * angles, float * outs, int num_particles, int num_angles)
  {
    ...
    for (int i = 0; i < iters; ++i) {
      ...
      rmc->numpy_calc_range_angles(
        &ins[i * num_in_chunk * 3], angles, &outs[i * num_in_chunk * num_angles], num_in_chunk,
        num_angles);
    }
#else
    throw std::string("Must compile with -DWITH_CUDA=ON to use this class.");
    ...
    float * ins, float * angles, float * obs, double * weights, int num_particles, int num_angles)
  {
    ...
    for (int i = 0; i < iters; ++i) {
      int num_in_chunk = particles_per_iter;
      if (i == iters - 1) num_in_chunk = num_particles - i * particles_per_iter;
      rmc->calc_range_repeat_angles_eval_sensor_model(
        &ins[i * num_in_chunk * 3], angles, obs, &weights[i * num_in_chunk], num_in_chunk,
        num_angles);
    }
#else
    throw std::string("Must compile with -DWITH_CUDA=ON to use this class.");
      ...
      for (int y = 0; y < map.height; ++y) {
        ...
        if (edge_map.isOccupied(x, y)) {
          ...
          for (int a = 0; a < theta_discretization / 2.0; ++a) {
            ...
            // std::cout << "center: (" << pixel_center.first << ", " << pixel_center.second << ")"
            // << std::endl; std::cout << "new center: (" << lut_space_center_x << ", " <<
            // lut_space_center_y << ")" << std::endl; std::cout << "bins:" << upper_bin << "    "
            // << (int) lut_space_center_y << "   " << lower_bin << std::endl; std::cout << "width:"
            // << half_lut_space_width << std::endl; std::cout << "trans" << lut_translations[a] <<
            // std::endl; std::cout << upper_bin << "   " << lower_bin << "   " <<
            // lut_translations[a] << std::endl; std::cout << lut_space_center_x << "  " <<
            // lut_space_center_y << std::endl;
          }
        }
      }
    }
      ...
      for (int x = 0; x < map.grid.size(); ++x) {
        ...
        for (int y = 0; y < map.grid[0].size(); ++y) {
          ...
          if (high > _BINARY_SEARCH_THRESHOLD) {
            ...
            index =
              std::lower_bound(lut_bin->begin(), lut_bin->end(), lut_space_x) - lut_bin->begin();
...
// if (cache.exists(key)) { return cache.get(key); }
    ...
    if (is_flipped) {
      ...
      if (high > _BINARY_SEARCH_THRESHOLD) {
        int index =
          std::upper_bound(lut_bin->begin(), lut_bin->end(), lut_space_x) - lut_bin->begin();
  ...
  std::pair<float, float> calc_range_pair(float x, float y, float heading)
  {
    ...
    if (is_flipped) {
      ...
      if (high > _BINARY_SEARCH_THRESHOLD) {
        ...
        // float val = *std::lower_bound(lut_bin->begin(), lut_bin->end(), lut_space_x);
        index =
          std::upper_bound(lut_bin->begin(), lut_bin->end(), lut_space_x) - lut_bin->begin() - 1;
        val = (*lut_bin)[index];
      } else {  // do linear search if array is very small
        for (int i = high; i >= 0; --i) {
          float obstacle_x = (*lut_bin)[i];
  ...
  void serializeYaml(std::stringstream * ss)
  {
    ...
    (*ss) << T1 << "map: " << std::endl;
    (*ss) << T2
          << "# note: map data is width and then height (width is number of rows) transposed from "
             "expectation:"
          << std::endl;
...
public:
  ...
  int lut_size() { return map.width * map.height * theta_discretization * sizeof(lut_t); }
...
#else
    return max_range * giant_lut[(int)x][(int)y][discretize_theta(heading)] /
...
{
public:
  Benchmark(range_T rm) : range(rm) { map = range.getMap(); };
  ~Benchmark(){};
  ...
  static int num_grid_samples(
    int step_size, int num_rays, int samples, int map_width, int map_height)
  {
    int num_samples = 0;
    for (int i = 0; i < num_rays; ++i)
  ...
  static void get_grid_samples(
    float * queries, int step_size, int num_rays, int samples, int map_width, int map_height)
  {
    float coeff = (2.0 * M_PI) / num_rays;
    double t_accum = 0;
    int ind = 0;
  ...
  void random_sample(int num_samples)
  {
    ...
    std::uniform_real_distribution<float> randx =
      std::uniform_real_distribution<float>(1.0, map->width - 1.0);
    std::uniform_real_distribution<float> randy =
      std::uniform_real_distribution<float>(1.0, map->height - 1.0);
...
#endif  // F1TENTH__PARTICLE_FILTER__RANGE_LIBC__INCLUDES__RANGELIB_H_
