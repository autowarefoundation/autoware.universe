#ifndef F1TENTH__PARTICLE_FILTER__RANGE_LIBC__INCLUDES__RANGEUTILS_H_
#define F1TENTH__PARTICLE_FILTER__RANGE_LIBC__INCLUDES__RANGEUTILS_H_
...
public:
  ...
  KeyMaker(int width, int height, int theta_discretization)
  {
    ...
    if (bitness > std::log2(std::numeric_limits<key_T>::max())) {
      std::cerr
        << "Key bitness too large for integer packing scheme. Check your KeyMaker template type."
        << std::endl;
    }
  ...
  std::tuple<int, int, int> unpack_key(key_T k)
  {
    return std::make_tuple(
      (int)((k & x_mask) >> x_shift), (int)((k & y_mask) >> y_shift), k & t_mask);
  }
...
// Here's some code for sampling without replacement based on Algorithm 3.4.2S of Knuth's book
...
#endif  // F1TENTH__PARTICLE_FILTER__RANGE_LIBC__INCLUDES__RANGEUTILS_H_
