...
__global__ void cuda_ray_marching(
  float * ins, float * outs, float * distMap, int width, int height, float max_range, int num_casts)
{
...
__global__ void cuda_ray_marching_world_to_grid(
  float * ins, float * outs, float * distMap, int width, int height, float max_range, int num_casts,
...
__global__ void cuda_ray_marching_angles_world_to_grid(
  float * ins, float * outs, float * distMap, int width, int height, float max_range,
  int num_particles, int num_angles, float world_origin_x, float world_origin_y, float world_scale,
  float inv_world_scale, float world_sin_angle, float world_cos_angle, float rotation_const)
{
  int ind = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void cuda_eval_sensor_table(
  float * obs, float * ranges, double * outs, double * sensorTable, int rays_per_particle,
  int particles, float inv_world_scale, int max_range)
{
  int ind = blockIdx.x * blockDim.x + threadIdx.x;
...
// this should be optimized to use shared memory, otherwise the random read performance is not great
// __global__ void cuda_accumulate_weights(float * obs, float * ranges, double * outs, double *
// sensorTable, int rays_per_particle, int particles, float inv_world_scale, int max_range) { 	int
// ind = blockIdx.x*blockDim.x + threadIdx.x; 	if (ind >= particles) return;
...
void RayMarchingCUDA::calc_range_many(float * ins, float * outs, int num_casts)
{
  ...
  cuda_ray_marching<<<CHUNK_SIZE / NUM_THREADS, NUM_THREADS>>>(
    d_ins, d_outs, d_distMap, width, height, max_range, num_casts);
  err_check();
...
void RayMarchingCUDA::numpy_calc_range(float * ins, float * outs, int num_casts)
{
  ...
  cuda_ray_marching_world_to_grid<<<CHUNK_SIZE / NUM_THREADS, NUM_THREADS>>>(
    d_ins, d_outs, d_distMap, width, height, max_range, num_casts, world_origin_x, world_origin_y,
  ...
  cudaDeviceSynchronize();
#else
  std::cout << "GPU numpy_calc_range only works with ROS world to grid conversion enabled"
            << std::endl;
#endif
}

// num_casts must be less than or equal to chunk size
void RayMarchingCUDA::numpy_calc_range_angles(
  float * ins, float * angles, float * outs, int num_particles, int num_angles)
{
  ...
  // also copy angles to end of GPU buffer, this assumes there is enough space (which there should
  // be)
  cudaMemcpy(&d_ins[num_particles * 3], angles, sizeof(float) * num_angles, cudaMemcpyHostToDevice);
  // execute queries on the GPU, have to pass coordinate space conversion constants
  cuda_ray_marching_angles_world_to_grid<<<CHUNK_SIZE / NUM_THREADS, NUM_THREADS>>>(
    d_ins, d_outs, d_distMap, width, height, max_range, num_particles, num_angles, world_origin_x,
    world_origin_y, world_scale, inv_world_scale, world_sin_angle, world_cos_angle, rotation_const);
  err_check();
  // copy results back to CPU
  cudaMemcpy(outs, d_outs, sizeof(float) * num_particles * num_angles, cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
#else
  std::cout << "GPU numpy_calc_range_angles only works with ROS world to grid conversion enabled"
            << std::endl;
#endif
}

void RayMarchingCUDA::calc_range_repeat_angles_eval_sensor_model(
  float * ins, float * angles, float * obs, double * weights, int num_particles, int num_angles)
{
#if ROS_WORLD_TO_GRID_CONVERSION == 1
  std::cout << "Do not use calc_range_repeat_angles_eval_sensor_model for GPU, unimplemented"
            << std::endl;
  std::cout
    << "Instead use numpy_calc_range_angles followed by a standard sensor evaluation method."
    << std::endl;
...
// cudaMemcpy(d_ins, ins, sizeof(float) * num_particles * 3,cudaMemcpyHostToDevice);
// // also copy angles to end of GPU buffer, this assumes there is enough space (which there should
// be) cudaMemcpy(&d_ins[num_particles * 3], angles, sizeof(float) *
// num_angles,cudaMemcpyHostToDevice);

// // execute queries on the GPU, have to pass coordinate space conversion constants
// cuda_ray_marching_angles_world_to_grid<<< CHUNK_SIZE / NUM_THREADS, NUM_THREADS >>>(d_ins,d_outs,
// d_distMap, 	width, height, max_range, num_particles, num_angles, world_origin_x, world_origin_y,
// 	world_scale, inv_world_scale, world_sin_angle, world_cos_angle, rotation_const);

// cudaMemcpy(d_ins, obs, sizeof(float) * num_angles,cudaMemcpyHostToDevice);

// // read from sensor table
// cuda_eval_sensor_table<<< CHUNK_SIZE / NUM_THREADS, NUM_THREADS >>>(d_ins, d_outs, d_weights,
// d_sensorTable, num_angles, num_particles, inv_world_scale, table_width);
