...
: Node("motion_velocity_smoother", node_options)
{
  ...
  initCommonParam();
  over_stop_velocity_warn_thr_ = declare_parameter("over_stop_velocity_warn_thr").get<double>();
  ...
  // debug
  publish_debug_trajs_ = declare_parameter("publish_debug_trajs").get<bool>();
...
{
  auto & p = node_param_;
  p.max_velocity = declare_parameter("max_velocity").get<double>();  // 72.0 kmph
  p.margin_to_insert_external_velocity_limit =
    declare_parameter("margin_to_insert_external_velocity_limit").get<double>();
  p.replan_vel_deviation = declare_parameter("replan_vel_deviation").get<double>();
  p.engage_velocity = declare_parameter("engage_velocity").get<double>();
  p.engage_acceleration = declare_parameter("engage_acceleration").get<double>();
  p.engage_exit_ratio = declare_parameter("engage_exit_ratio").get<double>();
  p.engage_exit_ratio = std::min(std::max(p.engage_exit_ratio, 0.0), 1.0);
  p.stopping_velocity = declare_parameter("stopping_velocity").get<double>();
  p.stopping_distance = declare_parameter("stopping_distance").get<double>();
  p.extract_ahead_dist = declare_parameter("extract_ahead_dist").get<double>();
  p.extract_behind_dist = declare_parameter("extract_behind_dist").get<double>();
  p.stop_dist_to_prohibit_engage = declare_parameter("stop_dist_to_prohibit_engage").get<double>();
  ...
  p.post_resample_param.max_trajectory_length =
    declare_parameter("post_max_trajectory_length").get<double>();
  p.post_resample_param.min_trajectory_length =
    declare_parameter("post_min_trajectory_length").get<double>();
  p.post_resample_param.resample_time = declare_parameter("post_resample_time").get<double>();
  p.post_resample_param.dense_resample_dt = declare_parameter("post_dense_resample_dt").get<double>();
  p.post_resample_param.dense_min_interval_distance =
    declare_parameter("post_dense_min_interval_distance").get<double>();
  p.post_resample_param.sparse_resample_dt = declare_parameter("post_sparse_resample_dt").get<double>();
  p.post_resample_param.sparse_min_interval_distance =
    declare_parameter("post_sparse_min_interval_distance").get<double>();
  p.algorithm_type = getAlgorithmType(declare_parameter("algorithm_type").get<std::string>());
}
