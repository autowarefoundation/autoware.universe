...
: SteeNode("motion_velocity_smoother", node_options)
{
  ...
  switch (node_param_.algorithm_type) {
    case AlgorithmType::JERK_FILTERED: {
      ...
      pub_forward_filtered_trajectory_ =
        create_stee_publisher<Trajectory>("~/debug/forward_filtered_trajectory", 1);
      pub_backward_filtered_trajectory_ =
        create_stee_publisher<Trajectory>("~/debug/backward_filtered_trajectory", 1);
      pub_merged_filtered_trajectory_ =
        create_stee_publisher<Trajectory>("~/debug/merged_filtered_trajectory", 1);
  ...
  pub_trajectory_ = create_stee_publisher<Trajectory>("~/output/trajectory", 1);
  ...
  pub_trajectory_raw_ = create_stee_publisher<Trajectory>("~/debug/trajectory_raw", 1);
  pub_trajectory_vel_lim_ =
    create_stee_publisher<Trajectory>("~/debug/trajectory_external_velocity_limited", 1);
  pub_trajectory_latacc_filtered_ =
    create_stee_publisher<Trajectory>("~/debug/trajectory_lateral_acc_filtered", 1);
  pub_trajectory_resampled_ = create_stee_publisher<Trajectory>("~/debug/trajectory_time_resampled", 1);
