...
#include "adaptive_cruise_planner/optimization_based_planner/optimization_based_planner.hpp"

#include "adaptive_cruise_planner/optimization_based_planner/resample.hpp"
  ...
  [[maybe_unused]] DebugData & debug_data)
{
  ...
  data.v0 = v0;
  RCLCPP_DEBUG(rclcpp::get_logger("AdaptiveCruisePlanner::OptimizationBasedPlanner"), "v0 %f", v0);
...
double OptimizationBasedPlanner::getClosestStopDistance(
  const AdaptiveCruisePlannerData & planner_data, const TrajectoryData & ego_traj_data,
  const std::vector<double> & resolutions)
{
  ...
  for (const auto & obj : planner_data.target_obstacles) {
    ...
    const auto current_object_pose = adaptive_cruise_utils::getCurrentObjectPoseFromPredictedPath(
  ...
  if (vehicle_speed < engage_vel_thr) {
    if (target_vel >= engage_velocity_) {
      ...
      RCLCPP_WARN_THROTTLE(
        rclcpp::get_logger("AdaptiveCruisePlanner::OptimizationBasedPlanner"), clock, 3000,
  ...
  const std::vector<double> & time_vec)
{
  ...
  if (obj_vel > object_low_velocity_threshold_) {
    // RCLCPP_DEBUG(rclcpp::get_logger("AdaptiveCruisePlanner::OptimizationBasedPlanner"), "Off
    // Trajectory Object");
    return getSBoundaries(
