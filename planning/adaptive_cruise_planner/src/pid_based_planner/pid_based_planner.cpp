...
#include "adaptive_cruise_planner/pid_based_planner/pid_based_planner.hpp"
...
Trajectory PIDBasedPlanner::generateTrajectory(
  const AdaptiveCruisePlannerData & planner_data, boost::optional<VelocityLimit> & vel_limit,
...
void PIDBasedPlanner::calcObstaclesToCruiseAndStop(
  const AdaptiveCruisePlannerData & planner_data,
  boost::optional<StopObstacleInfo> & stop_obstacle_info,
  boost::optional<CruiseObstacleInfo> & cruise_obstacle_info)
{
...
double PIDBasedPlanner::calcDistanceToObstacle(
  const AdaptiveCruisePlannerData & planner_data, const TargetObstacle & obstacle)
{
  ...
  //     adaptive_cruise_utils::getCurrentObjectPoseFromPredictedPath(
  ...
  //
  //   RCLCPP_INFO_EXPRESSION(
  //     rclcpp::get_logger("AdaptiveCruisePlanner::PIDBasedPlanner"), true,
...
boost::optional<size_t> PIDBasedPlanner::doStop(
  const AdaptiveCruisePlannerData & planner_data, const StopObstacleInfo & stop_obstacle_info,
  ...
  visualization_msgs::msg::MarkerArray & debug_wall_marker) const
{
  ...
  const auto marker_pose = adaptive_cruise_utils::calcForwardPose(
...
VelocityLimit PIDBasedPlanner::doCruise(
  const AdaptiveCruisePlannerData & planner_data, const CruiseObstacleInfo & cruise_obstacle_info,
...
void PIDBasedPlanner::publishDebugValues(const AdaptiveCruisePlannerData & planner_data) const
{
