...
#include "adaptive_cruise_planner/node.hpp"

#include "adaptive_cruise_planner/polygon_utils.hpp"
#include "adaptive_cruise_planner/utils.hpp"
...
VelocityLimitClearCommand createVelocityLimitClearCommandMsg(const rclcpp::Time & current_time)
{
  ...
  msg.sender = "adaptive_cruise_planner";
  msg.command = true;
  return msg;
}
...
{
AdaptiveCruisePlannerNode::AdaptiveCruisePlannerNode(const rclcpp::NodeOptions & node_options)
: Node("adaptive_cruise_planner", node_options),
  ...
  vehicle_info_(vehicle_info_util::VehicleInfoUtil(*this).getVehicleInfo())
{
  ...
  trajectory_sub_ = create_subscription<Trajectory>(
    ...
    std::bind(&AdaptiveCruisePlannerNode::onTrajectory, this, _1));
  smoothed_trajectory_sub_ = create_subscription<Trajectory>(
    "/planning/scenario_planning/trajectory", rclcpp::QoS{1},
    std::bind(&AdaptiveCruisePlannerNode::onSmoothedTrajectory, this, _1));
  objects_sub_ = create_subscription<PredictedObjects>(
    "~/input/objects", rclcpp::QoS{1}, std::bind(&AdaptiveCruisePlannerNode::onObjects, this, _1));
  odom_sub_ = create_subscription<Odometry>(
    "~/input/odometry", rclcpp::QoS{1},
    std::bind(&AdaptiveCruisePlannerNode::onOdometry, this, std::placeholders::_1));
  ...
  set_param_res_ = this->add_on_set_parameters_callback(
    std::bind(&AdaptiveCruisePlannerNode::onParam, this, std::placeholders::_1));
}

AdaptiveCruisePlannerNode::PlanningAlgorithm AdaptiveCruisePlannerNode::getPlanningAlgorithmType(
...
rcl_interfaces::msg::SetParametersResult AdaptiveCruisePlannerNode::onParam(
...
void AdaptiveCruisePlannerNode::onTrajectory(const Trajectory::ConstSharedPtr msg)
{
  ...
  RCLCPP_INFO_EXPRESSION(
    rclcpp::get_logger("AdaptiveCruisePlanner"), is_showing_debug_info_, "%s := %f [ms]", __func__,
    calculation_time);
}

AdaptiveCruisePlannerData AdaptiveCruisePlannerNode::createPlannerData(
  ...
  DebugData & debug_data)
{
  ...
  // create planner_data
  AdaptiveCruisePlannerData planner_data;
  ...
  const double calculation_time = stop_watch_.toc(__func__);
  RCLCPP_INFO_EXPRESSION(
    rclcpp::get_logger("AdaptiveCruisePlanner"), is_showing_debug_info_, "  %s := %f [ms]",
    __func__, calculation_time);

  return planner_data;
}

double AdaptiveCruisePlannerNode::calcCurrentAccel() const
{
...
std::vector<TargetObstacle> AdaptiveCruisePlannerNode::filterObstacles(
...
geometry_msgs::msg::Point AdaptiveCruisePlannerNode::calcNearestCollisionPoint(
...
double AdaptiveCruisePlannerNode::calcCollisionTimeMargin(
...
void AdaptiveCruisePlannerNode::publishVelocityLimit(
...
void AdaptiveCruisePlannerNode::publishDebugData(const DebugData & debug_data) const
{
  stop_watch_.tic(__func__);
  ...
  for (size_t i = 0; i < debug_data.obstacles_to_cruise.size(); ++i) {
    const auto marker = adaptive_cruise_utils::getObjectMarker(
...
void AdaptiveCruisePlannerNode::publishCalculationTime(const double calculation_time) const
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(motion_planning::AdaptiveCruisePlannerNode)
