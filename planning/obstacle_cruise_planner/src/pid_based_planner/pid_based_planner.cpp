  ...
  const double a0, const double target_acc, const double target_jerk_ratio) const
{
  ...
  const auto vt = [&](
                    ...
                    const double target_acc, const double target_jerk_ratio) {
  ...
  for (double tmp_idx = 0.0; tmp_idx < max_sampling_num; ++tmp_idx) {
    if (v_vec.back() <= 0.0) {
      ...
      const double v_current = vt(
        v1, a1, jerk, t_current + t_delta_min, target_acc,
        target_jerk_ratio);  // TODO(murooka) + t_delta_min is not required
  ...
  std::cerr << "v_vec.back() " << v_vec.back() << std::endl;
  ...
  std::cerr << "unique_v_vec.back() " << unique_v_vec.back() << std::endl;

  for (size_t i = 0; i < unique_s_vec.size(); ++i) {
    std::cerr << i << " " << unique_s_vec.at(i) << " " << unique_v_vec.at(i) << std::endl;
  }

  /*
  bool positive_flag = false;
  constexpr double positive_epsilon = 1e3;
  constexpr double initial_min_vel = 1.0;
  for (int i = unique_v_vec.size() - 1; 0 <= i; --i) {
    if (1e3 < unique_v_vec.at(i)) {
      positive_flag = true;
    }

    if (positive_flag && unique_v_vec.at(i) < 1e3) {
      unique_v_vec.at(i) = initial_min_vel;
    }
  }
  */
