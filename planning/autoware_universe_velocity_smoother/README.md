...
`autoware_universe_velocity_smoother` outputs a desired velocity profile on a reference trajectory.
...
We call this module `autoware_universe_velocity_smoother` because the limitations of the acceleration and the jerk means the smoothness of the velocity profile.
...
It applies the velocity limit input from the external of `autoware_universe_velocity_smoother`.
