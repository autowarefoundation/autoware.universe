...
TEST(Trajectory, resample)
{
  ...
  ASSERT_EQ(t2.longitudinal_accelerations.size(), 3);
  ...
  ASSERT_EQ(t2.lateral_velocities.size(), 3);
  ...
  ASSERT_EQ(t2.lateral_accelerations.size(), 3);
...
TEST(Trajectory, resampleTime)
{
  ...
  Trajectory t2 = t.resampleTime(0.5);
  ASSERT_EQ(t2.points.size(), 3);
  EXPECT_NEAR(t2.points[0].x(), 0.0, eps);
  EXPECT_NEAR(t2.points[1].x(), 0.5, eps);
  EXPECT_NEAR(t2.points[2].x(), 1.0, eps);
