...
std::vector<Polygon2d> PredictedObjectsToPolygons(const PredictedObjects & objects)
{
  (void)objects;
  std::printf("WARNING : PredictedObjectsToPolygons is not implemented\n");
  std::vector<Polygon2d> polygons;
  // TODO(Maxime): improve
  /*
  constexpr auto max_t = 3.0;  // [s] TODO(Maxime): param
  for(const auto & object : objects.objects) {
    for(const auto & path : object.kinematics.predicted_paths) {
      LineString2d ls;
      auto t = 0.0;
      const auto dt = rclcpp::Duration(path.time_step).seconds();
      for(const auto & p : path.path) {
        t += dt;
        if(t > max_t) break;
        ls.emplace_back(p.position.x, p.position.y);
      }
      std::vector<Polygon2d> path_polys;
      // boost::geometry::buffer(ls, path_polys, object.shape.dimensions.y);
      if(!path_polys.empty()) polygons.push_back(path_polys.front());
    }
  }
  */
  return polygons;
