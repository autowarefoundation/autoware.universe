  ...
  const sampler_common::transform::Spline2D & path_spline, const Parameters & params)
{
  ...
  const auto path_curv_deriv = (path_spline.curvature(s + delta_s) - path_curvature) / delta_s;
  ...
  if (cos_yaw == 0.0) {
    ...
    initial_frenet_state.lateral_acceleration =
      -(path_curv_deriv * d + path_curvature * initial_frenet_state.lateral_velocity) *
        std::tan(frenet_yaw) +
