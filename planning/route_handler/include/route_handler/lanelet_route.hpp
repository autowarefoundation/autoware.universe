...
// TODO(vrichard) eventually, we should be able to replace every lanelet::ConstLanelet by LaneletSection.
// Besides the LaneletRoute building functions, LaneletRoute does not rely much on lanelet lib algoritms.
// In the long term, it should be possible to create generic routes from non lanelet vector maps.
...
public:
  ...
  //! Max search distance can be set to fasten search. It is typically used if we just need to know if there is "enough" distance left to the end of the route boundaries.
  ...
  //! If within_route is true, the path is also restricted to the route boundaries (from start to goal, within the route lanelets).
  //! If within_route is false, eventual overlapping path sections will be removed according to chosen strategy
  ...
  // TODO(vrichard) temporarily glue code because lanelets are used as path everywhere in the path planning modules
  ...
  //! If within_route is true, extended path will be restricted to the route boundaries (from start to goal, within route lanelets)
  //! If within_route is false, eventual overlapping path sections will be removed according to chosen strategy
  //! @note This function is similar to getStraightPath(), except it takes a path instead of a point as input
  ...
  [[nodiscard]] lanelet::ConstLanelets getNeighborsWithinRoute(const lanelet::ConstLanelet & lanelet) const;
  ...
  //! @note Also consider points before/after start/goal but on the route lanelets
  //! @return signed number of lane changes (left > 0, right < 0), 0 if the point is on a preferred lane, nothing if the point is not on the route lanelets.
  [[nodiscard]] std::optional<int> getNumLaneChangeToPreferredLane(const LaneletPoint & lanelet_point) const;

  // lanelet query

  // TODO(vrichard) makes this private when outside code has been fixed (planning functions should not manipulate lanelets directly)
  ...
  [[nodiscard]] lanelet::ConstLanelets getFollowingLanelets(
    const LaneletPoint & lanelet_point,
    const bool within_route = true) const;
  ...
  //! But if the goal pose is behind the start pose on the same lane, the full lane is "split" in 2, and
  //! the first segment corresponds to the lane section on the start pose side.
  ...
  //! But if the goal pose is behind the start pose on the same lane, the full lane is "split" in 2, and
  //! the last segment corresponds to the lane section on the goal pose side.
...
private:
  ...
  // TODO(vrichard) eventually, we should get rid of lanelet::ConstLanelets and use LaneletSections instead
  ...
  std::vector<LaneletPoint> start_line_; //!< start point projected on each of the starting segments
