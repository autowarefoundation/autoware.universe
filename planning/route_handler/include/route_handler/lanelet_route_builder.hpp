...
public:
  ...
  [[nodiscard]] LaneletRoutePtr buildFromLaneletRouteMsg(const LaneletRouteMsg & route_msg) const;
...
private:
  ...
  //! In most cases, start and goal segments contains all the lanelets around the start and goal
  //! point But if the goal is behind the start point on the same or a neighbor lanelet, the
  //! sections are shrinked so that they don't overlap.
  ...
  [[nodiscard]] lanelet::ConstLanelets getRouteLanelets(
    const lanelet::ConstLanelets & path_lanelets) const;

  //! @brief Get main path from start to goal.
  //! @note By construction, the main path starts at the beginning of the first preferred lanelet,
  //! and goes to the goal point on the goal lanelet. The first lanelet of the main path may be
