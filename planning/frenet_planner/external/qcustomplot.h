...
enum AntialiasedElement {
  ...
  ,
  aeScatters = 0x0080  ///< <tt>0x0080</tt> Scatter symbols of plottables (excluding scatter symbols
                       ///< of type ssPixmap)
  ,
...
extern const QMetaObject
  staticMetaObject;  // in moc-run we create a static meta object for QCP "fake" object. This line
                     // is the link to it via QCP::staticMetaObject in normal operation as namespace
...
public:
  ...
  */
  enum PainterMode {
    ...
    ,
    pmNonCosmetic = 0x04  ///< <tt>0x04</tt> Turns pen widths 0 to 1, i.e. disables cosmetic pens.
                          ///< (A cosmetic pen is always drawn with width 1 pixel in the vector
...
public:
  explicit QCPPaintBufferGlFbo(
    const QSize & size, double devicePixelRatio, QWeakPointer<QOpenGLContext> glContext,
...
public:
  QCPLayerable(
    QCustomPlot * plot, QString targetLayer = QString(), QCPLayerable * parentLayerable = nullptr);
  ...
  virtual double selectTest(
    const QPointF & pos, bool onlySelectable, QVariant * details = nullptr) const;
...
protected:
  ...
  QVector<int> getSectionSizes(
    QVector<int> maxSizes, QVector<int> minSizes, QVector<double> stretchFactors,
    int totalSize) const;
...
public:
  ...
  */
  enum FillOrder {
    foRowsFirst  ///< Rows are filled first, and a new element is wrapped to the next column if the
                 ///< row count would exceed \ref setWrap.
    ,
    foColumnsFirst  ///< Columns are filled first, and a new element is wrapped to the next row if
                    ///< the column count would exceed \ref setWrap.
  };
  Q_ENUMS(FillOrder)
...
protected:
  ...
  // introduced virtual methods:
  virtual void drawLabelMaybeCached(
    QCPPainter * painter, const QFont & font, const QColor & color, const QPointF & pos,
  ...
  QByteArray cacheKey(
    const QString & text, const QColor & color, double rotation, AnchorSide side) const;
  AnchorSide skewedAnchorSide(
    const QPointF & tickPos, double sideExpandHorz, double sideExpandVert) const;
...
public:
  ...
  */
  ...
  virtual void generate(
    const QCPRange & range, const QLocale & locale, QChar formatChar, int precision,
...
public:
  ...
  */
  enum LabelSide {
    lsInside  ///< Tick labels will be displayed inside the axis rect and clipped to the inner axis
              ///< rect
    ,
...
protected:
  ...
  // events:
  virtual void selectEvent(
    QMouseEvent * event, bool additive, const QVariant & details,
...
protected:
  ...
  virtual void placeTickLabel(
    QCPPainter * painter, double position, int distanceToAxis, const QString & text,
...
public:
  ...
  */
  enum ScatterShape {
    ...
    ,
    ssDisc  ///< \enumimage{ssDisc.png} a circle which is filled with the pen's color (not the brush
            ///< as with ssCircle)
    ,
  ...
  QCPScatterStyle(
    const QPainterPath & customPath, const QPen & pen, const QBrush & brush = Qt::NoBrush,
    double size = 6);
...
public:
  ...
  QCPRange valueRange(
    bool & foundRange, QCP::SignDomain signDomain = QCP::sdBoth,
...
void QCPDataContainer<DataType>::add(const QCPDataContainer<DataType> & data)
{
  ...
  } else  // don't need to prepend, so append and merge if necessary
  {
    ...
    if (
      oldSize > 0 &&
      !qcpLessThanSortKey<DataType>(
        *(constEnd() - n - 1), *(constEnd() - n)))  // if appended range keys aren't all greater
...
void QCPDataContainer<DataType>::remove(double sortKey)
{
  ...
  if (it != end() && it->sortKey() == sortKey) {
    if (it == begin())
      ++mPreallocSize;  // don't actually delete, just add it to the preallocated block (if it gets
                        // too large, squeeze will take care of it)
    else
      mData.erase(it);
  }
  if (mAutoSqueeze) performAutoSqueeze();
...
QCPRange QCPDataContainer<DataType>::keyRange(bool & foundRange, QCP::SignDomain signDomain)
{
  ...
  if (signDomain == QCP::sdBoth)  // range may be anywhere
  {
    if (DataType::sortKeyIsMainKey())  // if DataType is sorted by main key (e.g. QCPGraph, but not
                                       // QCPCurve), use faster algorithm by finding just first and
  ...
  bool & foundRange, QCP::SignDomain signDomain, const QCPRange & inKeyRange)
{
  ...
  if (signDomain == QCP::sdBoth)  // range may be anywhere
  {
    for (QCPDataContainer<DataType>::const_iterator it = itBegin; it != itEnd; ++it) {
      if (
        restrictKeyRange && (it->mainKey() < inKeyRange.lower || it->mainKey() > inKeyRange.upper))
        continue;
  ...
  } else if (signDomain == QCP::sdNegative)  // range may only be in the negative sign domain
  {
    for (QCPDataContainer<DataType>::const_iterator it = itBegin; it != itEnd; ++it) {
      ...
      current = it->valueRange();
      if (
        (current.lower < range.lower || !haveLower) && current.lower < 0 &&
        !qIsNaN(current.lower)) {
        range.lower = current.lower;
        haveLower = true;
      }
      if (
        (current.upper > range.upper || !haveUpper) && current.upper < 0 &&
...
void QCPDataContainer<DataType>::performAutoSqueeze()
{
  ...
  if (totalAlloc > 650000)  // if allocation is larger, shrink earlier with respect to total used
                            // size
  {
...
public:
  QCPItemAnchor(
    QCustomPlot * parentPlot, QCPAbstractItem * parentItem, const QString & name,
    int anchorId = -1);
  ...
  */
  enum PositionType {
    ptAbsolute  ///< Static positioning in pixels, starting from the top left corner of the
                ///< viewport/widget.
    ,
    ptViewportRatio  ///< Static positioning given by a fraction of the viewport size. For example,
                     ///< if you call setCoords(0, 0), the position will be at the top left corner
                     ///< of the viewport/widget. setCoords(1, 1) will be at the bottom right
                     ///< corner, setCoords(0.5, 0) will be horizontally centered and vertically at
    ...
    ptAxisRectRatio  ///< Static positioning given by a fraction of the axis rect size (see \ref
                     ...
                     ///< be at the top left corner of the axis rect. setCoords(1, 1) will be at the
...
public:
  ...
  */
  enum RefreshPriority {
    ...
    ,
    rpRefreshHint  ///< Whether to use immediate or queued refresh depends on whether the plotting
                   ///< hint \ref QCP::phImmediateRefresh is set, see \ref setPlottingHints.
    ,
  ...
  PlottableType * plottableAt(
    const QPointF & pos, bool onlySelectable = false, int * dataIndex = nullptr) const;
  ...
  bool addLayer(
    const QString & name, QCPLayer * otherLayer = nullptr, LayerInsertMode insertMode = limAbove);
  ...
  bool savePdf(
    const QString & fileName, int width = 0, int height = 0,
    QCP::ExportPen exportPen = QCP::epAllowCosmetic, const QString & pdfCreator = QString(),
    const QString & pdfTitle = QString());
  bool savePng(
    const QString & fileName, int width = 0, int height = 0, double scale = 1.0, int quality = -1,
    int resolution = 96, QCP::ResolutionUnit resolutionUnit = QCP::ruDotsPerInch);
  bool saveJpg(
    const QString & fileName, int width = 0, int height = 0, double scale = 1.0, int quality = -1,
    int resolution = 96, QCP::ResolutionUnit resolutionUnit = QCP::ruDotsPerInch);
  bool saveBmp(
    const QString & fileName, int width = 0, int height = 0, double scale = 1.0,
    int resolution = 96, QCP::ResolutionUnit resolutionUnit = QCP::ruDotsPerInch);
  bool saveRastered(
    const QString & fileName, int width, int height, double scale, const char * format,
...
protected:
  ...
  QCPLayerable * layerableAt(
    const QPointF & pos, bool onlySelectable, QVariant * selectionDetails = nullptr) const;
...
*/
template <class PlottableType>
PlottableType * QCustomPlot::plottableAt(
  const QPointF & pos, bool onlySelectable, int * dataIndex) const
{
  ...
  foreach (QCPAbstractPlottable * plottable, mPlottables) {
    ...
    if (
      ...
      (onlySelectable &&
       !currentPlottable
          ->selectable()))  // we could have also passed onlySelectable to the selectTest function,
                            // but checking here is faster, because we have access to
...
ItemType * QCustomPlot::itemAt(const QPointF & pos, bool onlySelectable) const
{
  ...
  foreach (QCPAbstractItem * item, mItems) {
    ...
    if (
      ...
      continue;
    if (
      !currentItem->clipToAxisRect() ||
      currentItem->clipRect().contains(pos.toPoint()))  // only consider clicks inside axis cliprect
                                                        // of the item if actually clipped to it
    {
...
: public QCPAbstractPlottable,
  public QCPPlottableInterface1D  // no QCP_LIB_DECL, template class ends up in header (cpp included
                                  // below)
{
  ...
  const QRectF & rect, bool onlySelectable) const
{
  ...
  if (DataType::sortKeyIsMainKey())  // we can assume that data is sorted by main key, so can reduce
                                     // the searched key interval:
  {
  ...
  for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it) {
    if (currentSegmentBegin == -1) {
      if (valueRange.contains(it->mainValue()) && keyRange.contains(it->mainKey()))  // start
                                                                                     // segment
        currentSegmentBegin = int(it - mDataContainer->constBegin());
    } else if (
      !valueRange.contains(it->mainValue()) || !keyRange.contains(it->mainKey()))  // segment
                                                                                   // just
                                                                                   // ended
    {
      result.addDataRange(
  ...
  const QPointF & pos, bool onlySelectable, QVariant * details) const
{
  ...
  for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it) {
    ...
    if (keyRange.contains(mainKey) && valueRange.contains(mainValue))  // make sure data point is
                                                                       // inside visible range, for
                                                                       // speedup in cases where
                                                                       // sort key isn't main key
                                                                       // and we iterate over all
                                                                       // points
    {
  ...
  QCPPainter * painter, const QVector<QPointF> & lineData) const
{
  ...
  if (
    mParentPlot->plottingHints().testFlag(QCP::phFastPolylines) &&
    ...
    while (i < lineDataSize) {
      if (!qIsNaN(lineData.at(i).y()) && !qIsNaN(lineData.at(i).x()))  // NaNs create a gap in the
                                                                       // line
      {
        if (!lastIsNan)
...
public:
  ...
  */
  enum GradientPreset {
    ...
    ,
    gpPolar  ///< Colors suitable to emphasize polarity around the center, with blue for negative,
             ///< black in the middle and red for positive values
    ,
    ...
    ,
    gpHues  ///< Full hue cycle, with highest and lowest color red (suitable for periodic data, such
            ///< as angles and phases, see \ref setPeriodic)
  };
  ...
  // non-property methods:
  void colorize(
    const double * data, const QCPRange & range, QRgb * scanLine, int n, int dataIndexFactor = 1,
    bool logarithmic = false);
  void colorize(
    const double * data, const unsigned char * alpha, const QCPRange & range, QRgb * scanLine,
...
public:
  ...
  */
  enum BracketStyle {
    ...
    ,
    bsUserStyle  ///< Start custom bracket styles at this index when subclassing and reimplementing
                 ///< \ref drawBracket.
  };
...
protected:
  ...
  virtual void getOptimizedLineData(
    QVector<QCPGraphData> * lineData, const QCPGraphDataContainer::const_iterator & begin,
    const QCPGraphDataContainer::const_iterator & end) const;
  virtual void getOptimizedScatterData(
    QVector<QCPGraphData> * scatterData, QCPGraphDataContainer::const_iterator begin,
  ...
  const QPolygonF getChannelFillPolygon(
    const QVector<QPointF> * thisData, QCPDataRange thisSegment, const QVector<QPointF> * otherData,
...
public:
  ...
  */
  ...
  void setData(
    const QVector<double> & t, const QVector<double> & keys, const QVector<double> & values,
...
protected:
  ...
  QPointF getOptimizedPoint(
    int otherRegion, double otherKey, double otherValue, double key, double value, double keyMin,
    double valueMax, double keyMax, double valueMin) const;
  QVector<QPointF> getOptimizedCornerPoints(
    int prevRegion, int currentRegion, double prevKey, double prevValue, double key, double value,
    double keyMin, double valueMax, double keyMax, double valueMin) const;
  bool mayTraverse(int prevRegion, int currentRegion) const;
  bool getTraverse(
    double prevKey, double prevValue, double key, double value, double keyMin, double valueMax,
    double keyMax, double valueMin, QPointF & crossA, QPointF & crossB) const;
  void getTraverseCornerPoints(
    int prevRegion, int currentRegion, double keyMin, double valueMax, double keyMax,
...
public:
  ...
  }  // note that bar base value isn't held in each QCPBarsData and thus can't/shouldn't be returned
     // here
...
public:
  ...
  QCPStatisticalBoxData(
    double key, double minimum, double lowerQuartile, double median, double upperQuartile,
...
public:
  ...
  void setData(
    const QVector<double> & keys, const QVector<double> & minimum,
...
public:
  ...
  */
  ...
  void setData(
    const QVector<double> & keys, const QVector<double> & open, const QVector<double> & high,
  ...
  static QCPFinancialDataContainer timeSeriesToOhlc(
    const QVector<double> & time, const QVector<double> & value, double timeBinSize,
...
public:
  /*!
    The different visual appearances a tracer item can have. Some styles size may be controlled with
    \ref setSize.
...
protected:
  ...
  // non-virtual methods:
  void drawRadialGrid(
    QCPPainter * painter, const QPointF & center, const QVector<double> & coords, const QPen & pen,
    const QPen & zeroPen = Qt::NoPen);
  void drawAngularGrid(
    QCPPainter * painter, const QPointF & center, double radius,
