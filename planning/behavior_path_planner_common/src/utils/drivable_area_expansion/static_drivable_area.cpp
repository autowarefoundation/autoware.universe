  ...
  PathWithLaneId & path, const std::vector<DrivableLanes> & lanes)
{
  ...
  const auto is_point_in_drivable_lanes = [&has_same_id_lane, &has_same_id_lanes, &original_points](
                                            const auto & lanes, size_t i) {
  ...
  for (size_t i = 0; i < original_points.size(); ++i) {
    const bool first_path_point_in_drivable_lane_found = std::any_of(
      ...
      [&is_point_in_drivable_lanes, i](const auto & lanes) {
        return is_point_in_drivable_lanes(lanes, i);
      });
