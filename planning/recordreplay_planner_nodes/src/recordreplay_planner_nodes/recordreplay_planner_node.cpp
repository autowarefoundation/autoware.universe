...
void RecordReplayPlannerNode::on_ego(const State::SharedPtr & msg)
{
  ...
  try {
    tf_map2odom =
      tf_buffer_->lookupTransform(m_recording_frame, msg->header.frame_id, tf2::TimePointZero);
  } catch (...) {
  ...
  tf2::doTransform(*msg.state.pose, msg_world.state.pose, tf_map2odom) msg_world.header.frame_id =
    m_recording_frame;
