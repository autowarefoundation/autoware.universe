...
Trajectory CollisionFreePathPlanner::generateTrajectory(const PlannerData & planner_data)
{
  ...
  const auto is_driving_forward = driving_direction_checker_.isDrivingForward(p.path.points);
  if (!is_driving_forward) {
    ...
    return points_utils::createTrajectory(p.path.header, traj_points);
  }
  ...
  return points_utils::createTrajectory(p.path.header, post_processed_traj_points);
}
  ...
  const PlannerData & planner_data)
{
  ...
  // check if replan (= optimizatoin) is required
  ...
  const CVMaps & cv_maps)
{
  ...
  for (auto & traj_point : mpt_traj_points) {
    ...
    const bool is_outside = cv_drivable_area_utils::isOutsideDrivableAreaFromRectangleFootprint(
      traj_point, road_clearance_map, map_info, vehicle_info_);

    if (is_outside) {
      // NOTE: No need to assign zero velocity to following points after being outside the drivable
      // area
      //       since this trajecotry is only optimized one (= short one) and alignVelocity function
      //       will assign zero velocity to the points.
      traj_point.longitudinal_velocity_mps = 0.0;
      debug_data_.stop_pose_by_drivable_area = traj_point.pose;
      break;
    }
  }
  ...
  const PlannerData & planner_data, const std::vector<TrajectoryPoint> & traj_points)
{
  ...
  if (enable_debug_marker_pub_) {
    ...
    const auto & debug_marker =
      getDebugVisualizationMarker(debug_data_, traj_points, vehicle_info_, false);
    ...
    // debug visualization wall marker
    stop_watch_.tic("getDebugVisualizationWallMarker");
    const auto & debug_wall_marker = getDebugVisualizationWallMarker(debug_data_, vehicle_info_);
...
void CollisionFreePathPlanner::publishDebugDataInMain(const Path & path) const
{
  ...
  {  // publish trajectories
    const auto debug_extended_fixed_traj =
      points_utils::createTrajectory(path.header, debug_data_.extended_fixed_traj);
    debug_extended_fixed_traj_pub_->publish(debug_extended_fixed_traj);

    const auto debug_extended_non_fixed_traj =
      points_utils::createTrajectory(path.header, debug_data_.extended_non_fixed_traj);
  ...
  {  // publish clearance map
    ...
    if (enable_area_with_objects_pub_) {  // false by default
      debug_area_with_objects_pub_->publish(
        getDebugCostmap(debug_data_.area_with_objects_map, path.drivable_area));
    }
    if (enable_object_clearance_map_pub_) {  // false by default
      debug_object_clearance_map_pub_->publish(
        getDebugCostmap(debug_data_.only_object_clearance_map, path.drivable_area));
    }
    if (enable_clearance_map_pub_) {  // true by default
      debug_clearance_map_pub_->publish(
        getDebugCostmap(debug_data_.clearance_map, path.drivable_area));
    }
