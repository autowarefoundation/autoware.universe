...
public:
  ...
  DemoWindow()
  {
    ...
    connect(
      s0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) { updatePlot(); });
    connect(
      v0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) { updatePlot(); });
    connect(
      a0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) { updatePlot(); });
    connect(
      sT, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) { updatePlot(); });
    connect(
      vT, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) { updatePlot(); });
    connect(
      aT, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) { updatePlot(); });
    connect(duration, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) {
      updatePlot();
    });
    connect(acc_check, QOverload<int>::of(&QCheckBox::stateChanged), [&](int) { updatePlot(); });
    connect(
      acc_s0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) { updatePlot(); });
    connect(
      acc_v0, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) { updatePlot(); });
    connect(acc_dist, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [&](double) {
      updatePlot();
    });
    connect(graph_select, QOverload<int>::of(&QComboBox::currentIndexChanged), [&](int) {
      updatePlot();
    });
  }
...
public slots:
  void updatePlot() const
  {
    ...
    QVector<double> as;
    QVector<double> js;
    frenet_planner::Polynomial velocity_polynomial(
      s0->value(), v0->value(), a0->value(), sT->value(), vT->value(), aT->value(),
      duration->value());
    for (auto t = 0.0; t <= duration->value(); t += resolution) {
      ts.push_back(t);
