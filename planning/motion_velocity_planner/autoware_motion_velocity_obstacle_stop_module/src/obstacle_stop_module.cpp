  ...
  const TrajectoryPolygonCollisionCheck & trajectory_polygon_collision_check, size_t ego_idx)
{
  ...
  auto obstacles = convert_to_obstacles(point_cloud, decimated_traj_points, vehicle_info, ego_idx);
  ...
  for (auto itr = stop_pc_obstacle_history_.begin(); itr != stop_pc_obstacle_history_.end();) {
    ...
    if (
      ...
      obstacle_filtering_param_.max_lat_margin_for_stop_against_unknown) {
