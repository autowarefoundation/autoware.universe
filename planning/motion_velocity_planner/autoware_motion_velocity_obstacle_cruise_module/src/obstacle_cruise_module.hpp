...
private:
  ...
  std::unique_ptr<CruisePlannerInterface> create_cruise_planner(rclcpp::Node & node) const
  {
    if (planning_algorithm_ == "pid_base") {
      ...
      return std::make_unique<OptimizationBasedPlanner>(
        node, common_param_, cruise_planning_param_);
    }
    ...
    const VehicleInfo & vehicle_info)
  {
    ...
    std::sort(
      stopped_objects.begin(), stopped_objects.end(), [&](const auto & o1, const auto & o2) {
    ...
    const TrajectoryPolygonCollisionCheck & trajectory_polygon_collision_check) const
  {
    ...
    // check label
    if (!isInsideCruiseObstacle(object->predicted_object.classification.at(0).label)) {
      RCLCPP_DEBUG(
    ...
    const TrajectoryPolygonCollisionCheck & trajectory_polygon_collision_check) const
  {
    ...
    // check label
    if (!isOutsideCruiseObstacle(object->predicted_object.classification.at(0).label)) {
      RCLCPP_DEBUG(
