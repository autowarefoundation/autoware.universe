...
PullOverParameters BehaviorPathPlannerNode::getPullOverParam()
{
...

  {
    ...
    p.after_backward_parking_straight_distance =
      dp("after_backward_parking_straight_distance", 0.5);
...

  {
    ...
    // search configs
    p.algorithm = dp("planning_algorithm", "astar");
    p.freespace_parking_velocity = dp("velocity", 1.0);
    p.vehicle_shape_margin = dp("vehicle_shape_margin", 1.0);
    p.common_parameters.time_limit = dp("time_limit", 5000.0);
    p.common_parameters.minimum_turning_radius = dp("minimum_turning_radius", 0.5);
    p.common_parameters.maximum_turning_radius = dp("maximum_turning_radius", 6.0);
    p.common_parameters.turning_radius_size = dp("turning_radius_size", 11);
    ...
    p.common_parameters.theta_size = dp("theta_size", 48);
    p.common_parameters.angle_goal_range = dp("angle_goal_range", 6.0);

    p.common_parameters.curve_weight = dp("curve_weight", 1.2);
    p.common_parameters.reverse_weight = dp("reverse_weight", 2.00);
    p.common_parameters.lateral_goal_range = dp("lateral_goal_range", 0.5);
    p.common_parameters.longitudinal_goal_range = dp("longitudinal_goal_range", 2.0);

    // costmap configs
    p.common_parameters.obstacle_threshold = dp("obstacle_threshold", 100);
  }

  {
    ...
    p.astar_parameters.only_behind_solutions = dp("only_behind_solutions", false);
    p.astar_parameters.use_back = dp("use_back", true);
    p.astar_parameters.distance_heuristic_weight = dp("distance_heuristic_weight", 1.0);
  }

  {
    ...
    p.rrt_star_parameters.enable_update = dp("enable_update", true);
    p.rrt_star_parameters.use_informed_sampling = dp("use_informed_sampling", true);
    p.rrt_star_parameters.max_planning_time = dp("max_planning_time", 150.0);
    p.rrt_star_parameters.neighbor_radius = dp("neighbor_radius", 8.0);
    p.rrt_star_parameters.margin = dp("margin", 0.1);
  }
