  ...
  const std::shared_ptr<const PlannerData> planner_data)
{
  ...
  const auto left_start_segment_idx = motion_utils::findNearestSegmentIndex(
    resampled_left_bound, current_pose->pose, dist_threshold, yaw_threshold);
  const auto right_start_segment_idx = motion_utils::findNearestSegmentIndex(
    resampled_right_bound, current_pose->pose, dist_threshold, yaw_threshold);
  ...
  const auto left_goal_segment_idx = motion_utils::findNearestSegmentIndex(
    resampled_left_bound, goal_pose, dist_threshold, yaw_threshold);
  const auto right_goal_segment_idx = motion_utils::findNearestSegmentIndex(
    resampled_right_bound, goal_pose, dist_threshold, yaw_threshold);
  ...
  if (left_goal_segment_idx) {
    const auto extended_pose = motion_utils::calcLongitudinalOffsetPose(
      resampled_left_bound, left_goal_segment_idx.get() + 1, vehicle_length);
    if (extended_pose) {
      const auto extended_idx = motion_utils::findNearestSegmentIndex(
        resampled_left_bound, extended_pose.get(), dist_threshold, yaw_threshold);
  ...
  if (right_goal_segment_idx) {
    const auto extended_pose = motion_utils::calcLongitudinalOffsetPose(
      resampled_right_bound, right_goal_segment_idx.get() + 1, vehicle_length);
    if (extended_pose) {
      const auto extended_idx = motion_utils::findNearestSegmentIndex(
        resampled_right_bound, extended_pose.get(), dist_threshold, yaw_threshold);
