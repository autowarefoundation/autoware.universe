...
std::vector<ExtendedPredictedObject> getSafetyCheckTargetObjects(
  ...
  const std::shared_ptr<AvoidanceParameters> & parameters, const bool is_shifting_to_right)
{
  const auto & p = parameters;
  const auto check_right_lanes = (is_shifting_to_right && p->check_shift_side_lane) ||
                                 (!is_shifting_to_right && p->check_other_side_lane);
  const auto check_left_lanes = (!is_shifting_to_right && p->check_shift_side_lane) ||
                                (is_shifting_to_right && p->check_other_side_lane);
