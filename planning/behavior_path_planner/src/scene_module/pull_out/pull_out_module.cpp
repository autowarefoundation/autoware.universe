...
std::vector<Pose> PullOutModule::searchBackedPoses()
{
  ...
  if (!lanelet::utils::query::getClosestLanelet(status_.pull_out_lanes, current_pose, &current_llt)) {
    return {};
  }
  const auto current_point = route_handler::LaneletPoint::fromProjection(current_llt, current_pose);
  const auto pull_out_path = lanelet_route_ptr->getStraightPath(current_point, check_distance, check_distance, false);
