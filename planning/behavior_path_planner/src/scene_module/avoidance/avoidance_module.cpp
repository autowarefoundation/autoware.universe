...
TurnSignalInfo AvoidanceModule::calcTurnSignalInfo(const ShiftedPath & path) const
{
  ...
  const size_t start_idx = std::min(front_shift_line.start_idx, path.shift_length.size() - 1);
  const size_t end_idx = std::min(front_shift_line.end_idx, path.shift_length.size() - 1);
