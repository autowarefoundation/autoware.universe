...
MarkerArray LaneChangeInterface::getModuleVirtualWall()
{
  ...
  const auto & start_pose = module_type_->getLaneChangePath().lane_change_info.lane_changing_start;
  const auto start_marker =
    createLaneChangingVirtualWallMarker(start_pose, name(), clock_->now(), "lane_change_start");

  const auto & end_pose = module_type_->getLaneChangePath().lane_change_info.lane_changing_end;
...
void LaneChangeInterface::updateSteeringFactorPtr(const BehaviorModuleOutput & output)
{
  ...
  const auto start_distance = motion_utils::calcSignedArcLength(
    ...
    status.lane_change_path.lane_change_info.shift_line.start.position);
  const auto finish_distance = motion_utils::calcSignedArcLength(
    output.path->points, current_position,
    status.lane_change_path.lane_change_info.shift_line.end.position);

  // TODO(tkhmy) add handle status TRYING
  steering_factor_interface_ptr_->updateSteeringFactor(
    {status.lane_change_path.lane_change_info.shift_line.start,
     status.lane_change_path.lane_change_info.shift_line.end},
  ...
  const CandidateOutput & output, const LaneChangePath & selected_path) const
{
  ...
  steering_factor_interface_ptr_->updateSteeringFactor(
    {selected_path.lane_change_info.shift_line.start,
     selected_path.lane_change_info.shift_line.end},
  ...
  const PathWithLaneId & path, const TurnSignalInfo & original_turn_signal_info)
{
  ...
  if (path.points.empty()) {
    ...
    current_turn_signal_info.desired_end_point =
      lane_change_path.lane_change_info.lane_changing_end;
    current_turn_signal_info.required_end_point =
      lane_change_path.lane_change_info.lane_changing_end;
