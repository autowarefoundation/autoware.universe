...
void NormalLaneChange::updateLaneChangeStatus()
{
  ...
  // Update status
  status_.current_lanes = status_.lane_change_path.lane_change_info.reference_lanelets;
  status_.lane_change_lanes = status_.lane_change_path.lane_change_info.target_lanelets;
...
std::pair<bool, bool> NormalLaneChange::getSafePath(LaneChangePath & safe_path) const
{
  ...
  if (valid_paths.empty()) {
    safe_path.lane_change_info.reference_lanelets = current_lanes;
    return {false, false};
  }
...
void NormalLaneChange::insertStopPoint(PathWithLaneId & path)
{
  const auto shift_intervals = getRouteHandler()->getLateralIntervalsToPreferredLane(
    status_.lane_change_path.lane_change_info.reference_lanelets.back());
...
PathWithLaneId NormalLaneChange::getReferencePath() const
{
  return utils::getCenterLinePathFromRootLanelet(
    status_.lane_change_path.lane_change_info.target_lanelets.front(), planner_data_);
}
...
std::optional<PathWithLaneId> NormalLaneChange::extendPath()
{
  ...
  const auto lc_start_point =
    status_.lane_change_path.lane_change_info.lane_changing_start.position;
...
TurnSignalInfo NormalLaneChange::updateOutputTurnSignal()
{
  ...
  const auto [turn_signal_command, distance_to_vehicle_front] = utils::getPathTurnSignal(
    ...
    status_.lane_change_path.lane_change_info.shift_line, getEgoPose(), getEgoTwist().linear.x,
...
bool NormalLaneChange::hasFinishedLaneChange() const
{
  ...
  const auto & lane_change_end = status_.lane_change_path.lane_change_info.shift_line.end;
...
{
  const auto & start_position = status_.lane_change_path.lane_change_info.shift_line.start.position;
...
bool NormalLaneChange::hasFinishedAbort() const
{
  ...
  const auto distance_to_finish = motion_utils::calcSignedArcLength(
    ...
    abort_path_->lane_change_info.shift_line.end.position);
...
TurnSignalInfo NormalLaneChange::calcTurnSignalInfo()
{
  ...
  const auto desired_end_length = path.lane_change_info.length.lane_changing * length_ratio;
  ...
  turn_signal_info.required_start_point = path.lane_change_info.shift_line.start;

  // required end pose = in the middle of the lane change
  const auto mid_lane_change_length = path.lane_change_info.length.lane_changing / 2;
...
bool NormalLaneChange::getAbortPath()
{
  ...
  const auto reference_lanelets = selected_path.lane_change_info.reference_lanelets;
  ...
  const auto shift_intervals = route_handler->getLateralIntervalsToPreferredLane(
    selected_path.lane_change_info.reference_lanelets.back(), direction);
  ...
  const PathWithLaneId reference_lane_segment = std::invoke([&]() {
    ...
    if (route_handler->isInGoalRouteSection(
          selected_path.lane_change_info.target_lanelets.back())) {
