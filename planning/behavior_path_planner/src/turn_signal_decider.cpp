  ...
  const RouteHandler & route_handler) const
{
  ...
  // front pose
  const auto vehicle_front_pose = tier4_autoware_utils::calcOffsetPose(current_pose, base_link2front_, 0.0, 0.0);

  // Get nearest intersection and decide turn signal
  auto lane_attribute = std::string("none");
  for (size_t i = 0; i < path.points.size(); ++i) {
    const double distance_from_vehicle_front = motion_utils::calcSignedArcLength(path.points, current_pose.position, current_seg_idx, i) - base_link2front_;

    if (search_distance < distance_from_vehicle_front && turn_signal_info.turn_signal.command == TurnIndicatorsCommand::DISABLE) {
    ...
    for (const auto & lane : lanelets) {
      ...
      const bool cond1 = lane.attributeOr("turn_direction", std::string("none")) != lane_attribute && distance_from_vehicle_front < lane.attributeOr("turn_signal_distance", search_distance);
      ...
      const geometry_msgs::msg::Point lane_front_point = lanelet::utils::conversion::toGeomMsgPt(lane.centerline3d().front());
      const geometry_msgs::msg::Point lane_back_point = lanelet::utils::conversion::toGeomMsgPt(lane.centerline3d().back());

      if(distance_from_vehicle_front > 0.0 && cond1) {
        // lanelet with turn direction is within the search distance
        const size_t nearest_seg_idx = motion_utils::findNearestSegmentIndex(path.points, lane_front_point);
        const double dist_to_lane_front = motion_utils::calcSignedArcLength(path.points, current_pose.position, current_seg_idx, lane_front_point, nearest_seg_idx) - base_link2front_;

        turn_signal_info.desired_start_point = dist_to_lane_front < 0.0 ? lane_front_point : vehicle_front_pose.position;
...
geometry_msgs::msg::Point TurnSignalDecider::get_required_end_point(const lanelet::ConstLineString3d& centerline) const
{
  ...
  const auto resampled_centerline = motion_utils::resamplePath(converted_centerline, resampling_arclength);
