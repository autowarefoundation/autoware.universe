...
CandidateOutput StaticObstacleAvoidanceModule::planCandidate() const
{
  ...
  const uint16_t steering_factor_direction = std::invoke([&output]() {
    return output.lateral_shift > 0.0 ? PlanningFactor::LEFT_SHIFT : PlanningFactor::RIGHT_SHIFT;
  });
  planning_factor_interface_->add(
    output.start_distance_to_path_change, output.finish_distance_to_path_change, sl_front.start,
    sl_back.end, steering_factor_direction, SafetyFactorArray{}, true, 0.0, output.lateral_shift);
