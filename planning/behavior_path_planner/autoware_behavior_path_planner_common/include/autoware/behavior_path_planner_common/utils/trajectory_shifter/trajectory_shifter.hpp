...
public:
   ...
   */
  void set_path(const PathWithLaneId & path);
   ...
   */
  void set_lateral_acceleration_limit(const double lateral_acc);

  /**
   * @brief  Set longitudinal acceleration
   */
  void set_longitudinal_acceleration(const double longitudinal_acc);

  /**
   * @brief  Add shift line. You don't have to care about the start/end_idx.
   */
  void add_shift_line(const ShiftLine & line);

  /**
   * @brief  Set new shift point. You don't have to care about the start/end_idx.
   */
  void set_shift_lines(const std::vector<ShiftLine> & lines);

  /**
   * @brief  Get shift points.
   */
  std::vector<ShiftLine> get_shift_lines() const { return shift_lines_; }

  /**
   * @brief  Get base offset.
   */
  double get_base_offset() const { return base_offset_; }

  /**
   * @brief  Get reference path.
   */
  PathWithLaneId get_reference_path() const { return reference_path_; }
   ...
   */
  void remove_behind_shift_line_and_set_base_offset(const size_t nearest_idx);
