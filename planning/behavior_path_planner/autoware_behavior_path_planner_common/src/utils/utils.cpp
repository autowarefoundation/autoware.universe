  ...
  const PredictedObject & dynamic_object)
{
  ...
  const auto vehicle_front_pose =
    autoware_utils::calc_offset_pose(ego_pose, base_link2front, 0, 0);
  const auto vehicle_rear_pose =
    autoware_utils::calc_offset_pose(ego_pose, base_link2rear, 0, 0);
  ...
  const double radius)
{
  ...
  const double angle_diff =
    std::abs(autoware_utils::normalize_radian(lanelet_angle - pose_yaw));
  ...
  const auto calcLateralDistanceFromBound =
    ...
    for (size_t i = 0; i < bound_line_2d.size() - 1; i++) {
      ...
      const Point inverse_p1 =
        autoware_utils::inverse_transform_point(p1, vehicle_corner_pose);
      const Point inverse_p2 =
        autoware_utils::inverse_transform_point(p2, vehicle_corner_pose);
...
Polygon2d toPolygon2d(const lanelet::BasicPolygon2d & polygon)
{
  ...
  return autoware_utils::is_clockwise(ret)
           ? ret
           : autoware_utils::inverse_clockwise(ret);
}
