...
{
CSpace::CSpace(const Pose& lo, const Pose& hi, double r, std::function<bool(Pose)> is_obstacle_free)
...
Pose CSpace::ellipticInformedSampling(double c_best, const Pose& p_start, const Pose& p_goal)
{
  ...
  while (true)
  {
    ...
    const bool is_inside_ellipse = (pow(x_local / ellipse_r1, 2) + pow(y_local / ellipse_r2, 2) < 1.0);
...
Pose CSpace::interpolate_child2parent(const Pose& pose_child, const Pose& pose_parent, double seg) const
{
...
void CSpace::sampleWayPoints_child2parent(const Pose& pose_child, const Pose& pose_parent, double step,
...
bool CSpace::isValidPath_child2parent(const Pose& pose_child, const Pose& pose_parent, double step) const
{
  auto path = rsspace_.reedsShepp(pose_child, pose_parent);
  const double path_true_length = path.length() * rsspace_.rho_;
...
RRTStar::RRTStar(Pose x_start, Pose x_goal, double mu, double collision_check_resolution, bool is_informed,
                 CSpace cspace)
  : mu_(mu), collision_check_resolution_(collision_check_resolution), is_informed_(is_informed), cspace_(cspace)
{
...
void RRTStar::extend()
{
  ...
  {
    x_rand = cspace_.ellipticInformedSampling(*node_goal_->cost_from_start, node_start_->pose, node_goal_->pose);
  }
  else
  {
  ...
  // Check if reached
  bool is_reached = cspace_.isValidPath_child2parent(node_goal_->pose, node_new->pose, collision_check_resolution_);
  if (is_reached)
  {
...
std::vector<Pose> RRTStar::sampleSolutionWaypoints() const
{
  ...
  {
    const auto node_parent = node->getParent();
    cspace_.sampleWayPoints_child2parent(node->pose, node_parent->pose, collision_check_resolution_, poses);
    node = node_parent;
  }
...
void RRTStar::dumpState(std::string filename) const
{
  ...
  auto serialize_node = [&](const NodeSharedPtr& node) {
    ...
    else
    {
      ...
      std::vector<Pose> poses;
      cspace_.sampleWayPoints_child2parent(node->pose, parent->pose, collision_check_resolution_, poses);
...
NodeSharedPtr RRTStar::addNewNode(const Pose& pose, NodeSharedPtr node_parent)
{
  ...
  auto node_new = std::make_shared<Node>(Node{ pose, cost_from_start, boost::none, cost_to_parent, node_parent });
  nodes_.push_back(node_new);
  node_parent->childs.push_back(node_new);
  return node_new;
}

NodeConstSharedPtr RRTStar::getReconnectTargeNode(const NodeConstSharedPtr node_new,
                                                  const std::vector<NodeConstSharedPtr>& neighbor_nodes) const
{
  ...
  {
    if (cspace_.isValidPath_child2parent(node_neighbor->pose, node_new->pose, collision_check_resolution_))
    {
...
NodeConstSharedPtr RRTStar::getBestParentNode(const Pose& pose_new, const NodeConstSharedPtr& node_nearest,
                                              const std::vector<NodeConstSharedPtr>& neighbor_nodes) const
{
  ...
  {
    const double cost_start_to_new = *(node->cost_from_start) + cspace_.distance(node->pose, pose_new);
