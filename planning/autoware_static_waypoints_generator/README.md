...
This package statically calculates the waypoints satisfying path footprints inside the drivable area.

On narrow-road driving, the default waypoints, which is the middle line between lanelets' right and left boundaries, often causes path footprints outside the drivable area.
...
Instead of online path shape optimization, we introduce static waypoints optimization.
With this static waypoints optimization, we have following advantages.

- We can see the optimized waypoints shape in advance.
...
There are two interfaces to communicate with the waypoints optimizer.
...
The optimized waypoints can be generated from Vector Map Builder's operation.
...
```sh
ros2 launch autoware_static_waypoints_generator run_planning_server.launch.xml vehicle_model:=<vehicle-model>
```
...
The optimized waypoints can be generated from the command line interface by designating
...
```sh
ros2 launch autoware_static_waypoints_generator static_waypoints_generator.launch.xml run_backgrond:=false lanelet2_input_file_path:=<input-osm-path> lanelet2_output_file_path:=<output-osm-path> start_lanelet_id:=<start-lane-id> end_lanelet_id:=<end-lane-id> vehicle_model:=<vehicle-model>
```

The default output map path containing the optimized waypoints locates `/tmp/lanelet2_map.osm`.
...
- The yellow footprints are the original ones from the osm map file.
  - FYI: Footprints are generated based on the waypoints and vehicle size.
...
Sometimes the optimized waypoints footprints are close to the lanes' boundaries.
