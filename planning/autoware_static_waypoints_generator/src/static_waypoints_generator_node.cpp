...
#include "static_waypoints_generator_node.hpp"

#include "autoware_static_waypoints_generator/msg/points_with_lane_id.hpp"
...
#include "utils.hpp"
#include "waypoints_source/bag_ego_trajectory_based_waypoints.hpp"
...
namespace autoware::static_waypoints_generator
{
namespace
{
...
: Node("static_waypoints_generator", node_options)
{
  // publishers
  pub_map_bin_ =
    create_publisher<LaneletMapBin>("lanelet2_map_topic", utils::create_transient_local_qos());
  pub_whole_waypoints_ =
    create_publisher<Trajectory>("output_whole_waypoints", utils::create_transient_local_qos());
  pub_waypoints_ =
    create_publisher<Trajectory>("output_waypoints", utils::create_transient_local_qos());
  ...
  sub_traj_start_index_ = create_subscription<std_msgs::msg::Int32>(
    "/waypoints_updater_helper/traj_start_index", rclcpp::QoS{1},
    [this](const std_msgs::msg::Int32 & msg) {
      update_waypoints_range(msg.data, traj_range_indices_.second);
    });
  sub_traj_end_index_ = create_subscription<std_msgs::msg::Int32>(
    "/waypoints_updater_helper/traj_end_index", rclcpp::QoS{1},
    [this](const std_msgs::msg::Int32 & msg) {
      update_waypoints_range(traj_range_indices_.first, msg.data);
    });
  sub_save_map_ = create_subscription<std_msgs::msg::Bool>(
    "/waypoints_updater_helper/save_map", rclcpp::QoS{1}, [this](const std_msgs::msg::Bool & msg) {
      const auto lanelet2_output_file_path =
        tier4_autoware_utils::getOrDeclareParameter<std::string>(
          *this, "lanelet2_output_file_path");
      if (!waypoints_with_route_ || msg.data) {
        const auto & c = *waypoints_with_route_;
        const auto selected_waypoints = std::vector<TrajectoryPoint>(
          c.waypoints.begin() + traj_range_indices_.first,
          c.waypoints.begin() + traj_range_indices_.second + 1);
        const auto selected_route_lane_ids = get_route_lane_ids_from_points(selected_waypoints);
        save_map(
          lanelet2_output_file_path,
          CenterlineWithRoute{selected_waypoints, selected_route_lane_ids});
      }
    });
  sub_traj_resample_interval_ = create_subscription<std_msgs::msg::Float32>(
    "/waypoints_updater_helper/traj_resample_interval", rclcpp::QoS{1},
  ...
  waypoints_source_ = [&]() {
    const auto waypoints_source_param = declare_parameter<std::string>("waypoints_source");
    if (waypoints_source_param == "optimization_trajectory_base") {
      optimization_trajectory_based_waypoints_ = OptimizationTrajectoryBasedCenterline(*this);
      return CenterlineSource::OptimizationTrajectoryBase;
    } else if (waypoints_source_param == "bag_ego_trajectory_base") {
      return CenterlineSource::BagEgoTrajectoryBase;
    }
    throw std::logic_error(
      "The waypoints source is not supported in autoware_static_waypoints_generator.");
  }();
}

void StaticCenterlineGeneratorNode::update_waypoints_range(
  const int traj_start_index, const int traj_end_index)
{
  if (!waypoints_with_route_ || traj_range_indices_.second + 1 < traj_start_index) {
    return;
  }

  traj_range_indices_ = std::make_pair(traj_start_index, traj_end_index);

  const auto & waypoints = waypoints_with_route_->waypoints;
  const auto selected_waypoints = std::vector<TrajectoryPoint>(
    waypoints.begin() + traj_range_indices_.first,
    waypoints.begin() + traj_range_indices_.second + 1);

  pub_waypoints_->publish(
    motion_utils::convertToTrajectory(selected_waypoints, create_header(this->now())));
}
...
void StaticCenterlineGeneratorNode::run()
{
  ...
  const auto waypoints_with_route = generate_waypoints_with_route();
  traj_range_indices_ = std::make_pair(0, waypoints_with_route.waypoints.size() - 1);

  evaluate(waypoints_with_route.route_lane_ids, waypoints_with_route.waypoints);
  save_map(lanelet2_output_file_path, waypoints_with_route);

  waypoints_with_route_ = waypoints_with_route;
}

CenterlineWithRoute StaticCenterlineGeneratorNode::generate_waypoints_with_route()
{
  ...
  // generate waypoints with route
  auto waypoints_with_route = [&]() {
    if (waypoints_source_ == CenterlineSource::OptimizationTrajectoryBase) {
      ...
      const auto optimized_waypoints =
        optimization_trajectory_based_waypoints_.generate_waypoints_with_optimization(
          *this, *route_handler_ptr_, route_lane_ids);
      return CenterlineWithRoute{optimized_waypoints, route_lane_ids};
    } else if (waypoints_source_ == CenterlineSource::BagEgoTrajectoryBase) {
      const auto bag_waypoints = generate_waypoints_with_bag(*this);
      const auto route_lane_ids = get_route_lane_ids_from_points(bag_waypoints);
      return CenterlineWithRoute{bag_waypoints, route_lane_ids};
    }
    throw std::logic_error(
      "The waypoints source is not supported in autoware_static_waypoints_generator.");
  }();
  ...
  waypoints_with_route.waypoints =
    resample_trajectory_points(waypoints_with_route.waypoints, output_trajectory_interval);

  pub_whole_waypoints_->publish(
    motion_utils::convertToTrajectory(waypoints_with_route.waypoints, create_header(this->now())));

  pub_waypoints_->publish(
    motion_utils::convertToTrajectory(waypoints_with_route.waypoints, create_header(this->now())));

  return waypoints_with_route;
}
...
void StaticCenterlineGeneratorNode::load_map(const std::string & lanelet2_input_file_path)
{
  ...
  const std::string debug_input_file_dir{"/tmp/autoware_static_waypoints_generator/input/"};
  ...
  map_bin_ptr_ = [&]() -> LaneletMapBin::ConstSharedPtr {
    ...
    // NOTE: The original map is stored here since the waypoints will be added to all the
    //       lanelet when lanelet::utils::overwriteLaneletCenterline is called.
    original_map_ptr_ =
      Lanelet2MapLoaderNode::load_map(lanelet2_input_file_path, *map_projector_info_);

    // overwrite more dense waypoints
  ...
  const PlanPath::Request::SharedPtr request, const PlanPath::Response::SharedPtr response)
{
  ...
  for (const auto & lanelet : route_lanelets) {
    ...
    if (!current_lanelet_points.empty()) {
      ...
      autoware_static_waypoints_generator::msg::PointsWithLaneId points_with_lane_id;
...
void StaticCenterlineGeneratorNode::save_map(
  const std::string & lanelet2_output_file_path, const CenterlineWithRoute & waypoints_with_route)
{
  if (!route_handler_ptr_) {
    return;
  }

  const auto & c = waypoints_with_route;
  const auto route_lanelets = utils::get_lanelets_from_ids(*route_handler_ptr_, c.route_lane_ids);

  // update waypoints in map
  utils::update_waypoints(original_map_ptr_, route_lanelets, c.waypoints);
  RCLCPP_INFO(get_logger(), "Updated waypoints in map.");

  // save map with modified center line
  std::filesystem::create_directory("/tmp/static_waypoints_generator");  // TODO(murooka)
  ...
  const std::string debug_output_file_dir{"/tmp/autoware_static_waypoints_generator/output/"};
