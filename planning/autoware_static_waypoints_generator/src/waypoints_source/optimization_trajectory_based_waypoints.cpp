...
OptimizationTrajectoryBasedCenterline::OptimizationTrajectoryBasedCenterline(rclcpp::Node & node)
{
  pub_raw_path_with_lane_id_ =
    node.create_publisher<PathWithLaneId>("input_waypoints", utils::create_transient_local_qos());
  pub_raw_path_ =
    node.create_publisher<Path>("debug/raw_waypoints", utils::create_transient_local_qos());
}

std::vector<TrajectoryPoint>
OptimizationTrajectoryBasedCenterline::generate_waypoints_with_optimization(
  ...
  const std::vector<lanelet::Id> & route_lane_ids)
{
  ...
  // optimize waypoints inside the lane
