...
#ifndef STATIC_WAYPOINTS_GENERATOR_NODE_HPP_
#define STATIC_WAYPOINTS_GENERATOR_NODE_HPP_

#include "autoware_static_waypoints_generator/srv/load_map.hpp"
#include "autoware_static_waypoints_generator/srv/plan_path.hpp"
#include "autoware_static_waypoints_generator/srv/plan_route.hpp"
#include "rclcpp/rclcpp.hpp"
#include "type_alias.hpp"
#include "vehicle_info_util/vehicle_info_util.hpp"
#include "waypoints_source/optimization_trajectory_based_waypoints.hpp"
...
{
using autoware_static_waypoints_generator::srv::LoadMap;
using autoware_static_waypoints_generator::srv::PlanPath;
using autoware_static_waypoints_generator::srv::PlanRoute;
...
private:
  ...
  // plan waypoints
  CenterlineWithRoute generate_waypoints_with_route();
  std::vector<lanelet::Id> get_route_lane_ids_from_points(
    const std::vector<TrajectoryPoint> & points);
  void on_plan_path(
    const PlanPath::Request::SharedPtr request, const PlanPath::Response::SharedPtr response);

  void update_waypoints_range(const int traj_start_index, const int traj_end_index);
  void evaluate(
  ...
  void save_map(
    ...
    const CenterlineWithRoute & waypoints_with_route);
  ...
  std::optional<CenterlineWithRoute> waypoints_with_route_{std::nullopt};

  enum class CenterlineSource {
    OptimizationTrajectoryBase = 0,
    BagEgoTrajectoryBase,
  };
  CenterlineSource waypoints_source_;
  OptimizationTrajectoryBasedCenterline optimization_trajectory_based_waypoints_;
  ...
  rclcpp::Publisher<Trajectory>::SharedPtr pub_whole_waypoints_{nullptr};
  rclcpp::Publisher<Trajectory>::SharedPtr pub_waypoints_{nullptr};
...
#endif  // STATIC_WAYPOINTS_GENERATOR_NODE_HPP_
