...
void update_waypoints(
  ...
  const std::vector<TrajectoryPoint> & new_waypoints)
{
  // get lanelet as reference to update waypoints
  ...
  // store new waypoints in lanelets
  size_t lanelet_idx = 0;
  lanelet::LineString3d waypoints(lanelet::utils::getId());
  for (size_t traj_idx = 0; traj_idx < new_waypoints.size(); ++traj_idx) {
    const auto & traj_pos = new_waypoints.at(traj_idx).pose.position;
    ...
    for (; lanelet_idx < lanelets_ref.size(); ++lanelet_idx) {
      ...
      if (is_inside) {
        ...
        // set center point
        waypoints.push_back(center_point);
      ...
      if (!waypoints.empty()) {
        // set waypoints
        lanelet_map_ptr->add(waypoints);
        lanelet_ref.setCenterline(waypoints);

        // prepare new waypoints
        waypoints = lanelet::LineString3d(lanelet::utils::getId());
      }
    }

    if (traj_idx == new_waypoints.size() - 1 && !waypoints.empty()) {
      auto & lanelet_ref = lanelets_ref.at(lanelet_idx);
