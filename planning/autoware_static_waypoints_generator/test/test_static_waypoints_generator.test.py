...
def generate_test_description():
    lanelet2_map_path = os.path.join(
        get_package_share_directory("autoware_static_waypoints_generator"),
        "test/data/lanelet2_map.osm",
    )

    static_waypoints_generator_node = Node(
        package="autoware_static_waypoints_generator",
        executable="main",
        output="screen",
        parameters=[
            ...
            {"rviz": False},
            {"waypoints_source": "optimization_trajectory_base"},
            ...
            os.path.join(
                get_package_share_directory("autoware_static_waypoints_generator"),
                "config/static_waypoints_generator.param.yaml",
            ),
            os.path.join(
    ...
    return (
        LaunchDescription(
            [
                static_waypoints_generator_node,
                # Start test after 1s - gives time for the autoware_static_waypoints_generator to finish initialization
