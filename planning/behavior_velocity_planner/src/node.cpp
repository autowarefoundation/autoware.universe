  ...
  planner_data_(*this)
{
  ...
  std::cerr << "Subscriber is listening to messages on the topic: "
            << trigger_sub_path_with_lane_id_->get_topic_name() << std::endl;
  ...
  const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  if (!pc.empty()) {
    std::cerr << "transforming pointcloud" << std::endl;
  ...
  const autoware_auto_planning_msgs::msg::PathWithLaneId::ConstSharedPtr input_path_msg)
{
  ...
  std::cerr << "Received trigger message at " << input_path_msg->header.stamp.sec << " sec "
            << input_path_msg->header.stamp.nanosec << " nsec" << std::endl;
  ...
  if (!planner_data_.route_handler_) {
    ...
    std::cerr << "Returned from function due to uninitialized route handler\n";
    return;
  }

  if (input_path_msg->points.empty()) {
    std::cerr << "Returned from function due to empty input path points\n";
    return;
  }
  ...
  lk.unlock();
  std::cerr << "path is about to be published" << std::endl;
