...
visualization_msgs::msg::MarkerArray makePathMarkers(
  const std::vector<autoware_auto_planning_msgs::msg::PathPointWithLaneId> & positions, double z,
  int id, const std::string & ns)
{
  ...
  for (const auto & position : positions) {
    debug_marker.pose.position = tier4_autoware_utils::createMarkerPosition(
      position.point.pose.position.x, position.point.pose.position.y, z);
...
visualization_msgs::msg::MarkerArray createPathMarkers(
  const T & debug_data, const builtin_interfaces::msg::Time current_time)
{
  int id = 0;
  visualization_msgs::msg::MarkerArray path_markers;
  appendMarkerArray(
    makePathMarkers(debug_data.original_path.points, debug_data.z, id++, "original_path"),
    current_time, &path_markers);
  appendMarkerArray(
    makePathMarkers(debug_data.limited_path.points, debug_data.z, id++, "limited_path"),
    current_time, &path_markers);
  appendMarkerArray(
    makePathMarkers(debug_data.interpolated_path.points, debug_data.z, id++, "interpolated_path"),
    current_time, &path_markers);
  return path_markers;
}
...
visualization_msgs::msg::MarkerArray OcclusionSpotInPrivateModule::createDebugMarkerArray()
{
  ...
  appendMarkerArray(createMarkers(debug_data_, module_id_), current_time, &debug_marker_array);
  appendMarkerArray(
    createPathMarkers(debug_data_, current_time), current_time, &debug_marker_array);
  return debug_marker_array;
}
}  // namespace behavior_velocity_planner
