...
//  stop_poses, const std::vector<geometry_msgs::msg::Pose> & slow_poses)
{
  ...
  appendMarkerArray(
    virtual_wall_marker_creator_->createStopVirtualWallMarker(stop_poses, "crosswalk", now, id),
    &wall_marker);
...
visualization_msgs::msg::MarkerArray WalkwayModule::createVirtualWallMarkerArray()
{
  ...
  appendMarkerArray(
    virtual_wall_marker_creator_->createStopVirtualWallMarker(stop_poses, "walkway", now, id),
    &wall_marker);
  return wall_marker;
}
