...
visualization_msgs::msg::MarkerArray TrafficLightModule::createVirtualWallMarkerArray()
{
  ...
  for (const auto & p : debug_data_.dead_line_poses) {
    ...
    appendMarkerArray(
      tier4_autoware_utils::createDeadLineVirtualWallMarker(wall_pose, "traffic_light", now, id++),
      now, &wall_marker);
  }
  ...
  for (const auto & p : debug_data_.stop_poses) {
    const auto wall_pose = planning_utils::toVehicleFrontPose(p, debug_data_.base_link2front);
    appendMarkerArray(
      tier4_autoware_utils::createStopVirtualWallMarker(wall_pose, "traffic_light", now, id++), now,
      &wall_marker);
  }
