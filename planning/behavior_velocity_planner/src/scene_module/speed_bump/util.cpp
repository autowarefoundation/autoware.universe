// Copyright 2020 Tier IV, Inc.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include <common/types.hpp>
#include <scene_module/speed_bump/util.hpp>
#include <utilization/util.hpp>

#include <boost/geometry.hpp>
#include <boost/geometry/geometries/linestring.hpp>
#include <boost/geometry/geometries/point_xy.hpp>

#include <cmath>

#define EIGEN_MPL2_ONLY
#include <Eigen/Core>

namespace behavior_velocity_planner
{
namespace speed_bump_util
{
namespace bg = boost::geometry;
using Point = bg::model::d2::point_xy<double>;
using Polygon = bg::model::polygon<Point>;
using Line = bg::model::linestring<Point>;

bool getBackwardPointFromBasePoint(
  const Eigen::Vector2d & line_point1, const Eigen::Vector2d & line_point2,
  const Eigen::Vector2d & base_point, const double backward_length, Eigen::Vector2d & output_point)
{
  Eigen::Vector2d line_vec = line_point2 - line_point1;
  Eigen::Vector2d backward_vec = backward_length * line_vec.normalized();
  output_point = base_point + backward_vec;
  return true;
}

double velocityByBumpHeight(double bump_height)
{
  // y = - (1.38/0.20) x + 3.105
  // y: velocity [m/s]
  // x: height [m]

  double velocity = 0;
  if (bump_height <= 0.05) {
    velocity = 2.76;  // 10 km/h
  } else if (0.05 < bump_height && bump_height < 0.25) {
    velocity = -1 * (1.38 / 0.20) * bump_height + 3.105;
  } else if (bump_height >= 0.25) {
    velocity = 1.38;  // 5 km/h
  }
  return velocity;
}

bool insertTargetVelocityPoint(
  const autoware_auto_planning_msgs::msg::PathWithLaneId & input, const Polygon & polygon,
  const double & slow_margin, const double & acc_margin, const double & bump_height,
  const PlannerData & planner_data, autoware_auto_planning_msgs::msg::PathWithLaneId & output,
  DebugData & debug_data)
{
  output = input;

  if (!debug_data.acc_poses.empty() && !debug_data.slow_poses.empty()) {
    return true;
  }

  for (size_t i = 0; i < output.points.size() - 1; ++i) {
    const auto p0 = output.points.at(i).point.pose.position;
    const auto p1 = output.points.at(i + 1).point.pose.position;
    const Line line{{p0.x, p0.y}, {p1.x, p1.y}};
    std::vector<Point> collision_points;
    bg::intersection(polygon, line, collision_points);

    if (collision_points.empty()) {
      continue;
    }
    if (debug_data.collision_points.size() > 1) {
      return false;
    }

    for (const auto & cp : collision_points) {
      Eigen::Vector3d point3d(cp.x(), cp.y(), planner_data.current_pose.pose.position.z);

      // check if the point is already in the list
      auto isUniqueMember =
        [debug_data](const Point & point, const std::vector<Eigen::Vector3d> & col_debug_points) {
          if (debug_data.collision_points.empty()) {
            return true;
          }
          for (const auto & cp_debug : col_debug_points) {
            double dist_collision =
              bg::distance(Point(point.x(), point.y()), Point(cp_debug.x(), cp_debug.y()));
            if (dist_collision < 1.0E-6) {
              return false;
            }
          }
          return true;
        };

      if (isUniqueMember(cp, debug_data.collision_points)) {
        debug_data.collision_points.push_back(point3d);
      }
    }

    // -- debug code --
    std::vector<Eigen::Vector3d> line3d;
    line3d.emplace_back(p0.x, p0.y, p0.z);
    line3d.emplace_back(p1.x, p1.y, p1.z);
    debug_data.collision_lines.push_back(line3d);
    // ----------------

    // calculate slow pose
    if (debug_data.slow_poses.empty()) {
      // check nearest collision point
      Point nearest_collision_point{};
      double min_dist = 0.0;
      for (size_t j = 0; j < collision_points.size(); ++j) {
        double dist = bg::distance(Point(p0.x, p0.y), collision_points.at(j));
        if (j == 0 || dist < min_dist) {
          min_dist = dist;
          nearest_collision_point = collision_points.at(j);
          debug_data.nearest_collision_point =
            planning_utils::toRosPoint(collision_points.at(j), p0.z);
        }
      }

      // search target point index
      size_t insert_target_point_idx = 0;
      const double base_link2front = planner_data.vehicle_info_.max_longitudinal_offset_m;
      double length_sum = 0;

      const double target_length = slow_margin + base_link2front;
      Eigen::Vector2d point1, point2;
      point1 << nearest_collision_point.x(), nearest_collision_point.y();
      point2 << p0.x, p0.y;
      length_sum += (point2 - point1).norm();
      for (size_t j = i; 0 < j; --j) {
        if (target_length < length_sum) {
          insert_target_point_idx = j + 1;
          break;
        }
        const auto pj1 = output.points.at(j).point.pose.position;
        const auto pj2 = output.points.at(j - 1).point.pose.position;
        point1 << pj1.x, pj1.y;
        point2 << pj2.x, pj2.y;
        length_sum += (point2 - point1).norm();
      }

      // create target point
      Eigen::Vector2d target_point;
      autoware_auto_planning_msgs::msg::PathPointWithLaneId target_point_with_lane_id;
      getBackwardPointFromBasePoint(
        point2, point1, point2, length_sum - target_length, target_point);
      const int target_velocity_point_idx =
        std::max(static_cast<int>(insert_target_point_idx) - 1, 0);
      target_point_with_lane_id = output.points.at(target_velocity_point_idx);
      target_point_with_lane_id.point.pose.position.x = target_point.x();
      target_point_with_lane_id.point.pose.position.y = target_point.y();
      if (insert_target_point_idx > 0) {
        // calculate z-position of the target point (Internal division point of point1/point2)
        // if insert_target_point_idx is zero, use z-position of target_velocity_point_idx
        const double internal_div_ratio =
          (point1 - target_point).norm() /
          ((point1 - target_point).norm() + (point2 - target_point).norm());
        target_point_with_lane_id.point.pose.position.z =
          output.points.at(insert_target_point_idx).point.pose.position.z *
            (1 - internal_div_ratio) +
          output.points.at(insert_target_point_idx - 1).point.pose.position.z * internal_div_ratio;
      }

      if ((point1 - point2).norm() > 1.0E-3) {
        const double yaw = std::atan2(point1.y() - point2.y(), point1.x() - point2.x());
        target_point_with_lane_id.point.pose.orientation =
          planning_utils::getQuaternionFromYaw(yaw);
      }

      target_point_with_lane_id.point.longitudinal_velocity_mps = velocityByBumpHeight(bump_height);
      // -- debug code --
      debug_data.slow_poses.push_back(target_point_with_lane_id.point.pose);

      planning_utils::insertVelocity(
        output, target_point_with_lane_id, velocityByBumpHeight(bump_height),
        insert_target_point_idx);
    }

    // calculate acc pose
    if (debug_data.acc_poses.empty() && debug_data.collision_points.size() == 2) {
      // check nearest collision point
      Point acc_nearest_collision_point{};
      double acc_min_dist = 0.0;
      for (size_t j = 0; j < collision_points.size(); ++j) {
        double acc_dist = bg::distance(Point(p1.x, p1.y), collision_points.at(j));
        if (j == 0 || acc_dist < acc_min_dist) {
          acc_min_dist = acc_dist;
          acc_nearest_collision_point = collision_points.at(j);
          debug_data.nearest_collision_point =
            planning_utils::toRosPoint(collision_points.at(j), p1.z);
        }
      }

      // search target point index
      size_t acc_insert_target_point_idx = 0;
      const double base_link2rear = planner_data.vehicle_info_.rear_overhang_m;
      double acc_length_sum = 0;

      const double acc_target_length = acc_margin + base_link2rear;
      Eigen::Vector2d point3, point4;
      point3 << acc_nearest_collision_point.x(), acc_nearest_collision_point.y();
      point4 << p1.x, p1.y;
      acc_length_sum += (point4 - point3).norm();
      for (size_t j = 0; i < j; ++j) {
        if (acc_target_length < acc_length_sum) {
          acc_insert_target_point_idx = j - 1;
          break;
        }
        const auto pj3 = output.points.at(j).point.pose.position;
        const auto pj4 = output.points.at(j - 1).point.pose.position;
        point3 << pj3.x, pj3.y;
        point4 << pj4.x, pj4.y;
        acc_length_sum += (point4 - point3).norm();
      }

      // create target point
      Eigen::Vector2d acc_target_point;
      autoware_auto_planning_msgs::msg::PathPointWithLaneId acc_target_point_with_lane_id;
      getBackwardPointFromBasePoint(
        point4, point3, point4, acc_length_sum - acc_target_length, acc_target_point);
      const int acc_target_velocity_point_idx =
        std::max(static_cast<int>(acc_insert_target_point_idx) - 1, 0);
      acc_target_point_with_lane_id = output.points.at(acc_target_velocity_point_idx);
      acc_target_point_with_lane_id.point.pose.position.x = acc_target_point.x();
      acc_target_point_with_lane_id.point.pose.position.y = acc_target_point.y();
      if (acc_insert_target_point_idx > 0) {
        // calculate z-position of the target point (Internal division point of point1/point2)
        // if insert_target_point_idx is zero, use z-position of target_velocity_point_idx
        const double acc_internal_div_ratio =
          (point3 - acc_target_point).norm() /
          ((point3 - acc_target_point).norm() + (point4 - acc_target_point).norm());
        acc_target_point_with_lane_id.point.pose.position.z =
          output.points.at(acc_insert_target_point_idx).point.pose.position.z *
            (1 - acc_internal_div_ratio) +
          output.points.at(acc_insert_target_point_idx - 1).point.pose.position.z *
            acc_internal_div_ratio;
      }

      if ((point3 - point4).norm() > 1.0E-3) {
        const double yaw = std::atan2(point3.y() - point4.y(), point3.x() - point4.x());
        acc_target_point_with_lane_id.point.pose.orientation =
          planning_utils::getQuaternionFromYaw(yaw);
      }
      debug_data.acc_poses.push_back(acc_target_point_with_lane_id.point.pose);
    }

    if (!debug_data.acc_poses.empty() && !debug_data.slow_poses.empty()) {
      return true;
    }

    // ----------------
  }
  return false;
}

void setVelocityFromIndex(const size_t begin_idx, const double vel, PathWithLaneId & input)
{
  for (size_t i = begin_idx; i < input.points.size(); ++i) {
    input.points.at(i).point.longitudinal_velocity_mps =
      std::min(static_cast<float>(vel), input.points.at(i).point.longitudinal_velocity_mps);
  }
}

}  // namespace speed_bump_util
}  // namespace behavior_velocity_planner
