...
visualization_msgs::msg::MarkerArray InvalidLaneletModule::createVirtualWallMarkerArray()
{
  ...
  if((state_ == State::APPROACH) || (state_ == State::INSIDE_INVALID_LANELET) || (state_ == State::STOPPED)){
  ...
  // std::vector<visualization_msgs::msg::Marker>::iterator position = std::find(wall_marker.markers.begin(), wall_marker.markers.end(), 
  // [&](const visualization_msgs::msg::Marker & mrkr){return (mrkr.action == visualization_msgs::msg::Marker::DELETE);});
  // if (position != wall_marker.markers.end()) // == wall_marker.markers.end() means the element was not found
