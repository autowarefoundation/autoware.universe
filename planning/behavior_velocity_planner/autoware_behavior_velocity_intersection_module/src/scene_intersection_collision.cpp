...
#include <autoware/behavior_velocity_planner_common/utilization/trajectory_utils.hpp>  // for smooth_path
  ...
  autoware_internal_debug_msgs::msg::Float64MultiArrayStamped * ego_ttc_array) const
{
  ...
  const double time_delay = (is_go_out || is_prioritized)
                              ...
                              : (planner_param_.collision_detection.collision_detection_hold_time -
                                 collision_state_machine_.get_duration());
  ...
  if (!smooth_path(reference_path, smoothed_reference_path, planner_data_)) {
