...
geometry_msgs::msg::Pose calcTargetPose(const T & path, const PathIndexWithOffset & offset_segment)
{
  ...
  const auto interpolate_ratio = (segment_length - remain_offset_length) / segment_length;  // const auto interpolate_ratio = remain_offset_length / (p_eigen_back - p_eigen_front).norm();
