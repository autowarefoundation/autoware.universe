...
TEST(state_machine, on_initialized)
{
  ...
  EXPECT_EQ(enumToInt(State::GO), enumToInt(state_machine.get_state()));
}

TEST(state_machine, set_state_stop)
{
  StateMachine state_machine = StateMachine();
  state_machine.set_state(State::STOP);
  EXPECT_EQ(enumToInt(State::STOP), enumToInt(state_machine.get_state()));
}
...
TEST(state_machine, set_state_stop_with_margin_time)
{
  ...
  state_machine.set_margin_time(margin_time);
  ...
  EXPECT_EQ(enumToInt(state_machine.get_state()), enumToInt(State::GO));
  state_machine.set_state_with_margin_time(State::STOP, logger, current_time);
  // set STOP immediately when stop is set
  EXPECT_EQ(enumToInt(state_machine.get_state()), enumToInt(State::STOP));
}
...
TEST(state_machine, set_state_go_with_margin_time)
{
  ...
  while (state_machine.get_state() == State::STOP) {
    EXPECT_EQ(enumToInt(state_machine.get_state()), enumToInt(State::STOP));
    rclcpp::Clock current_time = rclcpp::Clock(RCL_ROS_TIME);
    if (state_machine.get_duration() > margin_time) {
      std::cerr << "stop duration is larger than margin time" << std::endl;
    }
    EXPECT_FALSE(state_machine.get_duration() > margin_time);
    state_machine.set_state_with_margin_time(State::GO, logger, current_time);
    loop_counter++;
  }
  // time past STOP -> GO
  if (loop_counter > 2) {
    EXPECT_TRUE(state_machine.get_duration() > margin_time);
    EXPECT_EQ(enumToInt(state_machine.get_state()), enumToInt(State::GO));
  } else {
