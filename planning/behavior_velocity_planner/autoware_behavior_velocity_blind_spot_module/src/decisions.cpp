...
void BlindSpotModule::setRTCStatusByDecision(
  const T &, [[maybe_unused]] const autoware_planning_msgs::msg::PathWithLaneId & path)
{
...
template <>
void BlindSpotModule::setRTCStatusByDecision(
  const Unsafe & decision, const autoware_planning_msgs::msg::PathWithLaneId & path)
{
  setSafe(false);
...
template <>
void BlindSpotModule::reactRTCApprovalByDecision(
  const Unsafe & decision, autoware_planning_msgs::msg::PathWithLaneId * path)
{
  if (!isActivated()) {
...
template <>
void BlindSpotModule::setRTCStatusByDecision(
  const Safe & decision, const autoware_planning_msgs::msg::PathWithLaneId & path)
{
  setSafe(true);
  const auto & current_pose = planner_data_->current_odometry->pose;
  setDistance(autoware::motion_utils::calcSignedArcLength(
...
template <>
void BlindSpotModule::reactRTCApprovalByDecision(
  const Safe & decision, autoware_planning_msgs::msg::PathWithLaneId * path)
{
  if (!isActivated()) {
    constexpr double stop_vel = 0.0;
    planning_utils::setVelocityFromIndex(decision.stop_line_idx, stop_vel, path);
