...
private:
  ...
  // setSafe(), setDistance()
  void setRTCStatus(
    const BlindSpotDecision & decision, const autoware_planning_msgs::msg::PathWithLaneId & path);
  template <typename Decision>
  void setRTCStatusByDecision(
    const Decision & decision, const autoware_planning_msgs::msg::PathWithLaneId & path);
  // stop/GO
  void reactRTCApproval(const BlindSpotDecision & decision, PathWithLaneId * path);
  template <typename Decision>
  void reactRTCApprovalByDecision(
    const Decision & decision, autoware_planning_msgs::msg::PathWithLaneId * path);
