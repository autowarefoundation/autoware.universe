...
void ModuleNotLaunched::update(PlannerDataPtr planner_data)
{
  ...
  if (
    rival_vehicle->longitudinal_from_ego <
    context_->getParameters().too_close_to_overtake_distance) {
    return;
  }
...
{
  if (is_first_time_to_plan_) return;
  auto center_path = *generateCenterLinePath(planner_data);
  auto self_pose_frenet =
    convertToFrenetPoint(center_path.points, planner_data->self_odometry->pose.pose.position, 0);
  auto overtake_finish_pose_frenet =
    convertToFrenetPoint(center_path.points, overtake_finish_pose_.position, 0);
...
{
  auto center_path = *generateCenterLinePath(planner_data);
  auto front_vehicle = util::detectRivalVehicleInEgoCourse(
    planner_data->self_odometry->pose.pose, center_path, planner_data->dynamic_object->objects);
...
void BackToCenter::getPath(PlannerDataPtr planner_data, BehaviorModuleOutput * output)
{
  if (is_first_time_to_plan_) {
    ...
    auto [back_to_center_path, back_to_center_end_pose] = util::calcBackToCenterPath(
      ...
      context_->getParameters().back_to_center_start_distance,
      context_->getParameters().back_to_center_end_distance);
