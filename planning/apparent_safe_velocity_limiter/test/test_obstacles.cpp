...
TEST(TestObstacles, ObstacleTreePoints)
{
  ...
  apparent_safe_velocity_limiter::ObstacleTree tree(points);

  apparent_safe_velocity_limiter::polygon_t query;
...
TEST(TestObstacles, ObstacleTreeLines)
{
  ...
  const apparent_safe_velocity_limiter::multilinestring_t lines = {
      {point_t{-0.5, -0.5}, point_t{0.5,0.5}},
      {point_t{0, 0}, point_t{1,1}},
      {point_t(2, 2), point_t(-2, 2)},
      {point_t{-3,-3}, point_t{-1,-1}, point_t{5, -5}}
  };
  apparent_safe_velocity_limiter::ObstacleTree tree(lines);
