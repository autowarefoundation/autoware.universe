...
TEST(spline_interpolation, spline_vector)
{
...
TEST(spline_interpolation, spline_single_point)
{
  {  // straight: query_keys is same as base_keys
    const std::vector<double> base_keys{0.0, 1.0, 2.0, 3.0, 4.0};
    const std::vector<double> base_values{0.0, 1.5, 3.0, 4.5, 6.0};
    ...
    for (size_t i = 0; i < query_values.size(); ++i) {
      auto query_value_i = spline.getSplineInterpolatedValues(query_keys.at(i));
