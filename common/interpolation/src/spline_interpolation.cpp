  ...
  const std::vector<double> & query_keys)
{
  ...
  return interpolator.compute(query_keys);
}
  ...
  const std::vector<double> & query_keys)
{
  ...
  for (size_t i = 0; i < base_keys.size(); ++i) {
    if (i == 0) {
      s_values.emplace_back(m_values.front());
      continue;
    }
    if (i == base_keys.size() - 1) {
      s_values.emplace_back(m_values.back());
      continue;
    }
    if (i == 1 || i == base_keys.size() - 2) {
      const double s_val = (m_values.at(i - 1) + m_values.at(i)) / 2.0;
      s_values.emplace_back(s_val);
      continue;
    }
  ...
  for (size_t i = 0; i < base_keys.size() - 1; ++i) {
    ...
    c.emplace_back(s_values.at(i));
    d.emplace_back(base_values.at(i));
  }
  ...
  for (const auto & query_key : query_keys) {
    ...
    res.emplace_back(d.at(j) + (c.at(j) + (b.at(j) + a.at(j) * ds) * ds) * ds);
  }
  return res;
}
}  // namespace interpolation
...
SplineInterpolation::SplineInterpolation(
  const std::vector<double> & base_keys, const std::vector<double> & base_values)
{
  calc_spline_coefficients(base_keys, base_values);
}

void SplineInterpolation::calc_spline_coefficients(
  const std::vector<double> & base_keys, const std::vector<double> & base_values)
{
  ...
  v.segment(1, n - 2) = interpolation::detail::solve_tridiagonal_matrix_algorithm(a, b, c, d);
  v[0] = 0;
  v[n - 1] = 0;
...
std::vector<double> SplineInterpolation::compute(const std::vector<double> & query_keys) const
{
  // throw exceptions for invalid arguments
  const auto validated_query_keys = interpolation_utils::validate_keys(base_keys_, query_keys);
...
std::vector<double> SplineInterpolation::compute_diff(const std::vector<double> & query_keys) const
{
  // throw exceptions for invalid arguments
  const auto validated_query_keys = interpolation_utils::validate_keys(base_keys_, query_keys);
...
std::vector<double> SplineInterpolation::compute_quad_diff(
