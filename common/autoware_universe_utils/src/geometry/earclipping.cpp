...
Earclipping::Point * Earclipping::eliminateHole(
  Earclipping::Point * hole, Earclipping::Point * outerNode)
{
...
Earclipping::Point * Earclipping::eliminateHoles(
  const std::vector<LinearRing2d> & inners, Earclipping::Point * outerPoint)
{
...
void Earclipping::earclippingLinked(Earclipping::Point * ear, int pass)
{
  ...
  while (ear->prev != ear->next) {
    ...
    if (ear == stop) {
      if (!pass)
        earclippingLinked(filterPoints(ear), 1);
...
bool Earclipping::pointInTriangle(
...
/// list)
Earclipping::Point * Earclipping::insertPoint(
