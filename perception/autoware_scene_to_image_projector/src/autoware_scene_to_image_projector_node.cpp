...
bool is_polygon_contained(
  const std::vector<cv::Point2f> & containee,
  const std::vector<std::vector<cv::Point2f>> & container)
{
  ...
  if (objects_type == "detected") {
    detected_objects_sub_ =
      this->create_subscription<autoware_perception_msgs::msg::DetectedObjects>(
        "~/input/objects", 10,
        std::bind(
          &SceneToImageProjectorNode::detected_objects_callback, this, std::placeholders::_1));
  ...
  if (camera_info_received_) {
    ...
    if (latest_detected_objects_received_) {
      ...
      if (!object_recognition_utils::transformObjects(
            *latest_detected_objects_, latest_camera_info_->header.frame_id, tf_buffer_,
            transformed_objects)) {
        return;
      }
        ...
        return (
          obj1.kinematics.pose_with_covariance.pose.position.x *
            ...
            obj1.kinematics.pose_with_covariance.pose.position.y *
              obj1.kinematics.pose_with_covariance.pose.position.y +
            obj1.kinematics.pose_with_covariance.pose.position.z *
              obj1.kinematics.pose_with_covariance.pose.position.z <
          obj2.kinematics.pose_with_covariance.pose.position.x *
              obj2.kinematics.pose_with_covariance.pose.position.x +
            obj2.kinematics.pose_with_covariance.pose.position.y *
              obj2.kinematics.pose_with_covariance.pose.position.y +
    ...
    if (latest_tracked_objects_received_) {
      ...
      if (!object_recognition_utils::transformObjects(
            *latest_tracked_objects_, latest_camera_info_->header.frame_id, tf_buffer_,
            transformed_objects)) {
        return;
      }
    ...
    if (latest_trajectory_received_) {
      const auto self_transform = get_transform_anonymous(
        tf_buffer_, latest_trajectory_->header.frame_id, latest_camera_info_->header.frame_id,
      ...
      if (!object_recognition_utils::transformTrajectory(
            *latest_trajectory_, latest_camera_info_->header.frame_id, tf_buffer_,
            transformed_trajectory)) {
        RCLCPP_WARN(get_logger(), "There was a problem with transforming the trajectory");
        return;
      }

      auto last_point = cv::Point2f(0, 0);
    ...
    if (latest_path_received_) {
      const auto self_transform = get_transform_anonymous(
        tf_buffer_, latest_path_->header.frame_id, latest_camera_info_->header.frame_id,
      ...
      auto last_point_left = cv::Point2f(0, 0);
      auto last_point_right = cv::Point2f(0, 0);

      for (size_t i = 1; i < transformed_path.left_bound.size(); i++) {
        Eigen::Vector3d left_position(
          transformed_path.left_bound[i].x, transformed_path.left_bound[i].y,
        ...
        if (last_point_left.x == 0) {
          Eigen::Vector3d left_position_before(
            transformed_path.left_bound[i - 1].x, transformed_path.left_bound[i - 1].y,
      ...
      for (size_t i = 1; i < transformed_path.right_bound.size(); i++) {
        Eigen::Vector3d right_position(
          transformed_path.right_bound[i].x, transformed_path.right_bound[i].y,
        ...
        if (last_point_right.x == 0) {
          Eigen::Vector3d right_position_before(
            transformed_path.right_bound[i - 1].x, transformed_path.right_bound[i - 1].y,
  ...
  std::vector<std::vector<cv::Point2f>> & previous_polygons)
{
  ...
  for (const auto & edge : edges) {
    ...
    if (
      !project_point(point_on_image_1_3d, projection_matrix, point_on_image_1) &&
      !project_point(point_on_image_2_3d, projection_matrix, point_on_image_2))
      continue;  // two of them are not in the projectable area
