...
: Node("pointcloud_based_occupancy_grid_map_node", node_options)
{
  ...
  use_projection_ = this->declare_parameter<bool>("use_projection");
  projection_dz_threshold_ = this->declare_parameter<double>("projection_dz_threshold");
  obstacle_separation_threshold_ = this->declare_parameter<double>("obstacle_separation_threshold");
  pub_debug_grid_ = this->declare_parameter<bool>("pub_debug_grid");
  ...
  if (pub_debug_grid_) {
    debug_grid_map_publisher_ptr_ = this->create_publisher<grid_map_msgs::msg::GridMap>(
  ...
  const PointCloud2::ConstSharedPtr & input_raw_msg)
{
  ...
  grid_map::GridMap debug_grid;
  if (use_projection_) {
    single_frame_occupancy_grid_map.updateWithPointCloud3D(
      filtered_raw_pc, filtered_obstacle_pc_common, robot_pose, scan_origin,
      projection_dz_threshold_, obstacle_separation_threshold_,
      (pub_debug_grid_) ? &debug_grid : nullptr);
  } else {
    single_frame_occupancy_grid_map.updateWithPointCloudSimple(
      filtered_raw_pc, filtered_obstacle_pc_common, robot_pose, scan_origin);
  }
  ...
  if (pub_debug_grid_) {
    debug_grid_map_publisher_ptr_->publish(grid_map::GridMapRosConverter::toMessage(debug_grid));
  }
}
