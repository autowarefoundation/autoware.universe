...
PreprocessCuda::PreprocessCuda(const TransfusionConfig & config, cudaStream_t & stream) :
  ...
  unsigned int mask_size = config_.grid_z_size_ * config_.grid_y_size_ *
                           config_.grid_x_size_ * sizeof(unsigned int);

  unsigned int voxels_size = config_.grid_z_size_ * config_.grid_y_size_ *
                             config_.grid_x_size_ *
                             config_.max_num_points_per_pillar_ *
                             config_.num_point_feature_size_ * sizeof(float);
...
int PreprocessCuda::generateVoxels(float *points, unsigned int points_size,
  ...
  CHECK_CUDA_ERROR(generateVoxels_random_launch(
      points, points_size, mask_, voxels_, stream_));

  CHECK_CUDA_ERROR(generateBaseFeatures_launch(
      mask_, voxels_, pillar_num, voxel_features,
...
__global__ void generateVoxels_random_kernel(
    ...
    float pillar_x_size, float pillar_y_size, float pillar_z_size, int grid_y_size, int grid_x_size,
    int points_per_voxel, unsigned int *mask, float *voxels) {
...
__global__ void generateBaseFeatures_kernel(unsigned int *mask, float *voxels,
                                            ...
                                            float points_per_voxel, float max_voxels,
                                            unsigned int *pillar_num,
                                            float *voxel_features,
                                            unsigned int *voxel_num,
                                            unsigned int *voxel_idxs) {
...
// create 4 channels
cudaError_t
PreprocessCuda::generateBaseFeatures_launch(
                            unsigned int *mask, float *voxels, unsigned int *pillar_num,
                            float *voxel_features, unsigned int *voxel_num,
                            unsigned int *voxel_idxs, cudaStream_t stream) {
  dim3 threads = {32, 32};
  dim3 blocks = {divup(config_.grid_x_size_, threads.x),
                 divup(config_.grid_y_size_, threads.y)};

  generateBaseFeatures_kernel<<<blocks, threads, 0, stream>>>(
      mask, voxels, config_.grid_y_size_, config_.grid_x_size_,
      config_.points_per_voxel_, config_.max_voxels_,
      pillar_num, voxel_features, voxel_num, voxel_idxs);
  cudaError_t err = cudaGetLastError();
  return err;
}

__device__ int decode_field(unsigned char *cloud_data, unsigned long point_offset,
    unsigned int field_offset, unsigned char field_size, bool is_bigendian) {
  int value = 0;
...
__global__ void generateVoxelsInput_kernel(
    ...
    unsigned char y_size, unsigned char z_size, unsigned char intensity_size,
    unsigned int point_step, bool is_bigendian, unsigned int cloud_capacity, 
    unsigned int points_agg, unsigned int points_size, float time_lag, float *affine_past2current,
    float *points) {
  int point_idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (point_idx >= points_size || points_agg + point_idx >= cloud_capacity)
    return;
  ...
  int intensity_raw = decode_field(cloud_data, point_offset, intensity_offset,
    intensity_size, is_bigendian);
...
cudaError_t PreprocessCuda::generateVoxelsInput_launch(
                        uint8_t *cloud_data, CloudInfo & cloud_info, unsigned int points_agg,
                        unsigned int points_size, float time_lag, float *affine_past2current,
                        float *points, cudaStream_t stream) {
  dim3 threads = {1024};
  ...
  generateVoxelsInput_kernel<<<blocks, threads, 0, stream>>>(
      ...
      cloud_info.z_datatype, cloud_info.intensity_datatype, cloud_info.x_size,
      cloud_info.y_size, cloud_info.z_size, cloud_info.intensity_size, cloud_info.point_step,
      cloud_info.is_bigendian, config_.cloud_capacity_, points_agg, points_size, time_lag,
      affine_past2current, points);
  cudaError_t err = cudaGetLastError();
  return err;
  }
