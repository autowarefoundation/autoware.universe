...
public:
  ...
  ///// INPUT PARAMETERS /////
  std::size_t cloud_capacity_ = 1000000;
  ///// KERNEL PARAMETERS /////
  ...
  ///// NETWORK PARAMETERS /////
  std::size_t num_classes_ = 10;
  std::size_t num_point_feature_size_ = 5;  // x, y, z, intensity, lag
  ...
  std::size_t out_size_factor_ = 4;
  std::size_t max_num_points_per_pillar_ = points_per_voxel_;
  std::size_t num_point_values_ = 4;
  std::size_t num_proposals_ = 200;
  // the number of feature maps for pillar scatter
  std::size_t num_feature_scatter_ = pillar_feature_size_;
  ...
  std::size_t pillar_points_bev_ = max_num_points_per_pillar_ * max_num_pillars_;
  // the detected boxes result decode by (x, y, z, w, l, h, yaw)
  std::size_t num_box_values_ = 8;
  // the input size of the 2D backbone network
  std::size_t grid_x_size_ = (max_x_range_ - min_x_range_) / voxel_x_size_;
  std::size_t grid_y_size_ = (max_y_range_ - min_y_range_) / voxel_y_size_;
  std::size_t grid_z_size_ = (max_z_range_ - min_z_range_) / voxel_z_size_;
