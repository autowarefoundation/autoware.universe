...
TrtYoloX::TrtYoloX(
  ...
  const size_t max_workspace_size, const std::string & calibration_image_list_file,
  const double norm_factor, tensorrt_common::BuildConfig build_config, const bool use_gpu_preprocess)
{
  src_width_ = -1;
  src_height_ = -1;
  ...
  if (precision == "int8") {
    if (calibration_image_list_file.empty()) {
      std::runtime_error("calibration_image_list_file should be passed to generate int8 engine.");
    }
...
void TrtYoloX::preprocess(const std::vector<cv::Mat> & images)
{
  ...
  const auto chw_images =
    cv::dnn::blobFromImages(dst_images, norm_factor_, cv::Size(), cv::Scalar(), false, false, CV_32F);
