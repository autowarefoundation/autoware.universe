...
: FusionNode<sensor_msgs::msg::PointCloud2, DetectedObjects>("pointpainting_fusion", options)
{
  ...
  const float yaw_norm_threshold =
    static_cast<float>(this->declare_parameter<double>("yaw_norm_threshold", 1.5));
  // densification param
  ...
  centerpoint::CenterPointConfig config(
    ...
    downsample_factor, encoder_in_feature_size, score_threshold,
    circle_nms_dist_threshold yaw_norm_threshold);
