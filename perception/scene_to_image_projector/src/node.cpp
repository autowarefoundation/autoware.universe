...
//  auto sub_road_boundaries = declare_parameter<bool>("sub_road_boundaries", false);
  ...
  if (use_trajectories) {
    trajectory_sub_ = this->create_subscription<autoware_auto_planning_msgs::msg::Trajectory>(
      "~/input/trajectory", 1,
      std::bind(&SceneToImageProjectorNode::TrajectoryCallback, this, std::placeholders::_1));
  }
...
{
  RCLCPP_INFO(get_logger(), "Trajectory received, header: %s", msg->header.frame_id.c_str());
  msg->points.size();
}
