  ...
  z_(object.kinematics.pose_with_covariance.pose.position.z)
{
  ...
  // Set initial state
  {
    ...
    if (!object.kinematics.has_position_covariance) {
      ...
      const double p0_stddev_x = 2.0;                                    // in object coordinate [m]
      const double p0_stddev_y = 2.0;                                    // in object coordinate [m]
      const double p0_stddev_yaw = tier4_autoware_utils::deg2rad(1000);  // in map coordinate [rad]
