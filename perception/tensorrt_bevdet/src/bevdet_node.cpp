...
void box3DToDetectedObjects(const std::vector<Box>& boxes, 
                    ...
                    float score_thre, const bool has_twist=true)
{
    for(auto b : boxes)
    {   
...
TRTBEVDetNode::TRTBEVDetNode(const std::string & node_name, const rclcpp::NodeOptions & node_options)
: rclcpp::Node(node_name, node_options)
{
    ...
    bevdet_ = std::make_shared<BEVDet>(model_config_, img_N_, sampleData_.param.cams_intrin, 
                sampleData_.param.cams2ego_rot, sampleData_.param.cams2ego_trans, onnx_file_, 
                                                    engine_file_);
    ...
    pub_cloud_ = this ->create_publisher<sensor_msgs::msg::PointCloud2>("~/output/painting_cloud", rclcpp::SensorDataQoS()); 
    pub_boxes_ = this ->create_publisher<autoware_perception_msgs::msg::DetectedObjects>("~/output/boxes", rclcpp::QoS{1});   
    
    sub_cloud_.subscribe(this, "~/input/topic_cloud", rclcpp::QoS{1}.get_rmw_qos_profile());//rclcpp::QoS{1}.get_rmw_qos_profile()
    sub_f_img_.subscribe(this, "~/input/topic_img_f", rclcpp::QoS{1}.get_rmw_qos_profile());//rmw_qos_profile_sensor_data
    ...
    sync_->registerCallback(std::bind(&TRTBEVDetNode::callback,this, _1, _2, _3, _4, _5, _6,_7)); // 绑定回调函数
  
}


void TRTBEVDetNode::callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr & msg_cloud, 
