...
#include "bevpool_plugin.hpp"
#include "gatherbev_plugin.hpp"
#include "preprocess_plugin.hpp"
...
void BEVDet::ExportEngine(const std::string & onnxFile, const std::string & trtFile)
{
  ...
  if (engineFile.fail()) {
    std::cout << "Failed saving .engine file!" << std::endl;
    return;
  }
  std::cout << "Succeeded saving .engine file!" << std::endl;
}
