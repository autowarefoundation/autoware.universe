...
protected:
    void SetUp() override {
        ...
        const auto plugin_dir =
                ament_index_cpp::get_package_prefix("trt_batched_nms");
        ...
        trt_rtmdet_ = std::make_unique<autoware::tensorrt_rtmdet::TrtRTMDet>(
                model_path, precision_, color_map_, score_threshold_, nms_threshold_, mask_threshold_, build_config,
                true, "", norm_factor, mean_, std_, cache_dir,
...
public:
    ...
    /**
     * @brief Wrapper function to test the autoware::tensorrt_rtmdet::TrtRTMDet::preprocess_gpu function.
     *
...
TEST_F(TrtRTMDetTest, TestFeedForward) {
    ...
    // Check if the output objects class ids is correct (the label_id is derived from the COCO dataset.)
    ...
    // Check if the output objects class ids is correct (label_id, which is used to process the outputs in Autoware.)
    EXPECT_EQ(class_ids.at(0), 1);

    // Check if the one of the mask pixel which is in the object is not zero (zero represents the background.)
    // There is a car in the test image, so the mask pixel value should not be zero.
...
TEST_F(TrtRTMDetTest, TestNmsSortedBboxes) {
    ...
    autoware::tensorrt_rtmdet::TrtRTMDet::nms_sorted_bboxes(input_objects, output_objects, nms_threshold);
