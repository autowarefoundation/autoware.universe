...
#ifndef TENSORRT_RTMDET__TRT_BATCHED_NMS__COMMON__NMS__KERNEL_H_
#define TENSORRT_RTMDET__TRT_BATCHED_NMS__COMMON__NMS__KERNEL_H_
...
pluginStatus_t allClassNMS(
  ...
  float nms_threshold, bool share_location, bool isNormalized, DataType DT_SCORE, DataType DT_BBOX,
...
size_t sortScoresPerClassWorkspaceSize(
  int num, int num_classes, int num_preds_per_class, DataType DT_CONF);
...
pluginStatus_t sortScoresPerImage(
  cudaStream_t stream, int num_images, int num_items_per_image, DataType DT_SCORE,
...
pluginStatus_t sortScoresPerClass(
  ...
  float confidence_threshold, DataType DT_SCORE, void * conf_scores_gpu, void * index_array_gpu,
  void * workspace);

size_t calculateTotalWorkspaceSize(size_t * workspaces, int count);

pluginStatus_t permuteData(
  cudaStream_t stream, int nthreads, int num_classes, int num_data, int num_dim, DataType DT_DATA,
...
pluginStatus_t gatherNMSOutputs(
  ...
  int topK, int keepTopK, DataType DT_BBOX, DataType DT_SCORE, const void * indices,
  const void * scores, const void * bboxData, void * nmsedDets, void * nmsedLabels,
  void * nmsedIndex = nullptr, bool clipBoxes = true, bool rotated = false);
...
#endif  // TENSORRT_RTMDET__TRT_BATCHED_NMS__COMMON__NMS__KERNEL_H_
