...
TrtRTMDet::TrtRTMDet(
  ...
  const std::string & color_map_path, const std::vector<std::string> & plugin_paths)
{
  src_width_ = -1;
  src_height_ = -1;
  ...
  tier4_perception_msgs::msg::DetectedObjectsWithMask & detected_objects_with_mask)
{
  ...
  for (size_t batch = 0; batch < batch_size; ++batch) {
    for (const auto & object : objects[batch]) {
      ...
      detected_object_with_mask.object.classification =
        std::vector<autoware_perception_msgs::msg::ObjectClassification>{classification};
      cv::Mat mask(
        model_input_height_, model_input_width_, CV_32F,
        &out_masks_h_
          [(batch * 20 * model_input_width_ * model_input_height_) +
  ...
  for (size_t batch = 0; batch < batch_size; ++batch) {
    ...
    for (const auto & object : objects[batch]) {
      ...
      cv::rectangle(
        output_image, cv::Point(static_cast<int>(object.x1), static_cast<int>(object.y1)),
        cv::Point(static_cast<int>(object.x2), static_cast<int>(object.y2)),
        color_map_[object.class_id].color, 2);
      // Write the class name
      cv::putText(
        output_image, color_map_[object.class_id].class_name,
        cv::Point(static_cast<int>(object.x1), static_cast<int>(object.y1)),
