...
#include "tensorrt_rtmdet/trt_batched_nms/common/trt_plugin_helper.hpp"
  ...
  for (int i = blockIdx.x * nthds_per_cta + threadIdx.x; i < numImages * keepTopK;
    ...
    if (index == -1) {
      ...
      if (rotated) {
        ...
        // clipped bbox angle
        nmsedDets[i * 6 + 4] =
          clipBoxes ? max(min(bboxData[bboxId + 4], T_BBOX(1.)), T_BBOX(0.)) : bboxData[bboxId + 4];
...
pluginStatus_t gatherNMSOutputs_gpu(
  cudaStream_t stream, const bool shareLocation, const int numImages, const int numPredsPerClass,
  const int numClasses, const int topK, const int keepTopK, const void * indices,
  const void * scores, const void * bboxData, void * nmsedDets, void * nmsedLabels,
  void * nmsedIndex, bool clipBoxes)
{
  const int BS = 32;
  const int GS = 32;
...
bool nmsOutputInit()
{
  nmsOutFuncVec.push_back(nmsOutLaunchConfig(
    DataType::kFLOAT, DataType::kFLOAT, false, gatherNMSOutputs_gpu<float, float, false>));
  nmsOutFuncVec.push_back(nmsOutLaunchConfig(
    DataType::kFLOAT, DataType::kFLOAT, true, gatherNMSOutputs_gpu<float, float, true>));
  return true;
}
...
pluginStatus_t gatherNMSOutputs(
  cudaStream_t stream, const bool shareLocation, const int numImages, const int numPredsPerClass,
  const int numClasses, const int topK, const int keepTopK, const DataType DT_BBOX,
  const DataType DT_SCORE, const void * indices, const void * scores, const void * bboxData,
  void * nmsedDets, void * nmsedLabels, void * nmsedIndex, bool clipBoxes, bool rotated)
{
  ...
  for (unsigned i = 0; i < nmsOutFuncVec.size(); ++i) {
    if (lc == nmsOutFuncVec[i]) {
      ...
      return nmsOutFuncVec[i].function(
        stream, shareLocation, numImages, numPredsPerClass, numClasses, topK, keepTopK, indices,
        scores, bboxData, nmsedDets, nmsedLabels, nmsedIndex, clipBoxes);
    }
  }
