  ...
  void * index_array_gpu, void * workspace)
{
  ...
  cub::DeviceSegmentedRadixSort::SortPairsDescending(
    ...
    (const int *)(temp_idx), (int *)(index_array_gpu), arrayLen, num_segments,
    (const int *)d_offsets, (const int *)d_offsets + 1, 0, sizeof(T_SCORE) * 8, stream);
...
pluginStatus_t sortScoresPerClass(
  cudaStream_t stream, const int num, const int num_classes, const int num_preds_per_class,
  const int background_label_id, const float confidence_threshold, const DataType DT_SCORE,
  ...
  for (unsigned i = 0; i < sspcFuncVec.size(); ++i) {
    if (lc == sspcFuncVec[i]) {
      ...
      return sspcFuncVec[i].function(
        stream, num, num_classes, num_preds_per_class, background_label_id, confidence_threshold,
        conf_scores_gpu, index_array_gpu, workspace);
    }
  }
...
size_t sortScoresPerClassWorkspaceSize(
  const int num, const int num_classes, const int num_preds_per_class, const DataType DT_CONF)
{
  size_t wss[4];
  const int arrayLen = num * num_classes * num_preds_per_class;
