  ...
  const std::string & file_name)
{
  ...
  for (size_t objects1_idx = 0; objects1_idx < objects1.objects.size(); ++objects1_idx) {
    ...
    for (size_t objects0_idx = 0; objects0_idx < objects0.objects.size(); ++objects0_idx) {
      ...
      const double score = calcScoreBetweenObjects(object0, object1, pair_log_data);
 ...
 *
 * @param objects0 : measurements
 * @param objects1 : tracker inner obhjects
 * @param debug_log
 * @param file_name
 * @return Eigen::MatrixXd
 */
Eigen::MatrixXd DataAssociation::calcScoreMatrix(
  const autoware_auto_perception_msgs::msg::TrackedObjects & objects0,
  const std::vector<TrackerState> & trackers, const bool debug_log, const std::string & file_name)
{
  // for debug
  nlohmann::json log_data;
  std::string log_file_name = "association_log.json";
  ...
  for (size_t trackers_idx = 0; trackers_idx < trackers.size(); ++trackers_idx) {
    ...
    for (size_t objects0_idx = 0; objects0_idx < objects0.objects.size(); ++objects0_idx) {
      const auto & object0 = objects0.objects.at(objects0_idx);
      nlohmann::json pair_log_data;
      pair_log_data["tracker_idx"] = trackers_idx;
      pair_log_data["measurement_idx"] = objects0_idx;
      const double score = calcScoreBetweenObjects(object0, object1, pair_log_data);
...
double DataAssociation::calcScoreBetweenObjects(
  ...
  const autoware_auto_perception_msgs::msg::TrackedObject & object1, nlohmann::json & pair_log_data)
{
