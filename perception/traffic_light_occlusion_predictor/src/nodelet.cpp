...
#include <tier4_autoware_utils/system/stop_watch.hpp>
  >>>>>>> 63dc9e9ab (fix: occlusion predictor)
  ...
  namespace traffic_light
{
    ...
    tf_listener_(tf_buffer_)
  {
    ...
    // publishers
    signal_pub_ = create_publisher<tier4_perception_msgs::msg::TrafficSignalArray>(
    ...
    const autoware_auto_mapping_msgs::msg::HADMapBin::ConstSharedPtr input_msg)
  {
    ...
    for (auto tl_itr = all_lanelet_traffic_lights.begin();
         tl_itr != all_lanelet_traffic_lights.end(); ++tl_itr) {
      ...
      for (auto lsp : lights) {
        ...
        traffic_light_position_map_[lsp.id()] =
          traffic_light_utils::getTrafficLightCenter(string3d);
      }
    }
  }
    ...
    const uint8_t traffic_light_type)
  {
    ...
    //           << in_signal_msg->header.stamp.nanosec << "- size:" <<
    //           in_signal_msg->signals.size()
    //           << std::endl;
    ...
    if (in_cloud_msg == nullptr || in_cam_info_msg == nullptr || in_roi_msg == nullptr) {
      ...
      // std::cout << "in_cloud_msg == nullptr || in_cam_info_msg == nullptr || in_roi_msg ==
      // nullptr"
      //           << std::endl;
      // return;
    } else {
      ...
      //   std::cout << +traffic_light_type << "traffic_light
      //   OcclusionPredictorNodelet::syncCallback"
      ...
      if (selected_roi_msg.rois.size() != in_signal_msg->signals.size()) {
        ...
        auto selected_roi_msg_ptr =
          std::make_shared<const tier4_perception_msgs::msg::TrafficLightRoiArray>(
            selected_roi_msg);
