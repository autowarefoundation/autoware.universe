...
autoware_perception_msgs::msg::PredictedObjects generate_perception_msg()
{
  ...
  // // {
    ...
    // {
      ...
      p.x = 9.0f; p.y = 11.0f; p.z = 0.0f; s.footprint.points.push_back(p);
      p.x = 11.0f; p.y = 11.0f; p.z = 0.0f; s.footprint.points.push_back(p);
      p.x = 11.0f; p.y = 9.0f; p.z = 0.0f; s.footprint.points.push_back(p);
      p.x = 9.0f; p.y = 9.0f; p.z = 0.0f; s.footprint.points.push_back(p);
...
TEST(AutowareAutoMsgsAdapter, TestPredectedObjecst)  // NOLINT for gtest
{
  ...
  auto sub =
    node_subscriber->create_subscription<autoware_auto_perception_msgs::msg::PredictedObjects>(
      topic_name_target, rclcpp::QoS{1},
      [&msg_perception, &test_completed](
        ...
        EXPECT_FLOAT_EQ(msg->objects[0].existence_probability, msg_perception.objects[0].existence_probability);
        EXPECT_EQ(msg->objects[0].classification[0].label, msg_perception.objects[0].classification[0].label);
        EXPECT_FLOAT_EQ(msg->objects[0].classification[0].probability, msg_perception.objects[0].classification[0].probability);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_pose_with_covariance.pose.position.x, 
              msg_perception.objects[0].kinematics.initial_pose_with_covariance.pose.position.x);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_pose_with_covariance.pose.position.y, 
              msg_perception.objects[0].kinematics.initial_pose_with_covariance.pose.position.y);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_pose_with_covariance.pose.position.z, 
              msg_perception.objects[0].kinematics.initial_pose_with_covariance.pose.position.z);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_pose_with_covariance.pose.orientation.x, 
              msg_perception.objects[0].kinematics.initial_pose_with_covariance.pose.orientation.x);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_pose_with_covariance.pose.orientation.y, 
              msg_perception.objects[0].kinematics.initial_pose_with_covariance.pose.orientation.y);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_pose_with_covariance.pose.orientation.z, 
              msg_perception.objects[0].kinematics.initial_pose_with_covariance.pose.orientation.z);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_pose_with_covariance.pose.orientation.w, 
              msg_perception.objects[0].kinematics.initial_pose_with_covariance.pose.orientation.w);

        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_twist_with_covariance.twist.linear.x, 
              msg_perception.objects[0].kinematics.initial_twist_with_covariance.twist.linear.x);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_twist_with_covariance.twist.linear.y, 
              msg_perception.objects[0].kinematics.initial_twist_with_covariance.twist.linear.y);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_twist_with_covariance.twist.linear.z, 
              msg_perception.objects[0].kinematics.initial_twist_with_covariance.twist.linear.z);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_twist_with_covariance.twist.angular.x, 
              msg_perception.objects[0].kinematics.initial_twist_with_covariance.twist.angular.x);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_twist_with_covariance.twist.angular.y, 
              msg_perception.objects[0].kinematics.initial_twist_with_covariance.twist.angular.y);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_twist_with_covariance.twist.angular.z, 
              msg_perception.objects[0].kinematics.initial_twist_with_covariance.twist.angular.z);

        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_acceleration_with_covariance.accel.linear.x, 
              msg_perception.objects[0].kinematics.initial_acceleration_with_covariance.accel.linear.x);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_acceleration_with_covariance.accel.linear.y, 
              msg_perception.objects[0].kinematics.initial_acceleration_with_covariance.accel.linear.y);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_acceleration_with_covariance.accel.linear.z, 
              msg_perception.objects[0].kinematics.initial_acceleration_with_covariance.accel.linear.z);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_acceleration_with_covariance.accel.angular.x, 
              msg_perception.objects[0].kinematics.initial_acceleration_with_covariance.accel.angular.x);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_acceleration_with_covariance.accel.angular.y, 
              msg_perception.objects[0].kinematics.initial_acceleration_with_covariance.accel.angular.y);
        EXPECT_FLOAT_EQ(msg->objects[0].kinematics.initial_acceleration_with_covariance.accel.angular.z, 
        ...
        {
          EXPECT_FLOAT_EQ(msg->objects[0].kinematics.predicted_paths[0].path[i].position.x, 
                msg_perception.objects[0].kinematics.predicted_paths[0].path[i].position.x);
          EXPECT_FLOAT_EQ(msg->objects[0].kinematics.predicted_paths[0].path[i].position.y, 
                msg_perception.objects[0].kinematics.predicted_paths[0].path[i].position.y);
          EXPECT_FLOAT_EQ(msg->objects[0].kinematics.predicted_paths[0].path[i].position.z, 
        ...
        {
          EXPECT_FLOAT_EQ(msg->objects[0].shape.footprint.points[i].x, 
                msg_perception.objects[0].shape.footprint.points[i].x);
          EXPECT_FLOAT_EQ(msg->objects[0].shape.footprint.points[i].y, 
                msg_perception.objects[0].shape.footprint.points[i].y);
          EXPECT_FLOAT_EQ(msg->objects[0].shape.footprint.points[i].z, 
                msg_perception.objects[0].shape.footprint.points[i].z);
        }
        EXPECT_FLOAT_EQ(msg->objects[0].shape.dimensions.x, 
              msg_perception.objects[0].shape.dimensions.x);
        EXPECT_FLOAT_EQ(msg->objects[0].shape.dimensions.y, 
              msg_perception.objects[0].shape.dimensions.y);
        EXPECT_FLOAT_EQ(msg->objects[0].shape.dimensions.z, 
              msg_perception.objects[0].shape.dimensions.z);
