...
TEST(AutowareAutoMsgsAdapter, TestTrajectory)  // NOLINT for gtest
{
    ...
    auto sub = node_subscriber->create_subscription<
        ...
        for (size_t i = 0; i < msg_planning.points.size(); i++) {
            EXPECT_FLOAT_EQ(
            msg->points.at(i).pose.position.x,
            msg_planning.points.at(i).pose.position.x);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).pose.position.y,
            msg_planning.points.at(i).pose.position.y);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).pose.position.z,
            msg_planning.points.at(i).pose.position.z);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).pose.orientation.x,
            msg_planning.points.at(i).pose.orientation.x);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).pose.orientation.y,
            msg_planning.points.at(i).pose.orientation.y);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).pose.orientation.z,
            msg_planning.points.at(i).pose.orientation.z);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).pose.orientation.w,
            msg_planning.points.at(i).pose.orientation.w);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).longitudinal_velocity_mps,
            msg_planning.points.at(i).longitudinal_velocity_mps);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).lateral_velocity_mps,
            msg_planning.points.at(i).lateral_velocity_mps);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).acceleration_mps2,
            msg_planning.points.at(i).acceleration_mps2);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).heading_rate_rps,
            msg_planning.points.at(i).heading_rate_rps);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).front_wheel_angle_rad,
            msg_planning.points.at(i).front_wheel_angle_rad);
            EXPECT_FLOAT_EQ(
            msg->points.at(i).rear_wheel_angle_rad,
            msg_planning.points.at(i).rear_wheel_angle_rad);        
        }
