...
RoutingNode::RoutingNode(const rclcpp::NodeOptions & options) : Node("routing", options)
{
  ...
  adaptor.init_cli(cli_change_route_, group_cli_);
...
{
  const auto inner_req = std::make_shared<planning_interface::ChangeRoute::Service::Request>();
  *inner_req = conversion::convert_set_route(*req);

  const auto inner_res = cli_change_route_->call(inner_req);
  component_interface_utils::status::copy(inner_res, res);  // NOLINT cpplint false positive
}
