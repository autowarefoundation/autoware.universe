...
void MotionNode::change_pause(bool pause)
{
  if (!is_calling_set_pause_ && cli_set_pause_->service_is_ready()) {
    const auto req =
      std::make_shared<autoware::component_interface_specs_universe::control::SetPause::Service::Request>();
    req->pause = pause;
...
void MotionNode::on_is_start_requested(
  const autoware::component_interface_specs_universe::control::IsStartRequested::Message::ConstSharedPtr msg)
{
